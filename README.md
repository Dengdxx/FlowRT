...
- `flowrt status`：扫描当前用户 runtime socket 并输出 live process handshake 与状态摘要。
...
当前仍未完成安装包和发布流程、多 graph / 多 task 完整语义、跨机器通信、variable payload runtime、Artifact ABI、外部进程组件语义，以及 Rust 用户组件免 Cargo 分发。
