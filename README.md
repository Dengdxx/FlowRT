...
当前仍未完成发布流程、多 graph / 多 task 完整语义、Artifact ABI、外部进程组件语义，以及 Rust 用户组件免 Cargo 分发。`zenoh` 已作为跨主机 copy backend 进入 capability catalog、Rust/C++ runtime backend、真实 transport endpoint 和 mixed demo 路径；生成物会输出 deterministic channel key expression，并对 bounded variable frame 生成 canonical codec。
...
generated supervisor 也会启动自己的 introspection socket，并轮询子进程 PID socket，把子进程 `starting` / `running` / `stale` / `exited` / `failed`、PID、tick 计数、最后一次可见时间和退出状态暴露给 `flowrt status`。
Rust/C++ runtime 已有 backend health 和 reconnect 基础抽象，用稳定状态、毫秒退避和 attempt 计数表达恢复过程。`zenoh` endpoint 已接入自动恢复：本地 session 关闭或 transport 操作失败后，endpoint 会按策略重建 session、publisher 和 subscriber；codec/schema 错误仍按 payload 错误返回，不触发重连。该抽象保持 C ABI 友好的数据形状，便于后续接入 C、Python 和更多语言 runtime；当前尚未提供 C API。
