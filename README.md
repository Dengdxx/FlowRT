...
当前仍未完成安装包和发布流程、多 graph / 多 task 完整语义、`flowrt echo --follow`、C++ generated runtime shell 的 introspection socket 接入、跨机器通信、variable payload runtime、Artifact ABI、外部进程组件语义，以及 Rust 用户组件免 Cargo 分发。

C++ runtime 已提供与 Rust wire JSON 兼容的 introspection socket API 基础，包括 status、channel snapshot、结构化错误响应和 PID 命名 socket 路径；当前还只是 runtime API，尚未由生成的 C++ shell 自动启动或写入 live channel 状态。
