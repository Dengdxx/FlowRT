...
```bash
version=v0.27.0  # 替换为要安装的 release tag
...
**Service policy：** `backend`（native generated Service 与 Operation 支持 `inproc` 与
`zenoh`；external endpoint 选择 `zenoh`）、`timeout_ms`（默认 5000）、
