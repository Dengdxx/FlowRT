...
fn handle_connection(
    ...
    match request {
        ...
        IntrospectionRequest::OperationStatus { operation_id } => {
            let response = match state.status_operation(&operation_id) {
                Ok(operation) => IntrospectionResponse::OperationValue {
                    handshake: handshake.clone(),
                    operation,
                },
        ...
        IntrospectionRequest::OperationResult { operation_id } => {
            let response = match state.result_operation(&operation_id) {
        ...
        IntrospectionRequest::OperationObserve {
            ...
            let response = match state.observe_operation(&operation_id, after_sequence, limit) {
