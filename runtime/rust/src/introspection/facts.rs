...
const DIAGNOSTICS_PAYLOAD_SCHEMA: &str = "flowrt.diagnostics.status";

/// live status、diagnostics 和 recorder diagnostics event 共用的运行态观测事实。
#[derive(Debug, Clone)]
pub(super) struct RuntimeObservabilityFacts {
    status: IntrospectionStatus,
}

#[derive(Debug, Clone, PartialEq, Eq)]
pub(crate) struct RecorderDiagnosticFact {
...
impl RuntimeObservabilityFacts {
    pub(super) fn from_state_inner(
        ...
        let status = IntrospectionStatus {
            ...
            channels: inner
                .channels
                .iter()
                .map(|(name, channel)| channel_status_fact(name, channel))
                .collect(),
    ...
    pub(super) fn from_status_snapshot(mut status: IntrospectionStatus) -> Self {
        status.diagnostics = diagnostics::derive_diagnostics(&status);
        Self { status }
    }

    pub(super) fn status_snapshot(&self) -> IntrospectionStatus {
        self.status.clone()
    }

    #[cfg(test)]
    pub(super) fn diagnostic_snapshot(&self) -> Vec<IntrospectionDiagnostic> {
        self.status.diagnostics.clone()
    }

    pub(super) fn recorder_diagnostic_events(&self) -> Vec<RecorderDiagnosticFact> {
        self.status
            .diagnostics
            .iter()
            .map(RecorderDiagnosticFact::from_diagnostic)
            .collect()
    }
}

impl RecorderDiagnosticFact {
    fn from_diagnostic(diagnostic: &IntrospectionDiagnostic) -> Self {
        ...
        Self {
            ...
            payload_schema: DIAGNOSTICS_PAYLOAD_SCHEMA,
            payload,
            monotonic_ns: diagnostic
                .observed_ms
...
fn channel_status_fact(name: &str, channel: &ChannelState) -> IntrospectionChannelStatus {
...
pub(super) fn derive_diagnostic_facts(
    status: &IntrospectionStatus,
) -> Vec<IntrospectionDiagnostic> {
