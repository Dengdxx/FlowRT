...
int main() {
...

    {
        ...
        auto handle = client.call(AddRequest{3, 5});
...

    {
        ...
        auto h1 = client.call(AddRequest{1, 2});
        auto h2 = client.call(AddRequest{10, 20});
        auto h3 = client.call(AddRequest{100, 200});
...

    {
        ...
        auto handle = client.call(AddRequest{1, 1}, 20);
        assert(!handle.ready());
...

    {
        ...
        flowrt::InprocServiceClient<AddRequest, AddResponse> client(
            service_name, orphan_state, 0, 0);
        auto handle = client.call(AddRequest{1, 1});
...

    {
        ...
        auto h3 = client.call(AddRequest{3, 3});
        auto busy_result = h3.poll();
        assert(busy_result.has_value());
...

    {
        ...
        auto h1 = client.call(AddRequest{10, 20}, 1);
        auto r1 = h1.wait();
        assert(r1.is_err());
...

    {
        ...
        auto handle = client.call(AddRequest{7, 8});
        assert(!handle.ready());
        assert(!handle.poll().has_value());
...

    {
        ...
        client.call(AddRequest{1, 1});
        assert(callback_count == 1);

        client.call(AddRequest{2, 2});
...

    {
        ...
        auto h1 = client.call(AddRequest{-1, 5});
        server.process_pending();
        auto r1 = h1.wait();
        assert(r1.is_err());
        assert(r1.error_code() == flowrt::ServiceError::Rejected);

        auto h2 = client.call(AddRequest{3, 5});
        server.process_pending();
        auto r2 = h2.wait();
        assert(r2.is_ok());
...

    {
        ...
        auto handle = client.call(AddRequest{40, 60});
...

    {
        ...
        auto handle = client.call(EchoRequest{"hello world"});
        server.process_pending();
        auto result = handle.wait();
        assert(result.is_ok());
...

    {
        ...
        auto h1 = normal_client.call(AddRequest{1, 1});
        auto h2 = normal_client.call(AddRequest{2, 2});
        ...
        auto deadlock_handle = same_lane_client.call(AddRequest{4, 4});
        deadlock_handle.poll();
