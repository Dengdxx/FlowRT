...
当前已实现 Rust CLI 的 `check`、`prepare`、`build`、`run`、`launch`、`inspect`、`list`、`nodes`、`status`、`echo` 和 `params` 基础闭环，安装后的 binary 名称为 `flowrt`。FlowRT 作为标准 Linux 应用分发：单个 deb 包同时安装 CLI、Rust runtime crate、C++ runtime header、CMake package 和基础文档。仓库内可以用 `cargo run -p flowrt-cli -- ...` 调试 CLI，但面向用户的文档、示例和最终回复应默认使用安装后的 `flowrt ...` 命令。
...
```text
crates/
  flowrt-cli/          # CLI：check / prepare / build / run / launch / inspect / list / status / echo / params
