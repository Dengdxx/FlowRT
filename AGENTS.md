...
`launch` 运行 FlowRT 管理的 Rust supervisor；supervisor 读取 `flowrt/launch/launch.json`，遍历全部 graph，并按 process group 启动生成应用。C++ only contract 会生成 supervisor-only Rust crate，`launch` 先构建 CMake app 再运行 supervisor，不生成 Rust runtime shell 或 Rust app binary。`inproc` 是单进程 backend，`launch` 必须拒绝 inproc dataflow 跨 RSDL process group；跨 process dataflow 必须选择 `iox2` 或把相关 instance 放回同一 process group。launch manifest 的 process group 必须暴露 `runtimes` 和 `runtime_kind`，task metadata 必须暴露 `inputs`、`outputs` 和 `priority` scheduler hint，便于后续 supervisor/scheduler 消费；graph instance 必须暴露 `runtime`，graph 必须暴露 `channels`。
