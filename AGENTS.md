...
当前 iox2/zenoh endpoint 已有对 peer endpoint 重建后的继续收发回归测试；generated shell 对真实 transport error 仍按 fatal `Status::Error` 处理，自动重连和 backend health/retry policy 需要单独设计。
