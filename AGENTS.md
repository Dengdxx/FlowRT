...
- 分配 stable IDs。
- 填充 defaults。
- 规范化 ordering；不具备顺序语义的 `package.imports` pattern、`bind.dataflow`、`target.runtime` 和 `target.backends` 必须稳定排序，不能让声明顺序改变 canonical Contract IR。`bind.dataflow` 按端点排序；排序不得吞掉重复项，重复声明仍由 parser 或 validator 拒绝。
