...
仓库目前已经形成 MVP 工具链、Rust/C++ runtime shell、跨进程 backend 和安装包闭环。主要目录包括：
...
`flowrt/launch/launch.json` 的 process group 必须包含 `runtimes` 和 `runtime_kind`，graph instance 必须包含 `runtime`，graph 必须包含 `channels`，每条 channel 在 `iox2` backend 下必须暴露 canonical service name；生成的 Rust supervisor 会遍历 manifest 中的全部 graph，并读取 `runtime_kind`，为 Rust process 选择 Rust app executable、为 C++ process 选择 C++ app executable，同时继续拒绝 mixed process group。generated supervisor 还会启动 `runtime=supervisor` 的 introspection socket，轮询子进程 PID socket，把子进程 heartbeat、tick stale 和 exit 状态交给 `flowrt status` 展示；restart policy 和自动重启是独立后续切片。默认构建仍走轻量 inproc 路径。不要提前引入大型依赖、复杂目录或半成品 runtime 代码。

当前已存在 `.github/workflows/ci.yml` CI：Linux 上运行 Rust fmt/test/clippy、C++ runtime CMake/CTest、C++ runtime zenoh smoke、deb 打包安装、deb 用户项目 smoke 和 FlowRT demo smoke，并上传 `flowrt-linux-deb` artifact。CI smoke 会先通过 deb 把 CLI、C++ runtime package 和 Rust runtime crate 安装到系统路径，再用安装后的 `flowrt ...` 跑示例；C++ only demo 执行 build/run/launch，mixed `imu_demo` 只执行 build，Rust-only `import_demo` 执行 run/launch，`mixed_iox2_demo` 与 `imu_demo_iox2` 执行 check，`profile_switch_demo` 执行 profile 切换 smoke，`variable_iox2_demo` 执行 C++ iox2 自动依赖解析、mixed launch 和 bounded variable frame marker 验证，`mixed_zenoh_demo` 执行 zenoh build/launch smoke。该 workflow 暂不做 cache、release 发布或多平台矩阵；含 C++ iox2 组件的生成 CMake 会先查找本机 `iceoryx2-cxx 0.9.1`，找不到时默认通过 `FetchContent` 拉取 `iceoryx2` v0.9.1；含 C++ zenoh 组件的生成 CMake 必须使用本机预装的 `zenohcxx::zenohc`。
...
当前 iox2/zenoh endpoint 已有对 peer endpoint 重建后的继续收发回归测试；Runtime 已提供 C ABI 友好形状的 `BackendHealthState`、`BackendHealthSnapshot`、`ReconnectPolicy` 和 `BackendHealthTracker`。`zenoh` endpoint 已接入自动恢复：本地 session 关闭或 transport 操作失败会重建 session、publisher 和 subscriber，codec/schema 错误不得触发重连。`iox2` endpoint 自动恢复尚未接入前，generated shell 对 iox2 transport error 仍按 fatal `Status::Error` 处理。后续 C、Python 或更多语言 runtime 应复用这套稳定状态形状，不要在 shell 中临时吞掉错误。
