...
work_dir="$(mktemp -d "${TMPDIR:-/tmp}/flowrt-v085-cross-sdk.XXXXXX")"

cleanup() {
    if [[ "${FLOWRT_KEEP_SMOKE_WORKDIR:-0}" == "1" ]]; then
        printf 'preserved v0.8.5 cross SDK smoke work dir: %s\n' "$work_dir" >&2
        return
    fi
    rm -rf "$work_dir"
}
...
require_command() {
    command -v "$1" >/dev/null || {
        printf '%s is required for v0.8.5 cross SDK smoke\n' "$1" >&2
        exit 1
    }
}
...
configure_demo_toolchain() {
    local demo_dir="$1"
    local sdk_root="$2"
    mkdir -p "$demo_dir/.flowrt"
    cat > "$demo_dir/.flowrt/toolchains.toml" <<EOF
[toolchain.linux-arm64]
sdk_overlays = ["$sdk_root"]
pkg_config_libdirs = ["$sdk_root/lib/aarch64-linux-gnu/pkgconfig", "$sdk_root/lib/pkgconfig"]
...
build_demo() {
    ...
    configure_demo_toolchain "$demo_dir" "$sdk_root"

    if ! (
        cd "$demo_dir"
        "$flowrt" doctor --target linux-arm64 > "$work_dir/$demo_name.doctor.out"
    ); then
        sed -n '1,220p' "$work_dir/$demo_name.doctor.out" >&2
        exit 1
    fi
    grep -q 'target platform: linux-arm64' "$work_dir/$demo_name.doctor.out"
    grep -q 'runtime dependency policy: external' "$work_dir/$demo_name.doctor.out"
...
"$flowrt" --version
printf 'v0.8.5 cross SDK smoke work dir: %s\n' "$work_dir"
...
build_demo libjpeg_cross_demo libjpeg_cross_demo "$sdk_root"
build_demo kleidiai_cross_demo kleidiai_cross_demo "$sdk_root"

printf 'v0.8.5 cross SDK demo smoke passed\n'
