...
并行执行。Rust iox2 endpoint 内部依赖非 `Send/Sync` SDK handle，因此含 iox2 route 的
Rust generated shell 当前走 scheduler 线程亲和的 `ReadyBatch::run_local` 路径；纯
send-safe graph 仍走 `WorkerPool`。Contract IR 现为 dataflow route 派生
`thread_affinity` metadata：`iox2` 为 `scheduler_local_commit`，`inproc` / `zenoh`
为 `send_safe`；validator 会重新推导并拒绝手工篡改，launch manifest 与
self-description 会暴露该字段，便于 `status` 或后续工具解释输出 commit 为何留在
scheduler/local owner 线程。这不是长期放弃并发，而是在传输状态未从 App 热路径拆出前
避免 unsafe 包装 backend handle。`v0.10.1` 收紧了 CLI 工程化细节：
