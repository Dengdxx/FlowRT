...
- debt 2 跨进程 strict determinism / global tick lockstep：先做。需 supervisor/transport
  ...
  global tick 基座与示例 prepare。FIFO feedback 确定性仍在本版本后续 commit 中收口。当前跨
  进程 feedback 是 seeded latest-snapshot，跨进程 fault injection determinism 因此不能承诺
  全局同拍。是 debt 3 跨进程片与 debt 1 的前置。
