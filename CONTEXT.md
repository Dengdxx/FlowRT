...
- debt 2 跨进程 strict determinism / global tick lockstep：先做。需 supervisor/transport
  ...
  建模、launch manifest 投影、supervisor global tick coordinator，以及 Rust runtime /
  generated shell 的 `ExternalTick` / `ExternalTickReport` 和 `flowrt_run_tick` 外部步进入口；
  C++ mirror、跨进程 smoke 与 FIFO feedback 确定性仍在本版本后续 commit 中收口。当前跨进程
  feedback 是 seeded latest-snapshot，跨进程 fault injection determinism 因此不能承诺
