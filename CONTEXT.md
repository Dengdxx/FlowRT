...
CLI 已新增 `flowrt init [path] --lang <rust|cpp|c>`，生成 `flowrt.toml`、
`rsdl/robot.rsdl` 和现代 `app/` 用户代码骨架；默认 Rust，可显式生成 C++ 或 C callback
table v0 骨架。未发布状态下也新增 `flowrt add message/module/component`：
省略 RSDL 时从 `flowrt.toml` 找主 RSDL，可追加根 message、当前可解析的
`rsdl/modules/*.rsdl` workspace module 注册，以及 Rust/C++/C native component、同名
instance、最小 periodic task 和现代 `app/` 用户骨架；写入前会重新解析、归一化并校验
更新后的 Contract IR，C component 骨架写入 `app/c/<component>.c`。手写或已有 RSDL 中
...
且不得设置未知 feature bit；`examples/c_counter_demo` 已覆盖 fixed-size counter message
的 C callback table build/run 闭环；C v0 仍只支持 fixed-size plain data 输入/输出，不支持
...
未发布状态下 runtime 已补齐 output transaction primitive：Rust/C++ `Output<T>::write(...)`
...
都会丢弃本次输出。
未发布状态下 Rust/C++ generated runtime shell 已接入 two-phase output commit 主路径：
...
未发布状态下 CLI 已新增正式 `flowrt explain [rsdl] [--format text|json]`：命令与
...
- `v0.11.0` 应把 FlowRT 从“可生成可运行”推进到“可作为 app SDK 顺手使用”：`flowrt init`
  ...
  仍是语义事实源。当前 C v0 用户入口已开放 `flowrt init --lang c` 和
  `flowrt add component --lang c`，骨架进入 `app/c/**`。
- `v0.11.0` 同时落地 C ABI v0 基础：C 侧事实源继续放
  ...
  未发布实现状态下，`language = "c"` 已进入 RSDL/Contract IR/validator/codegen/CLI
  用户入口；validator 会拒绝 C v0 暂不支持的 params、service、operation、
  variable frame、`io_boundary` 和 `external` kind，codegen 只在 C v0 native /
  fixed-size message 范围内生成 callback table adapter。
...
flowrt init [path] --lang c
flowrt add component <Name> --lang c [--input name:Type] [--output name:Type]
