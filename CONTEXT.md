...
`scripts/check-evidence-matrix.sh` 作为证据合同事实源检查所有带 runtime shell snapshot 的
...
boundary input replay source、self-description canonical message identity 和 generated Operation
关键接线。
...
- Operation 控制面：generated Operation runtime 已支持 inproc、zenoh，并在 v0.25.0 接入
  ...
  `--format json` 结构化输出；result/follow JSON 同时暴露原始 payload byte array，并按
  self-description Message ABI 或 `message_frames` canonical frame 解码 `value`，其中
  canonical frame 覆盖无界和 `max=N` 有界 variable 字段。
  generated Rust/C++ hidden task 会同步驱动 Operation runtime retained snapshot 与
  introspection result/event retention 清理；
  ...
  progress/result/error 仍只作为 observation evidence，不参与重放。`v0.27.1` 已收口
  `flowrt replay --verify-operation-observations`：它会读取录制中的
...
- 跨进程 strict determinism / global tick lockstep：需 supervisor/transport
  ...
  `examples/global_tick_demo` 和 `scripts/test-v0233-global-tick-determinism-smoke.sh` 已覆盖
...
- Operation zenoh generated runtime 与 policy 主路径已接线：Rust/C++ generated client API
  仍是 Operation，内部复用 zenoh service transport；runtime 支持 `queue`、
...
- 注入是 **test-only codegen-time overlay**（不改 RSDL 契约结构），镜像 `temporary_island`：场景为
  独立 TOML（`[[inject]]`，按名引用 `instance`/`task`，`kind` 默认 `status_error`，`invocations`
  显式集合或 `from_invocation` 起点），经 `flowrt prepare/build/run --inject <场景>` 投影进
  `ContractArtifactIr.fault_injection`，与 `temporary_overlay` 并列可叠加；置 `test_only=true`、
  `clock_source=simulated_replay`，`bundle`/`deploy` 默认拒绝（需 `--allow-island`）；
...
`v0.15.1` 的核心 Module 是 release evidence contract：发布分支 `dev/vX.Y.Z` 的
release candidate push run 跑完整矩阵和 deb package 后，`Release Evidence Gate`
校验版本、release notes、amd64/arm64 deb 元数据和 `SHA256SUMS`，再上传
`flowrt-release-evidence` artifact。
独立的 `.github/workflows/release.yml` 只监听 `v*` tag，解析 tag 指向的 commit SHA，
查询同一 commit SHA 上成功的 release candidate push run 和 `Release Evidence Gate`，
下载同一 run 的 deb 与 evidence artifact，复核 version/tag/sha、deb 元数据和校验和后创建
GitHub Release。
...
cooperative cancel check，并通过 typed progress publisher 发布 progress。当前 generated
runtime 已支持 `concurrency = "reject" | "queue"`、`preempt = "reject" |
"cancel_running"`、`max_in_flight > 0`、`feedback = "latest" | "fifo"` 和
`result_retention_ms`；queue 按 `queue_depth` 有界排队，抢占先发出 cooperative cancel，
终态 status 按 retention deadline 保留。
...
Message ABI 或 `message_frames` canonical frame 解码后的对象，bounded `string<max=N>` /
`bytes<max=N>` / `sequence<T,max=N>` 使用 generated self-description 中的同一 layout。
generated Rust/C++ hidden task 会驱动
...
当前 `.github/workflows/ci.yml` 是日常快速验证入口，覆盖生成物保护、Rust
格式化/测试/clippy、C++ runtime 和 codegen 编译网；push/PR 会触发该短矩阵，并用
concurrency 取消同一 ref 上的过期 run。日常 CI 不构建 deb、不上传 release evidence，
也不运行历史发布矩阵。

完整发布同款矩阵迁移到 `.github/workflows/release-candidate.yml`，只在
`dev/v*` 发布分支 push 时触发。该 workflow 覆盖生成物保护、Rust
...
runtime、demo smoke、ROS2 bridge smoke 和 release evidence。Linux job 默认运行在官方
ROS2 Jazzy base 容器上；ROS2 bridge smoke 覆盖 Jazzy 和 Lyrical 两个发行版，并安装
对应的 `rmw_zenoh_cpp`。

release candidate 的架构相关 job 使用 `amd64` / `arm64` 双矩阵：Rust fmt/test/clippy、C++ runtime、
...
release candidate workflow 会运行完整矩阵，最后由 `Release Evidence Gate` 校验版本、
release notes、deb 架构、deb 版本和 `SHA256SUMS`，并上传
`flowrt-release-evidence`。tag release 由独立 `release.yml` 处理，解析 tag 指向的
commit SHA 后只查询同一 commit SHA 上成功的 `release-candidate.yml` push run，要求其中
`Release Evidence Gate` 成功，再下载同一 run 的 deb 与 evidence artifact 创建
GitHub Release。tag workflow 不重跑完整矩阵，不再依赖手工
`workflow_dispatch`。
...
生产消费路径。`v0.15.1 CI Release Evidence Smoke` 检查日常 CI 不产出 release evidence、
release candidate 不暴露手工 RC 入口、tag release 只消费同一 commit SHA 的
release-candidate evidence，以及本地 helper 不再触发远端 CI。
...
当前发布先由发布分支 release candidate run 验证分支 HEAD 并产出 release evidence，再由推送对应
`vX.Y.Z` tag 触发 GitHub Release。

workflow 对 Rust 构建产物、FlowRT deps cache 和必要的公开 SDK overlay 做分层缓存；
release artifact、release notes 和 deb 成品在发布分支 release candidate run 中从源码重建；tag release
