...
const PACKAGE_NAME: &str = "zenoh_operation_rust";
...
operation_control_0: operation_control_0.clone(),
    ...
    pub fn run(self, backend: &dyn flowrt::Backend, run_ticks: Option<usize>) -> flowrt::Status {
        ...
        let zenoh_operation_session = match flowrt::zenoh::open_session_from_environment() {
            ...
            Err(error) => {
                eprintln!("FlowRT: failed to open zenoh operation session: {error}");
                status = flowrt::Status::Error;
                None
            }
        };
        if let Some(session) = zenoh_operation_session.as_ref() {
            let _ = app.operation_client_controller_plan.start_client.set(flowrt::zenoh::ZenohServiceClient::open("__flowrt_operation_controller_plan_start", session.clone()));
            let _ = app.operation_client_controller_plan.cancel_client.set(flowrt::zenoh::ZenohServiceClient::open("__flowrt_operation_controller_plan_cancel", session.clone()));
            let _ = app.operation_client_controller_plan.status_client.set(flowrt::zenoh::ZenohServiceClient::open("__flowrt_operation_controller_plan_status", session.clone()));
        }
        let _operation_feedback_endpoint_0 = "__flowrt_operation_controller_plan_feedback";
let _operation_result_endpoint_0 = "__flowrt_operation_controller_plan_result";
let _zenoh_operation_start_server_0 = if let Some(session) = zenoh_operation_session.as_ref() {
...
match flowrt::zenoh::ZenohServiceServer::open("__flowrt_operation_controller_plan_start", session.clone(), operation_start_handler_0) {
...
None
        ...
        let _zenoh_operation_cancel_server_0 = if let Some(session) = zenoh_operation_session.as_ref() {
let operation_cancel_handler_0_control = app.operation_control_0.clone();
let operation_cancel_handler_0 = move |id: flowrt::OperationId| -> flowrt::ServiceResult<flowrt::OperationStatusSnapshot> {
let mut control = operation_cancel_handler_0_control.lock().unwrap_or_else(|poisoned| poisoned.into_inner());
...
match flowrt::zenoh::ZenohServiceServer::open("__flowrt_operation_controller_plan_cancel", session.clone(), operation_cancel_handler_0) {
Ok(server) => Some(server),
Err(error) => {
