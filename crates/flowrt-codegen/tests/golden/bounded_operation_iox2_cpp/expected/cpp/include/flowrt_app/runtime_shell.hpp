...
public:
     ...
     * @param navigator 用户组件实例所有权；shell 在生命周期内独占持有该对象。
     */
    explicit App(
...
private:
    ...
    FlowrtTaskOutcome step_task_navigator_main(std::size_t tick, flowrt::Context& tick_context, flowrt::IntrospectionState& introspection_state, flowrt::ScheduleWaiter& scheduler_events, std::map<std::string, flowrt::IntrospectionTaskHealth>& health_map);
    ...
    FlowrtTaskOutcome step_process_client_proc_task_controller_main(std::size_t tick, flowrt::Context& tick_context, flowrt::IntrospectionState& introspection_state, flowrt::ScheduleWaiter& scheduler_events, std::map<std::string, flowrt::IntrospectionTaskHealth>& health_map);
    ...
    FlowrtTaskOutcome step_process_server_proc_task_navigator_main(std::size_t tick, flowrt::Context& tick_context, flowrt::IntrospectionState& introspection_state, flowrt::ScheduleWaiter& scheduler_events, std::map<std::string, flowrt::IntrospectionTaskHealth>& health_map);
    ...
    std::shared_ptr<NavigatorInterface> navigator_;
    OperationClient_controller_plan operation_client_controller_plan_;
    std::shared_ptr<flowrt::OperationControl> operation_control_0_; 
    std::optional<flowrt::iox2::Iox2FrameServiceServer<flowrt::OperationStartRequest<PlanGoal>, flowrt::OperationStartAck, 40, 49>> operation_start_server_navigator_plan_;
    std::optional<flowrt::iox2::Iox2ServiceServer<flowrt::OperationId, flowrt::OperationStatusSnapshot>> operation_cancel_server_navigator_plan_;
    std::optional<flowrt::iox2::Iox2ServiceServer<flowrt::OperationId, flowrt::OperationStatusSnapshot>> operation_status_server_navigator_plan_;
    flowrt::Status step_operation_navigator_plan(std::size_t tick, flowrt::Context& tick_context, flowrt::IntrospectionState& introspection_state, flowrt::ScheduleWaiter& scheduler_events, std::map<std::string, flowrt::IntrospectionTaskHealth>& health_map);
};
