...
#[test]
fn plan_request_wire_codec_omits_native_padding() {
    let value = sample_plan_request();
    let wire = value.to_wire_vec().unwrap();
    assert_eq!(wire, vec![2, 0, 0, 0]);
    assert_eq!(flowrt_app::messages::PlanRequest::decode_wire(&wire).unwrap(), value);
}
...
#[test]
fn plan_response_wire_codec_omits_native_padding() {
    let value = sample_plan_response();
    let wire = value.to_wire_vec().unwrap();
    assert_eq!(wire, vec![1]);
    assert_eq!(flowrt_app::messages::PlanResponse::decode_wire(&wire).unwrap(), value);
}
