...
constexpr std::array<std::uint8_t, 4> EXPECTED_CMD_BYTES{{2, 0, 0, 0}};
...
void test_cmd_message_abi() {
    ...
    static_assert(sizeof(flowrt_app::Cmd) == 4);
    static_assert(alignof(flowrt_app::Cmd) == 4);
    assert_default_bytes_zero<flowrt_app::Cmd>();
    static_assert(offsetof(flowrt_app::Cmd, value) == 0);
