...
App::App(
    ...
    : source_(std::move(source)),
      sink_(std::move(sink)),
      bind_0_(flowrt::iox2::Iox2FramePubSub<Packet, 60>::open_with_config("FlowRT/bounded_channel_iox2_cpp/default/bind_0/source_packet_to_sink_packet", flowrt::iox2::Iox2ChannelConfig::latest().with_stale_config(flowrt::StaleConfig{flowrt::StalePolicy::Warn}))) {
}
...
flowrt::Status App::step(std::size_t tick, flowrt::Context& tick_context, flowrt::IntrospectionState& introspection_state, flowrt::ScheduleWaiter& scheduler_events, std::map<std::string, flowrt::IntrospectionTaskHealth>& health_map) {
    ...
    (void)health_map;
    {
        health_map["source.main"].name = "source.main";
        health_map["source.main"].lane = "source_serial";
        flowrt::Output<Packet> source_packet;
        if (source_) {
            switch (source_->on_tick(source_packet)) {
        ...
        if (const auto* value = source_packet.as_ref()) {
            ...
            record_introspection_publish_frame(introspection_state, "source.packet_to_sink.packet", "Packet", this->introspection_probe_bind_0, *value, tick_time_ms);
                        break;
                    case flowrt::ChannelWriteOutcome::DroppedOldest:
                        introspection_state.record_route_publish("source.packet_to_sink.packet", tick_time_ms);
                        introspection_state.record_route_drop("source.packet_to_sink.packet");
                        scheduler_events.notify_data();
            record_introspection_publish_frame(introspection_state, "source.packet_to_sink.packet", "Packet", this->introspection_probe_bind_0, *value, tick_time_ms);
                        break;
                    case flowrt::ChannelWriteOutcome::DroppedNewest:
                        ...
                        break;
                    case flowrt::ChannelWriteOutcome::Backpressured:
                        introspection_state.record_route_backpressure("source.packet_to_sink.packet");
                        break;
                }
            }
    ...
    {
        const auto sink_packet = bind_0_.cached_latest_at(tick_time_ms);
        if (sink_packet.stale()) {
            health_map["sink.main"].stale_input += 1;
        }
        health_map["sink.main"].name = "sink.main";
        health_map["sink.main"].lane = "sink_serial";
        if (sink_packet.present()) {
            if (sink_) {
                switch (sink_->on_tick(sink_packet)) {
                    case flowrt::Status::Ok:
                        break;
                    case flowrt::Status::Retry:
                        return flowrt::Status::Retry;
...
FlowrtTaskOutcome App::step_task_source_main(std::size_t tick, flowrt::Context& tick_context, flowrt::IntrospectionState& introspection_state, flowrt::ScheduleWaiter& scheduler_events, std::map<std::string, flowrt::IntrospectionTaskHealth>& health_map) {
    const auto tick_time_ms = static_cast<std::uint64_t>(tick);
    (void)tick_time_ms;
    (void)tick_context;
    (void)introspection_state;
    ...
    std::vector<FlowrtOutputCommit> flowrt_output_commits;
    {
        ...
        if (const auto* value = source_packet.as_ref()) {
            ...
            introspection_state.record_route_backend_health("source.packet_to_sink.packet", app.bind_0_.health());
            if (std::holds_alternative<flowrt::ChannelError>(bind_0_result)) {
                introspection_state.record_route_transport_error("source.packet_to_sink.packet", flowrt::OverflowPolicy::DropOldest, std::get<flowrt::ChannelError>(bind_0_result), "publish transport route");
            } else {
                switch (std::get<flowrt::ChannelWriteOutcome>(bind_0_result)) {
                    case flowrt::ChannelWriteOutcome::Accepted:
                        introspection_state.record_route_publish("source.packet_to_sink.packet", tick_time_ms);
                        scheduler_events.notify_data();
            record_introspection_publish_frame(introspection_state, "source.packet_to_sink.packet", "Packet", app.introspection_probe_bind_0, *value, tick_time_ms);
                        break;
                    case flowrt::ChannelWriteOutcome::DroppedOldest:
                        introspection_state.record_route_publish("source.packet_to_sink.packet", tick_time_ms);
                        introspection_state.record_route_drop("source.packet_to_sink.packet");
...
flowrt::Status App::step_process_sink_proc(std::size_t tick, flowrt::Context& tick_context, flowrt::IntrospectionState& introspection_state, flowrt::ScheduleWaiter& scheduler_events, std::map<std::string, flowrt::IntrospectionTaskHealth>& health_map) {
    const auto tick_time_ms = static_cast<std::uint64_t>(tick);
    (void)tick_time_ms;
    (void)tick_context;
    (void)introspection_state;
...
flowrt::Status App::step_process_sink_proc_startup(std::size_t tick, flowrt::Context& tick_context, flowrt::IntrospectionState& introspection_state, flowrt::ScheduleWaiter& scheduler_events, std::map<std::string, flowrt::IntrospectionTaskHealth>& health_map) {
    const auto tick_time_ms = static_cast<std::uint64_t>(tick);
    (void)tick_time_ms;
    (void)tick_context;
    (void)introspection_state;
...
flowrt::Status App::step_process_sink_proc_shutdown(std::size_t tick, flowrt::Context& tick_context, flowrt::IntrospectionState& introspection_state, flowrt::ScheduleWaiter& scheduler_events, std::map<std::string, flowrt::IntrospectionTaskHealth>& health_map) {
    const auto tick_time_ms = static_cast<std::uint64_t>(tick);
    (void)tick_time_ms;
    (void)tick_context;
    (void)introspection_state;
...
FlowrtTaskOutcome App::step_process_sink_proc_task_sink_main(std::size_t tick, flowrt::Context& tick_context, flowrt::IntrospectionState& introspection_state, flowrt::ScheduleWaiter& scheduler_events, std::map<std::string, flowrt::IntrospectionTaskHealth>& health_map) {
    const auto tick_time_ms = static_cast<std::uint64_t>(tick);
    (void)tick_time_ms;
    (void)tick_context;
    (void)introspection_state;
...
flowrt::Status App::step_process_source_proc(std::size_t tick, flowrt::Context& tick_context, flowrt::IntrospectionState& introspection_state, flowrt::ScheduleWaiter& scheduler_events, std::map<std::string, flowrt::IntrospectionTaskHealth>& health_map) {
    const auto tick_time_ms = static_cast<std::uint64_t>(tick);
    (void)tick_time_ms;
    (void)tick_context;
    (void)introspection_state;
...
flowrt::Status App::step_process_source_proc_startup(std::size_t tick, flowrt::Context& tick_context, flowrt::IntrospectionState& introspection_state, flowrt::ScheduleWaiter& scheduler_events, std::map<std::string, flowrt::IntrospectionTaskHealth>& health_map) {
    const auto tick_time_ms = static_cast<std::uint64_t>(tick);
    (void)tick_time_ms;
    (void)tick_context;
    (void)introspection_state;
...
flowrt::Status App::step_process_source_proc_shutdown(std::size_t tick, flowrt::Context& tick_context, flowrt::IntrospectionState& introspection_state, flowrt::ScheduleWaiter& scheduler_events, std::map<std::string, flowrt::IntrospectionTaskHealth>& health_map) {
    const auto tick_time_ms = static_cast<std::uint64_t>(tick);
    (void)tick_time_ms;
    (void)tick_context;
    (void)introspection_state;
...
FlowrtTaskOutcome App::step_process_source_proc_task_source_main(std::size_t tick, flowrt::Context& tick_context, flowrt::IntrospectionState& introspection_state, flowrt::ScheduleWaiter& scheduler_events, std::map<std::string, flowrt::IntrospectionTaskHealth>& health_map) {
    const auto tick_time_ms = static_cast<std::uint64_t>(tick);
    (void)tick_time_ms;
    (void)tick_context;
    (void)introspection_state;
...
flowrt::Status App::run(const flowrt::Backend& backend, std::optional<std::size_t> run_ticks) {
    ...
    bind_0_.set_schedule_waiter(scheduler_events);
    introspection_state.set_self_description_json(std::string{flowrt_app::self_description_json()});
    this->introspection_probe_bind_0 = register_introspection_channel(introspection_state, "source.packet_to_sink.packet", "Packet", std::nullopt);
    auto introspection_server = flowrt::spawn_status_server(
        flowrt::IntrospectionIdentity{
            ...
            .package = "bounded_channel_iox2_cpp",
            .process = "main",
            .runtime = "cpp",
        },
        introspection_state);
    ...
    if (status == flowrt::Status::Ok && source_) {
        status = source_->on_init(lifecycle_context);
        source_initialized = status == flowrt::Status::Ok;
        introspection_state.record_lifecycle_state("source", source_initialized ? flowrt::LifecycleState::Initialized : flowrt::LifecycleState::Faulted);
    }
    ...
    if (status == flowrt::Status::Ok && source_initialized && source_) {
        status = source_->on_start(lifecycle_context);
        source_started = status == flowrt::Status::Ok;
        introspection_state.record_lifecycle_state("source", source_started ? flowrt::LifecycleState::Running : flowrt::LifecycleState::Faulted);
    }
    ...
    (void)"sink_serial";
    scheduler.add_lane(flowrt::LaneId{flowrt::fnv1a64("source_serial")}, flowrt::LaneKind::Serial);
    (void)"source_serial";
    scheduler.add_task(flowrt::TaskSpec{.id = flowrt::TaskId{1}, .lane = flowrt::LaneId{flowrt::fnv1a64("sink_serial")}, .priority = 0});
    scheduler.add_task(flowrt::TaskSpec{.id = flowrt::TaskId{2}, .lane = flowrt::LaneId{flowrt::fnv1a64("source_serial")}, .priority = 0});
    scheduler.add_periodic(flowrt::PeriodicSpec{.task = flowrt::TaskId{2}, .period = std::chrono::milliseconds{5}});
    ...
    while (status == flowrt::Status::Ok && !shutdown.is_requested() && ((!run_ticks.has_value() || tick_base < *run_ticks) || !pending_task_order.empty())) {
        ...
        {
            auto& health = health_map["source.main"];
            health.name = "source.main";
            health.lane = "source_serial";
        }
        ...
        while (true) {
            ...
            (void)bind_0_.receive_latest_at(tick_time_ms);
                ...
                const auto submitted = worker_pool.submit_collect(admission.task, task_completion_queue, [this, &introspection_state, &scheduler_events, &task_health_mutex, &task_health_from_workers, admission, scheduled_delta_ms, observed_delta_ms, task_clock_source, tick_base, tick_time_ms]() {
                    ...
                    const auto [task_name, task_trigger] = [&]() -> std::pair<std::string_view, std::string_view> {
                        switch (admission.task.value) {
                            case 1: return {"sink.main", "on_message"};
                            case 2: return {"source.main", "periodic"};
...
return task_outcome;
                    ...
                    case 2: {
auto flowrt_lane_guard = flowrt::enter_lane(flowrt::LaneId{flowrt::fnv1a64("source_serial")});
(void)flowrt_lane_guard;
auto task_outcome = step_task_source_main(static_cast<std::size_t>(tick_time_ms), local_context, introspection_state, scheduler_events, local_health_map);
merge_local_health(std::move(local_health_map));
return task_outcome;
}
...
break;
        ...
        if (scheduler.lane_starvation_ticks(flowrt::LaneId{flowrt::fnv1a64("source_serial")}) > fairness_starvation_threshold) {
            for (auto &[name, health] : health_map) {
        ...
        if (status == flowrt::Status::Ok) {
            ...
            const auto next_periodic_deadline_ms = std::optional<std::chrono::milliseconds>{std::min({scheduler.next_deadline(flowrt::TaskId{2})})};
    ...
    if (source_started && source_) {
        const auto stop_status = source_->on_stop(lifecycle_context);
        if (status == flowrt::Status::Ok && stop_status != flowrt::Status::Ok) {
            status = flowrt::Status::Error;
        }
        introspection_state.record_lifecycle_state("source", stop_status == flowrt::Status::Ok ? flowrt::LifecycleState::Stopped : flowrt::LifecycleState::Faulted);
    }
    ...
    if (source_initialized && source_) {
        const auto shutdown_status = source_->on_shutdown(lifecycle_context);
        if (status == flowrt::Status::Ok && shutdown_status != flowrt::Status::Ok) {
            status = flowrt::Status::Error;
        }
        introspection_state.record_lifecycle_state("source", shutdown_status == flowrt::Status::Ok ? flowrt::LifecycleState::ShutDown : flowrt::LifecycleState::Faulted);
    }
...
flowrt::Status App::run_process(const flowrt::Backend& backend, std::string_view process, std::optional<std::size_t> run_ticks) {
    if (process == "sink_proc") {
        return run_process_sink_proc(backend, run_ticks);
    }
    if (process == "source_proc") {
        return run_process_source_proc(backend, run_ticks);
    }
    return flowrt::Status::Error;
}

flowrt::Status App::run_process_sink_proc(const flowrt::Backend& backend, std::optional<std::size_t> run_ticks) {
    flowrt::Context lifecycle_context;
    auto status = flowrt::Status::Ok;
    (void)backend;
    auto shutdown = flowrt::install_signal_shutdown_token();
    ...
    if (status == flowrt::Status::Ok) {
        std::map<std::string, flowrt::IntrospectionTaskHealth> startup_health_map;
        status = step_process_sink_proc_startup(0, lifecycle_context, introspection_state, scheduler_events, startup_health_map);
    }
    flowrt::DeterministicExecutor scheduler{1};
    flowrt::WorkerPool worker_pool{1};
    scheduler.add_lane(flowrt::LaneId{flowrt::fnv1a64("sink_serial")}, flowrt::LaneKind::Serial);
...
auto flowrt_lane_guard = flowrt::enter_lane(flowrt::LaneId{flowrt::fnv1a64("sink_serial")});
(void)flowrt_lane_guard;
auto task_outcome = step_process_sink_proc_task_sink_main(static_cast<std::size_t>(tick_time_ms), local_context, introspection_state, scheduler_events, local_health_map);
merge_local_health(std::move(local_health_map));
return task_outcome;
}
                    default: return FlowrtTaskOutcome::error(std::vector<FlowrtOutputCommit>{});
...
break;
        ...
        if (status == flowrt::Status::Ok) {
            ...
            const auto next_periodic_deadline_ms = std::optional<std::chrono::milliseconds>{};
            const auto next_wake_deadline = next_periodic_deadline_ms.has_value()
                ? std::optional<std::chrono::steady_clock::time_point>{
                      std::chrono::steady_clock::now() +
                      std::chrono::milliseconds{static_cast<std::chrono::milliseconds::rep>(
    ...
    if (status == flowrt::Status::Ok) {
        std::map<std::string, flowrt::IntrospectionTaskHealth> shutdown_health_map;
        status = step_process_sink_proc_shutdown(0, lifecycle_context, introspection_state, scheduler_events, shutdown_health_map);
    }
    if (sink_started && sink_) {
        const auto stop_status = sink_->on_stop(lifecycle_context);
        if (status == flowrt::Status::Ok && stop_status != flowrt::Status::Ok) {
...
flowrt::Status App::run_process_source_proc(const flowrt::Backend& backend, std::optional<std::size_t> run_ticks) {
    flowrt::Context lifecycle_context;
    auto status = flowrt::Status::Ok;
    (void)backend;
    auto shutdown = flowrt::install_signal_shutdown_token();
    ...
    if (status == flowrt::Status::Ok) {
        std::map<std::string, flowrt::IntrospectionTaskHealth> startup_health_map;
        status = step_process_source_proc_startup(0, lifecycle_context, introspection_state, scheduler_events, startup_health_map);
    }
    flowrt::DeterministicExecutor scheduler{1};
    flowrt::WorkerPool worker_pool{1};
    scheduler.add_lane(flowrt::LaneId{flowrt::fnv1a64("source_serial")}, flowrt::LaneKind::Serial);
    (void)"source_serial";
    scheduler.add_task(flowrt::TaskSpec{.id = flowrt::TaskId{1}, .lane = flowrt::LaneId{flowrt::fnv1a64("source_serial")}, .priority = 0});
        ...
        while (true) {
            ...
            for (const auto admission : ready_batch.admissions()) {
                ...
                const auto submitted = worker_pool.submit_collect(admission.task, task_completion_queue, [this, &introspection_state, &scheduler_events, &task_health_mutex, &task_health_from_workers, admission, scheduled_delta_ms, observed_delta_ms, task_clock_source, tick_base, tick_time_ms]() {
                    ...
                    const auto [task_name, task_trigger] = [&]() -> std::pair<std::string_view, std::string_view> {
                        switch (admission.task.value) {
                            case 1: return {"source.main", "periodic"};
                            default: return {"__flowrt_hidden", "on_message"};
                        }
                    }();
                    auto local_context = flowrt::Context::with_timing(flowrt::TaskTiming{
...
auto flowrt_lane_guard = flowrt::enter_lane(flowrt::LaneId{flowrt::fnv1a64("source_serial")});
(void)flowrt_lane_guard;
auto task_outcome = step_process_source_proc_task_source_main(static_cast<std::size_t>(tick_time_ms), local_context, introspection_state, scheduler_events, local_health_map);
merge_local_health(std::move(local_health_map));
return task_outcome;
}
                    default: return FlowrtTaskOutcome::error(std::vector<FlowrtOutputCommit>{});
...
break;
    ...
    if (status == flowrt::Status::Ok) {
        std::map<std::string, flowrt::IntrospectionTaskHealth> shutdown_health_map;
        status = step_process_source_proc_shutdown(0, lifecycle_context, introspection_state, scheduler_events, shutdown_health_map);
    }
    if (source_started && source_) {
        const auto stop_status = source_->on_stop(lifecycle_context);
        if (status == flowrt::Status::Ok && stop_status != flowrt::Status::Ok) {
