...
const PACKAGE_NAME: &str = "fault_injection_restart_demo";
...
pub struct App {
    ...
    flaky: std::sync::Arc<std::sync::Mutex<Box<dyn Flaky + Send>>>,
...
impl App {
    ...
    fn step(
        ...
        let _ = tick_time_ms;
        {
            ...
            if sample.present() {
                ...
                match self.flaky.lock().unwrap_or_else(|poisoned| poisoned.into_inner()).on_tick(sample, &mut echo) {
                    flowrt::Status::Ok => {}
    ...
    fn step_task_flaky_main(
        __flowrt_component_flaky: std::sync::Arc<std::sync::Mutex<Box<dyn Flaky + Send>>>,
        ...
        let mut __flowrt_output_commits: Vec<FlowrtOutputCommit> = Vec::new();
        {
            ...
            if sample.present() {
                let mut echo = flowrt::Output::<Sample>::new();
                match __flowrt_component_flaky.lock().unwrap_or_else(|poisoned| poisoned.into_inner()).on_tick(sample, &mut echo) {
                    flowrt::Status::Ok => {}
    ...
    pub fn run(self, backend: &dyn flowrt::Backend, run_ticks: Option<usize>) -> flowrt::Status {
        ...
        let mut boundary_input_feed_seen_revision_for_flaky_main: u64 = 0;
                ...
                || flaky_next_restart_ms.is_some())
        {
            ...
            loop {
                ...
                if app.boundary_input_feed.revision() != boundary_input_feed_seen_revision_for_flaky_main {
                    boundary_input_feed_seen_revision_for_flaky_main = app.boundary_input_feed.revision();
                ...
                for admission in ready_batch.admissions().iter().copied() {
                    ...
                    let submitted = match admission.task {
                        flowrt::TaskId(1) => {
                            ...
                            __inject_count_1 += 1;
                            let __inject_fault_1 = __inject_count_1 >= 1u64;
                            ...
                            let task_outcome = if __inject_fault_1 {
                                ...
                                Self::step_task_flaky_main(__flowrt_component_flaky, __flowrt_input_sample_value, __flowrt_input_sample_stale, __flowrt_input_sample_revision, tick_time_ms as usize, &mut local_context, &introspection_state, &scheduler_events, &mut local_health_map)
                            };
                    ...
                    let submitted = match admission.task {
                        flowrt::TaskId(1) => {
                            ...
                            let task_outcome = if __inject_fault_1 {
                                ...
                                let _flowrt_lane_guard = flowrt::enter_lane(flowrt::LaneId(1));
                                Self::step_process_main_task_flaky_main(__flowrt_component_flaky, __flowrt_input_sample_value, __flowrt_input_sample_stale, __flowrt_input_sample_revision, tick_time_ms as usize, &mut local_context, &introspection_state, &scheduler_events, &mut local_health_map)
                            };
                            if let Some(health) = local_health_map.get_mut(task_name) {
                                health.inflight = false;
                                health.scheduled_time_ms = Some(admission.scheduled_time_ms);
