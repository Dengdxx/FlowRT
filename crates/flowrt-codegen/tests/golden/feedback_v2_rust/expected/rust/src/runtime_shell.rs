...
const PACKAGE_NAME: &str = "feedback_v2_rust";
...
pub struct App {
    ...
    bind_0: std::sync::Arc<std::sync::Mutex<flowrt::LatestChannel<Cmd>>>,
    introspection_probe_bind_0: std::sync::OnceLock<flowrt::IntrospectionChannelProbe>,
    bind_1: std::sync::Arc<std::sync::Mutex<flowrt::FifoChannel<State>>>,
...
impl App {
    pub fn new(
        ...
        Self {
            ...
            bind_1: std::sync::Arc::new(std::sync::Mutex::new(flowrt::FifoChannel::new(2, flowrt::OverflowPolicy::DropOldest))),
    ...
    fn step(
        ...
        let _ = tick_time_ms;
        {
            ...
            let state_read = __flowrt_bind_1_guard.pop_at(tick_time_ms);
            let __flowrt_state_revision = __flowrt_bind_1_guard.revision();
            let state = state_read.view();
            ...
            if let Some(value) = cmd.as_ref().cloned() {
                ...
                if let Some(introspection_probe_bind_0_probe) = self.introspection_probe_bind_0.get() {
                    record_introspection_publish_copy(&introspection_state, "controller.cmd_to_plant.cmd", "Cmd", introspection_probe_bind_0_probe, &value, tick_time_ms);
                }
            }
        }
        {
            let __flowrt_bind_0_guard = self.bind_0.lock().unwrap_or_else(|poisoned| poisoned.into_inner());
            let cmd = __flowrt_bind_0_guard.view_at(tick_time_ms);
            let __flowrt_cmd_revision = __flowrt_bind_0_guard.revision();
            ...
            if let Some(value) = state.as_ref().cloned() {
                match self.bind_1.lock().unwrap_or_else(|poisoned| poisoned.into_inner()).push_at(value.clone(), tick_time_ms) {
                ...
                if let Some(introspection_probe_bind_1_probe) = self.introspection_probe_bind_1.get() {
                    record_introspection_publish_copy(&introspection_state, "plant.state_to_controller.state", "State", introspection_probe_bind_1_probe, &value, tick_time_ms);
                }
                    }
                ...
                if let Some(introspection_probe_bind_1_probe) = self.introspection_probe_bind_1.get() {
                    ...
                    Ok(flowrt::ChannelWriteOutcome::Backpressured) => {
                        ...
                        health_map.entry("plant.main".to_string()).or_default().backpressure += 1;
                        return flowrt::Status::Retry;
                    }
                    Err(flowrt::ChannelError::Overflow) => {
                        introspection_state.record_route_overflow("plant.state_to_controller.state");
                        health_map.entry("plant.main".to_string()).or_default().overflow += 1;
    ...
    fn step_task_plant_main(
        ...
        let mut __flowrt_output_commits: Vec<FlowrtOutputCommit> = Vec::new();
        {
            ...
            if let Some(value) = state.as_ref().cloned() {
                ...
                match app.bind_1.lock().unwrap_or_else(|poisoned| poisoned.into_inner()).push_at(value.clone(), tick_time_ms) {
                    Ok(flowrt::ChannelWriteOutcome::Accepted) => {
                        introspection_state.record_route_publish("plant.state_to_controller.state", Some(tick_time_ms));
                        scheduler_events.notify_data();
    ...
    #[allow(dead_code)]
    fn step_process_main_task_controller_main(
        __flowrt_component_controller: std::sync::Arc<std::sync::Mutex<Box<dyn Controller + Send>>>,
        __flowrt_input_state_value: Option<State>,
        __flowrt_input_state_stale: bool,
        __flowrt_input_state_revision: u64,
    ...
    #[allow(dead_code)]
    fn step_process_main_task_plant_main(
        __flowrt_component_plant: std::sync::Arc<std::sync::Mutex<Box<dyn Plant + Send>>>,
        __flowrt_input_cmd_value: Option<Cmd>,
        __flowrt_input_cmd_stale: bool,
        __flowrt_input_cmd_revision: u64,
                    ...
                    let submitted = match admission.task {
                        flowrt::TaskId(1) => {
                            ...
                            let (__flowrt_input_state_value, __flowrt_input_state_stale, __flowrt_input_state_revision) = {
                                ...
                                let __flowrt_fifo_read = __flowrt_bind_1_snapshot_guard.pop_at(tick_time_ms);
                                let __flowrt_state_snapshot_view = __flowrt_fifo_read.view();
                                (__flowrt_state_snapshot_view.as_ref().cloned(), __flowrt_state_snapshot_view.stale(), __flowrt_bind_1_snapshot_guard.revision())
                            };
                        ...
                        flowrt::TaskId(2) => {
                            ...
                            let (__flowrt_input_cmd_value, __flowrt_input_cmd_stale, __flowrt_input_cmd_revision) = {
                                ...
                                let __flowrt_cmd_snapshot_view = __flowrt_bind_0_snapshot_guard.view_at(tick_time_ms);
                                (__flowrt_cmd_snapshot_view.as_ref().cloned(), __flowrt_cmd_snapshot_view.stale(), __flowrt_bind_0_snapshot_guard.revision())
                            };
                            let introspection_state = introspection_state.clone();
                            let scheduler_events = scheduler_events.clone();
                            let task_health_from_worker = task_health_from_workers.clone();
                ...
                for admission in ready_batch.admissions().iter().copied() {
                    ...
                    let submitted = match admission.task {
                        flowrt::TaskId(1) => {
                            ...
                            let task_outcome = {
                                let _flowrt_lane_guard = flowrt::enter_lane(flowrt::LaneId(1));
                                Self::step_process_main_task_controller_main(__flowrt_component_controller, __flowrt_input_state_value, __flowrt_input_state_stale, __flowrt_input_state_revision, tick_time_ms as usize, &mut local_context, &introspection_state, &scheduler_events, &mut local_health_map)
                            };
                            if let Some(health) = local_health_map.get_mut(task_name) {
                                health.inflight = false;
                                health.scheduled_time_ms = Some(admission.scheduled_time_ms);
                        ...
                        flowrt::TaskId(2) => {
                            ...
                            let task_outcome = {
                                let _flowrt_lane_guard = flowrt::enter_lane(flowrt::LaneId(2));
                                Self::step_process_main_task_plant_main(__flowrt_component_plant, __flowrt_input_cmd_value, __flowrt_input_cmd_stale, __flowrt_input_cmd_revision, tick_time_ms as usize, &mut local_context, &introspection_state, &scheduler_events, &mut local_health_map)
                            };
                            if let Some(health) = local_health_map.get_mut(task_name) {
                                health.inflight = false;
                                health.scheduled_time_ms = Some(admission.scheduled_time_ms);
