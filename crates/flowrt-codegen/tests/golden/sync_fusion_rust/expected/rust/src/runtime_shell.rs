...
const PACKAGE_NAME: &str = "sync_fusion_rust_demo";
...
pub struct App {
    ...
    imu_src: std::sync::Arc<std::sync::Mutex<Box<dyn ImuSrc + Send>>>,
    odom_src: std::sync::Arc<std::sync::Mutex<Box<dyn OdomSrc + Send>>>,
    fusion: std::sync::Arc<std::sync::Mutex<Box<dyn Fusion + Send>>>,
    sink: std::sync::Arc<std::sync::Mutex<Box<dyn Sink + Send>>>,
    bind_0: std::sync::Arc<std::sync::Mutex<flowrt::LatestChannel<Estimate>>>,
    ...
    bind_2: std::sync::Arc<std::sync::Mutex<flowrt::LatestChannel<Odom>>>,
    introspection_probe_bind_2: std::sync::OnceLock<flowrt::IntrospectionChannelProbe>,
    __flowrt_sync_fusion_main: std::sync::Arc<std::sync::Mutex<flowrt::synchronizer::Synchronizer<()>>>,
}

impl App {
    pub fn new(
        imu_src: Box<dyn ImuSrc + Send>,
        odom_src: Box<dyn OdomSrc + Send>,
        fusion: Box<dyn Fusion + Send>,
        sink: Box<dyn Sink + Send>,
    ) -> Self {
        let startup_status = flowrt::Status::Ok;
        let imu_src = std::sync::Arc::new(std::sync::Mutex::new(imu_src));
        let odom_src = std::sync::Arc::new(std::sync::Mutex::new(odom_src));
        let fusion = std::sync::Arc::new(std::sync::Mutex::new(fusion));
        let sink = std::sync::Arc::new(std::sync::Mutex::new(sink));
        Self {
            imu_src: imu_src.clone(),
            odom_src: odom_src.clone(),
            fusion: fusion.clone(),
            sink: sink.clone(),
            ...
            bind_2: std::sync::Arc::new(std::sync::Mutex::new(flowrt::LatestChannel::with_stale_config(flowrt::StaleConfig::new(None, flowrt::StalePolicy::Warn)))),
            introspection_probe_bind_2: std::sync::OnceLock::new(),
            __flowrt_sync_fusion_main: std::sync::Arc::new(std::sync::Mutex::new(flowrt::synchronizer::Synchronizer::<()>::new(2, 8, 5000000))),
            startup_status,
        }
    }
    #[allow(dead_code)]
    fn step(
        ...
        {
            {
                let __h = health_map.entry("imu_src.main".to_string()).or_default();
                __h.name = "imu_src.main".to_string();
                __h.lane = "imu_src_serial".to_string();
            }
            let mut imu = flowrt::Output::<Imu>::new();
            match self.imu_src.lock().unwrap_or_else(|poisoned| poisoned.into_inner()).on_tick(&mut imu) {
                flowrt::Status::Ok => {}
            ...
            if let Some(value) = imu.as_ref().cloned() {
                ...
                introspection_state.record_route_publish("imu_src.imu_to_fusion.imu", Some(tick_time_ms));
                scheduler_events.notify_data();
                if let Some(introspection_probe_bind_1_probe) = self.introspection_probe_bind_1.get() {
                    record_introspection_publish_copy(&introspection_state, "imu_src.imu_to_fusion.imu", "Imu", introspection_probe_bind_1_probe, &value, tick_time_ms);
                }
            }
        }
        {
            {
                let __h = health_map.entry("odom_src.main".to_string()).or_default();
                __h.name = "odom_src.main".to_string();
                __h.lane = "odom_src_serial".to_string();
            }
            let mut odom = flowrt::Output::<Odom>::new();
            match self.odom_src.lock().unwrap_or_else(|poisoned| poisoned.into_inner()).on_tick(&mut odom) {
                flowrt::Status::Ok => {}
                flowrt::Status::Retry => return flowrt::Status::Retry,
                flowrt::Status::Error => return flowrt::Status::Error,
            }
            if let Some(value) = odom.as_ref().cloned() {
                self.bind_2.lock().unwrap_or_else(|poisoned| poisoned.into_inner()).publish_at(value.clone(), tick_time_ms);
                introspection_state.record_route_publish("odom_src.odom_to_fusion.odom", Some(tick_time_ms));
                scheduler_events.notify_data();
                if let Some(introspection_probe_bind_2_probe) = self.introspection_probe_bind_2.get() {
                    record_introspection_publish_copy(&introspection_state, "odom_src.odom_to_fusion.odom", "Odom", introspection_probe_bind_2_probe, &value, tick_time_ms);
                }
            }
        }
        {
            ...
            record_introspection_input_read(&introspection_state, "fusion.main.imu", "fusion.main", "imu", "imu_src.imu_to_fusion.imu", "Imu", &imu, __flowrt_imu_revision, tick_time_ms);
            if imu.stale() {
                health_map.entry("fusion.main".to_string()).or_default().stale_input += 1;
            }
            let __flowrt_bind_2_guard = self.bind_2.lock().unwrap_or_else(|poisoned| poisoned.into_inner());
            let odom = __flowrt_bind_2_guard.view_at(tick_time_ms);
            let __flowrt_odom_revision = __flowrt_bind_2_guard.revision();
            record_introspection_input_read(&introspection_state, "fusion.main.odom", "fusion.main", "odom", "odom_src.odom_to_fusion.odom", "Odom", &odom, __flowrt_odom_revision, tick_time_ms);
            if odom.stale() {
                health_map.entry("fusion.main".to_string()).or_default().stale_input += 1;
            }
            if let Some(__flowrt_sync_sample) = imu.as_ref() {
                self.__flowrt_sync_fusion_main.lock().unwrap_or_else(|poisoned| poisoned.into_inner()).push(0, (__flowrt_sync_sample.stamp_ns as u64).saturating_mul(1), ());
            }
            if let Some(__flowrt_sync_sample) = odom.as_ref() {
                self.__flowrt_sync_fusion_main.lock().unwrap_or_else(|poisoned| poisoned.into_inner()).push(1, (__flowrt_sync_sample.stamp_ns as u64).saturating_mul(1), ());
            }
            let __flowrt_sync_fusion_main_ready = self.__flowrt_sync_fusion_main.lock().unwrap_or_else(|poisoned| poisoned.into_inner()).poll().is_some();
            {
                let __h = health_map.entry("fusion.main".to_string()).or_default();
                __h.name = "fusion.main".to_string();
                __h.lane = "fusion_serial".to_string();
            }
            if __flowrt_sync_fusion_main_ready {
                let mut estimate = flowrt::Output::<Estimate>::new();
                match self.fusion.lock().unwrap_or_else(|poisoned| poisoned.into_inner()).on_tick(imu, odom, &mut estimate) {
                    flowrt::Status::Ok => {}
                    flowrt::Status::Retry => return flowrt::Status::Retry,
                    flowrt::Status::Error => return flowrt::Status::Error,
                }
                if let Some(value) = estimate.as_ref().cloned() {
                    self.bind_0.lock().unwrap_or_else(|poisoned| poisoned.into_inner()).publish_at(value.clone(), tick_time_ms);
                    introspection_state.record_route_publish("fusion.estimate_to_sink.estimate", Some(tick_time_ms));
                    scheduler_events.notify_data();
                    if let Some(introspection_probe_bind_0_probe) = self.introspection_probe_bind_0.get() {
                        record_introspection_publish_copy(&introspection_state, "fusion.estimate_to_sink.estimate", "Estimate", introspection_probe_bind_0_probe, &value, tick_time_ms);
                    }
                }
            }
        }
        {
            ...
            let estimate = __flowrt_bind_0_guard.view_at(tick_time_ms);
            let __flowrt_estimate_revision = __flowrt_bind_0_guard.revision();
            record_introspection_input_read(&introspection_state, "sink.main.estimate", "sink.main", "estimate", "fusion.estimate_to_sink.estimate", "Estimate", &estimate, __flowrt_estimate_revision, tick_time_ms);
            if estimate.stale() {
            ...
            if estimate.present() {
                match self.sink.lock().unwrap_or_else(|poisoned| poisoned.into_inner()).on_tick(estimate) {
                    flowrt::Status::Ok => {}
                    flowrt::Status::Retry => return flowrt::Status::Retry,
                    flowrt::Status::Error => return flowrt::Status::Error,
                }
    ...
    #[allow(dead_code)]
    fn step_task_fusion_main(
        __flowrt_component_fusion: std::sync::Arc<std::sync::Mutex<Box<dyn Fusion + Send>>>,
        ...
        let mut __flowrt_output_commits: Vec<FlowrtOutputCommit> = Vec::new();
        {
            ...
            if let Some(__flowrt_sync_sample) = imu.as_ref() {
                __flowrt_sync_fusion_main.lock().unwrap_or_else(|poisoned| poisoned.into_inner()).push(0, (__flowrt_sync_sample.stamp_ns as u64).saturating_mul(1), ());
            }
            if let Some(__flowrt_sync_sample) = odom.as_ref() {
                __flowrt_sync_fusion_main.lock().unwrap_or_else(|poisoned| poisoned.into_inner()).push(1, (__flowrt_sync_sample.stamp_ns as u64).saturating_mul(1), ());
            }
            let __flowrt_sync_fusion_main_ready = __flowrt_sync_fusion_main.lock().unwrap_or_else(|poisoned| poisoned.into_inner()).poll().is_some();
            {
                let __h = health_map.entry("fusion.main".to_string()).or_default();
                __h.name = "fusion.main".to_string();
                __h.lane = "fusion_serial".to_string();
            ...
            if __flowrt_sync_fusion_main_ready {
                let mut estimate = flowrt::Output::<Estimate>::new();
                match __flowrt_component_fusion.lock().unwrap_or_else(|poisoned| poisoned.into_inner()).on_tick(imu, odom, &mut estimate) {
                    flowrt::Status::Ok => {}
    ...
    #[allow(dead_code)]
    fn step_task_imu_src_main(
        __flowrt_component_imu_src: std::sync::Arc<std::sync::Mutex<Box<dyn ImuSrc + Send>>>,
        tick: usize,
        _tick_context: &mut flowrt::Context,
        introspection_state: &flowrt::IntrospectionState,
        scheduler_events: &flowrt::ScheduleWaiter,
        ...
        let mut __flowrt_output_commits: Vec<FlowrtOutputCommit> = Vec::new();
        {
            ...
            let mut imu = flowrt::Output::<Imu>::new();
            match __flowrt_component_imu_src.lock().unwrap_or_else(|poisoned| poisoned.into_inner()).on_tick(&mut imu) {
                flowrt::Status::Ok => {}
                flowrt::Status::Retry => return flowrt::TaskRunOutcome::retry(Vec::new()),
                flowrt::Status::Error => return flowrt::TaskRunOutcome::error(Vec::new()),
            }
    ...
    #[allow(dead_code)]
    fn step_task_odom_src_main(
        __flowrt_component_odom_src: std::sync::Arc<std::sync::Mutex<Box<dyn OdomSrc + Send>>>,
        tick: usize,
        _tick_context: &mut flowrt::Context,
        introspection_state: &flowrt::IntrospectionState,
        scheduler_events: &flowrt::ScheduleWaiter,
        ...
        let mut __flowrt_output_commits: Vec<FlowrtOutputCommit> = Vec::new();
        {
            ...
            let mut odom = flowrt::Output::<Odom>::new();
            match __flowrt_component_odom_src.lock().unwrap_or_else(|poisoned| poisoned.into_inner()).on_tick(&mut odom) {
                flowrt::Status::Ok => {}
                flowrt::Status::Retry => return flowrt::TaskRunOutcome::retry(Vec::new()),
                flowrt::Status::Error => return flowrt::TaskRunOutcome::error(Vec::new()),
            }
            if let Some(value) = odom.as_ref().cloned() {
                let value = value.clone();
                __flowrt_output_commits.push(Box::new(move |app, introspection_state, scheduler_events, _health_map| {
                app.bind_2.lock().unwrap_or_else(|poisoned| poisoned.into_inner()).publish_at(value.clone(), tick_time_ms);
                introspection_state.record_route_publish("odom_src.odom_to_fusion.odom", Some(tick_time_ms));
                scheduler_events.notify_data();
    ...
    fn step_task_sink_main(
        ...
        __flowrt_input_estimate_value: Option<Estimate>,
        __flowrt_input_estimate_stale: bool,
        __flowrt_input_estimate_revision: u64,
        tick: usize,
        _tick_context: &mut flowrt::Context,
        introspection_state: &flowrt::IntrospectionState,
        scheduler_events: &flowrt::ScheduleWaiter,
        ...
        let mut __flowrt_output_commits: Vec<FlowrtOutputCommit> = Vec::new();
        {
            let estimate = flowrt::Latest::new(__flowrt_input_estimate_value.as_ref(), __flowrt_input_estimate_stale);
            let __flowrt_estimate_revision = __flowrt_input_estimate_revision;
            record_introspection_input_read(&introspection_state, "sink.main.estimate", "sink.main", "estimate", "fusion.estimate_to_sink.estimate", "Estimate", &estimate, __flowrt_estimate_revision, tick_time_ms);
            if estimate.stale() {
                health_map.entry("sink.main".to_string()).or_default().stale_input += 1;
            }
            ...
            if estimate.present() {
                match __flowrt_component_sink.lock().unwrap_or_else(|poisoned| poisoned.into_inner()).on_tick(estimate) {
                    flowrt::Status::Ok => {}
                    flowrt::Status::Retry => return flowrt::TaskRunOutcome::retry(Vec::new()),
                    flowrt::Status::Error => return flowrt::TaskRunOutcome::error(Vec::new()),
                }
    ...
    #[allow(dead_code)]
    fn step_process_main_task_fusion_main(
        __flowrt_component_fusion: std::sync::Arc<std::sync::Mutex<Box<dyn Fusion + Send>>>,
        __flowrt_input_imu_value: Option<Imu>,
        __flowrt_input_imu_stale: bool,
        __flowrt_input_imu_revision: u64,
    ...
    #[allow(dead_code)]
    fn step_process_main_task_imu_src_main(
        __flowrt_component_imu_src: std::sync::Arc<std::sync::Mutex<Box<dyn ImuSrc + Send>>>,
        tick: usize,
        _tick_context: &mut flowrt::Context,
        introspection_state: &flowrt::IntrospectionState,
    ...
    #[allow(dead_code)]
    fn step_process_main_task_odom_src_main(
        __flowrt_component_odom_src: std::sync::Arc<std::sync::Mutex<Box<dyn OdomSrc + Send>>>,
        tick: usize,
        _tick_context: &mut flowrt::Context,
        introspection_state: &flowrt::IntrospectionState,
    ...
    #[allow(dead_code)]
    fn step_process_main_task_sink_main(
        __flowrt_component_sink: std::sync::Arc<std::sync::Mutex<Box<dyn Sink + Send>>>,
        __flowrt_input_estimate_value: Option<Estimate>,
        __flowrt_input_estimate_stale: bool,
        __flowrt_input_estimate_revision: u64,
    ...
    pub fn run(self, backend: &dyn flowrt::Backend, run_ticks: Option<usize>) -> flowrt::Status {
        ...
        let introspection_probe_bind_0 = register_introspection_channel(&introspection_state, "fusion.estimate_to_sink.estimate", "Estimate", Some(8));
        ...
        introspection_state.register_route(flowrt::IntrospectionRouteStatus {
            name: "fusion.estimate_to_sink.estimate".to_string(),
            from: "fusion.estimate".to_string(),
            to: "sink.estimate".to_string(),
            message_type: "Estimate".to_string(),
        ...
        introspection_state.record_input_status(flowrt::IntrospectionInputStatus {
            ...
            input: "estimate".to_string(),
            channel: "fusion.estimate_to_sink.estimate".to_string(),
            message_type: "Estimate".to_string(),
            ..Default::default()
        });
        let introspection_probe_bind_1 = register_introspection_channel(&introspection_state, "imu_src.imu_to_fusion.imu", "Imu", Some(16));
        let _ = app.introspection_probe_bind_1.set(introspection_probe_bind_1);
        introspection_state.register_route(flowrt::IntrospectionRouteStatus {
            name: "imu_src.imu_to_fusion.imu".to_string(),
            from: "imu_src.imu".to_string(),
            to: "fusion.imu".to_string(),
            message_type: "Imu".to_string(),
            backend: "inproc".to_string(),
            selected_reason: "profile_default".to_string(),
            ..Default::default()
        });
        let introspection_probe_bind_2 = register_introspection_channel(&introspection_state, "odom_src.odom_to_fusion.odom", "Odom", Some(16));
        let _ = app.introspection_probe_bind_2.set(introspection_probe_bind_2);
        introspection_state.register_route(flowrt::IntrospectionRouteStatus {
            name: "odom_src.odom_to_fusion.odom".to_string(),
            from: "odom_src.odom".to_string(),
            to: "fusion.odom".to_string(),
        ...
        .ok();
        let mut imu_src_initialized = false;
        let mut imu_src_started = false;
        introspection_state.record_lifecycle_state("imu_src", flowrt::LifecycleState::Uninitialized);
        let mut odom_src_initialized = false;
        let mut odom_src_started = false;
        introspection_state.record_lifecycle_state("odom_src", flowrt::LifecycleState::Uninitialized);
        let mut fusion_initialized = false;
        let mut fusion_started = false;
        ...
        if status == flowrt::Status::Ok {
            status = app.imu_src.lock().unwrap_or_else(|poisoned| poisoned.into_inner()).on_init(&mut lifecycle_context);
            imu_src_initialized = status == flowrt::Status::Ok;
            introspection_state.record_lifecycle_state("imu_src", if imu_src_initialized { flowrt::LifecycleState::Initialized } else { flowrt::LifecycleState::Faulted });
        }
        if status == flowrt::Status::Ok {
            status = app.odom_src.lock().unwrap_or_else(|poisoned| poisoned.into_inner()).on_init(&mut lifecycle_context);
            odom_src_initialized = status == flowrt::Status::Ok;
            introspection_state.record_lifecycle_state("odom_src", if odom_src_initialized { flowrt::LifecycleState::Initialized } else { flowrt::LifecycleState::Faulted });
        }
        if status == flowrt::Status::Ok {
            status = app.fusion.lock().unwrap_or_else(|poisoned| poisoned.into_inner()).on_init(&mut lifecycle_context);
            fusion_initialized = status == flowrt::Status::Ok;
            introspection_state.record_lifecycle_state("fusion", if fusion_initialized { flowrt::LifecycleState::Initialized } else { flowrt::LifecycleState::Faulted });
        }
        if status == flowrt::Status::Ok {
        ...
        if status == flowrt::Status::Ok && imu_src_initialized {
            status = app.imu_src.lock().unwrap_or_else(|poisoned| poisoned.into_inner()).on_start(&mut lifecycle_context);
            imu_src_started = status == flowrt::Status::Ok;
            introspection_state.record_lifecycle_state("imu_src", if imu_src_started { flowrt::LifecycleState::Running } else { flowrt::LifecycleState::Faulted });
        }
        if status == flowrt::Status::Ok && odom_src_initialized {
            status = app.odom_src.lock().unwrap_or_else(|poisoned| poisoned.into_inner()).on_start(&mut lifecycle_context);
            odom_src_started = status == flowrt::Status::Ok;
            introspection_state.record_lifecycle_state("odom_src", if odom_src_started { flowrt::LifecycleState::Running } else { flowrt::LifecycleState::Faulted });
        }
        if status == flowrt::Status::Ok && fusion_initialized {
            status = app.fusion.lock().unwrap_or_else(|poisoned| poisoned.into_inner()).on_start(&mut lifecycle_context);
            fusion_started = status == flowrt::Status::Ok;
            introspection_state.record_lifecycle_state("fusion", if fusion_started { flowrt::LifecycleState::Running } else { flowrt::LifecycleState::Faulted });
        }
        ...
        let _ = "imu_src_serial";
        scheduler.add_lane(flowrt::LaneId(3), flowrt::LaneKind::Serial);
        let _ = "odom_src_serial";
        scheduler.add_lane(flowrt::LaneId(4), flowrt::LaneKind::Serial);
        let _ = "sink_serial";
        ...
        scheduler.add_task(flowrt::TaskSpec { id: flowrt::TaskId(4), lane: flowrt::LaneId(4), priority: 0 });
        let mut bind_1_seen_revision_for_fusion_main: u64 = 0;
        let mut bind_2_seen_revision_for_fusion_main: u64 = 0;
                ...
                || !pending_task_order.is_empty())
        {
            ...
            loop {
                ...
                if app.bind_1.lock().unwrap_or_else(|poisoned| poisoned.into_inner()).revision() != bind_1_seen_revision_for_fusion_main || app.bind_2.lock().unwrap_or_else(|poisoned| poisoned.into_inner()).revision() != bind_2_seen_revision_for_fusion_main {
                    bind_1_seen_revision_for_fusion_main = app.bind_1.lock().unwrap_or_else(|poisoned| poisoned.into_inner()).revision();
                    bind_2_seen_revision_for_fusion_main = app.bind_2.lock().unwrap_or_else(|poisoned| poisoned.into_inner()).revision();
                ...
                if app.bind_0.lock().unwrap_or_else(|poisoned| poisoned.into_inner()).revision() != bind_0_seen_revision_for_sink_main {
                    ...
                    scheduler.wake(flowrt::TaskId(4));
                    woke_on_message = true;
                }
                ...
                for admission in ready_batch.admissions().iter().copied() {
                    ...
                    let submitted = match admission.task {
                        flowrt::TaskId(1) => {
                            let __flowrt_component_fusion = app.fusion.clone();
                            ...
                            let (__flowrt_input_odom_value, __flowrt_input_odom_stale, __flowrt_input_odom_revision) = {
                                let __flowrt_bind_2_snapshot_guard = app.bind_2.lock().unwrap_or_else(|poisoned| poisoned.into_inner());
                                let __flowrt_odom_snapshot_view = __flowrt_bind_2_snapshot_guard.view_at(tick_time_ms);
                                (__flowrt_odom_snapshot_view.as_ref().cloned(), __flowrt_odom_snapshot_view.stale(), __flowrt_bind_2_snapshot_guard.revision())
                            };
                            let __flowrt_sync_fusion_main = app.__flowrt_sync_fusion_main.clone();
                            ...
                            let task_name = "fusion.main";
                            let task_trigger = "on_synchronized";
                            ...
                            let task_outcome = {
                                ...
                                Self::step_task_fusion_main(__flowrt_component_fusion, __flowrt_input_imu_value, __flowrt_input_imu_stale, __flowrt_input_imu_revision, __flowrt_input_odom_value, __flowrt_input_odom_stale, __flowrt_input_odom_revision, __flowrt_sync_fusion_main, tick_time_ms as usize, &mut local_context, &introspection_state, &scheduler_events, &mut local_health_map)
                            };
                        ...
                        flowrt::TaskId(2) => {
                            let __flowrt_component_imu_src = app.imu_src.clone();
                            let introspection_state = introspection_state.clone();
                            let scheduler_events = scheduler_events.clone();
                            let task_health_from_worker = task_health_from_workers.clone();
                            worker_pool.submit_collect(admission.task, &task_completion_queue_for_task, move || {
                            let task_name = "imu_src.main";
                            ...
                            let task_outcome = {
                                ...
                                Self::step_task_imu_src_main(__flowrt_component_imu_src, tick_time_ms as usize, &mut local_context, &introspection_state, &scheduler_events, &mut local_health_map)
                            };
                            if let Some(health) = local_health_map.get_mut(task_name) {
                                health.inflight = false;
                                health.scheduled_time_ms = Some(admission.scheduled_time_ms);
                        ...
                        flowrt::TaskId(3) => {
                            let __flowrt_component_odom_src = app.odom_src.clone();
                            let introspection_state = introspection_state.clone();
                            let scheduler_events = scheduler_events.clone();
                            let task_health_from_worker = task_health_from_workers.clone();
                            worker_pool.submit_collect(admission.task, &task_completion_queue_for_task, move || {
                            let task_name = "odom_src.main";
                            let task_trigger = "periodic";
                            let mut local_context = flowrt::Context::with_timing(flowrt::TaskTiming {
                                step: tick_base as u64,
                                task_name: task_name.to_string(),
                            ...
                            let task_outcome = {
                                ...
                                Self::step_task_odom_src_main(__flowrt_component_odom_src, tick_time_ms as usize, &mut local_context, &introspection_state, &scheduler_events, &mut local_health_map)
                            };
                            if let Some(health) = local_health_map.get_mut(task_name) {
                                health.inflight = false;
                                health.scheduled_time_ms = Some(admission.scheduled_time_ms);
                        ...
                        flowrt::TaskId(4) => {
                            ...
                            let (__flowrt_input_estimate_value, __flowrt_input_estimate_stale, __flowrt_input_estimate_revision) = {
                                let __flowrt_bind_0_snapshot_guard = app.bind_0.lock().unwrap_or_else(|poisoned| poisoned.into_inner());
                                let __flowrt_estimate_snapshot_view = __flowrt_bind_0_snapshot_guard.view_at(tick_time_ms);
                                (__flowrt_estimate_snapshot_view.as_ref().cloned(), __flowrt_estimate_snapshot_view.stale(), __flowrt_bind_0_snapshot_guard.revision())
                            };
                            let introspection_state = introspection_state.clone();
                            let scheduler_events = scheduler_events.clone();
                            let task_health_from_worker = task_health_from_workers.clone();
                            ...
                            let mut local_health_map: std::collections::BTreeMap<String, flowrt::IntrospectionTaskHealth> = std::collections::BTreeMap::new();
                            let task_outcome = {
                                let _flowrt_lane_guard = flowrt::enter_lane(flowrt::LaneId(4));
                                Self::step_task_sink_main(__flowrt_component_sink, __flowrt_input_estimate_value, __flowrt_input_estimate_stale, __flowrt_input_estimate_revision, tick_time_ms as usize, &mut local_context, &introspection_state, &scheduler_events, &mut local_health_map)
                            };
                            if let Some(health) = local_health_map.get_mut(task_name) {
                                health.inflight = false;
                                health.scheduled_time_ms = Some(admission.scheduled_time_ms);
                    ...
                    match submitted {
                        Ok(()) => {
                            ...
                            match admission.task {
                                flowrt::TaskId(1) => {
                                    let health = health_map.entry("fusion.main".to_string()).or_default();
                                    health.name = "fusion.main".to_string();
                                    health.lane = "fusion_serial".to_string();
                                ...
                                flowrt::TaskId(2) => {
                                    let health = health_map.entry("imu_src.main".to_string()).or_default();
                                    health.name = "imu_src.main".to_string();
                                    health.lane = "imu_src_serial".to_string();
                                    health.inflight = true;
                                    health.scheduled_time_ms = Some(admission.scheduled_time_ms);
                                    health.observed_time_ms = Some(admission.observed_time_ms);
                                    health.lateness_ms = Some(admission.lateness_ms);
                                ...
                                flowrt::TaskId(3) => {
                                    let health = health_map.entry("odom_src.main".to_string()).or_default();
                                    health.name = "odom_src.main".to_string();
                                    health.lane = "odom_src_serial".to_string();
                                    health.inflight = true;
                                    health.scheduled_time_ms = Some(admission.scheduled_time_ms);
                                    health.observed_time_ms = Some(admission.observed_time_ms);
                                    health.lateness_ms = Some(admission.lateness_ms);
            ...
            if scheduler.lane_starvation_ticks(flowrt::LaneId(3)) > FAIRNESS_STARVATION_THRESHOLD {
                for health in health_map.values_mut() {
                    if health.lane == "odom_src_serial" {
                        health.fairness_violations += 1;
                    }
                }
            }
            if scheduler.lane_starvation_ticks(flowrt::LaneId(4)) > FAIRNESS_STARVATION_THRESHOLD {
                for health in health_map.values_mut() {
        ...
        if fusion_started {
            let stop_status = app.fusion.lock().unwrap_or_else(|poisoned| poisoned.into_inner()).on_stop(&mut lifecycle_context);
            if status == flowrt::Status::Ok && stop_status != flowrt::Status::Ok {
                status = flowrt::Status::Error;
            }
            introspection_state.record_lifecycle_state("fusion", if stop_status == flowrt::Status::Ok { flowrt::LifecycleState::Stopped } else { flowrt::LifecycleState::Faulted });
        }
        if odom_src_started {
            let stop_status = app.odom_src.lock().unwrap_or_else(|poisoned| poisoned.into_inner()).on_stop(&mut lifecycle_context);
            if status == flowrt::Status::Ok && stop_status != flowrt::Status::Ok {
                status = flowrt::Status::Error;
            }
            introspection_state.record_lifecycle_state("odom_src", if stop_status == flowrt::Status::Ok { flowrt::LifecycleState::Stopped } else { flowrt::LifecycleState::Faulted });
        }
        if imu_src_started {
            let stop_status = app.imu_src.lock().unwrap_or_else(|poisoned| poisoned.into_inner()).on_stop(&mut lifecycle_context);
            if status == flowrt::Status::Ok && stop_status != flowrt::Status::Ok {
                status = flowrt::Status::Error;
            }
            introspection_state.record_lifecycle_state("imu_src", if stop_status == flowrt::Status::Ok { flowrt::LifecycleState::Stopped } else { flowrt::LifecycleState::Faulted });
        }
        if sink_initialized {
        ...
        if fusion_initialized {
            let shutdown_status = app.fusion.lock().unwrap_or_else(|poisoned| poisoned.into_inner()).on_shutdown(&mut lifecycle_context);
            if status == flowrt::Status::Ok && shutdown_status != flowrt::Status::Ok {
                status = flowrt::Status::Error;
            }
            introspection_state.record_lifecycle_state("fusion", if shutdown_status == flowrt::Status::Ok { flowrt::LifecycleState::ShutDown } else { flowrt::LifecycleState::Faulted });
        }
        if odom_src_initialized {
            let shutdown_status = app.odom_src.lock().unwrap_or_else(|poisoned| poisoned.into_inner()).on_shutdown(&mut lifecycle_context);
            if status == flowrt::Status::Ok && shutdown_status != flowrt::Status::Ok {
                status = flowrt::Status::Error;
            }
            introspection_state.record_lifecycle_state("odom_src", if shutdown_status == flowrt::Status::Ok { flowrt::LifecycleState::ShutDown } else { flowrt::LifecycleState::Faulted });
        }
        if imu_src_initialized {
            let shutdown_status = app.imu_src.lock().unwrap_or_else(|poisoned| poisoned.into_inner()).on_shutdown(&mut lifecycle_context);
            if status == flowrt::Status::Ok && shutdown_status != flowrt::Status::Ok {
                status = flowrt::Status::Error;
            }
            introspection_state.record_lifecycle_state("imu_src", if shutdown_status == flowrt::Status::Ok { flowrt::LifecycleState::ShutDown } else { flowrt::LifecycleState::Faulted });
        }
        status
    }
                ...
                for admission in ready_batch.admissions().iter().copied() {
                    ...
                    let submitted = match admission.task {
                        flowrt::TaskId(1) => {
                            ...
                            let task_outcome = {
                                let _flowrt_lane_guard = flowrt::enter_lane(flowrt::LaneId(1));
                                Self::step_process_main_task_fusion_main(__flowrt_component_fusion, __flowrt_input_imu_value, __flowrt_input_imu_stale, __flowrt_input_imu_revision, __flowrt_input_odom_value, __flowrt_input_odom_stale, __flowrt_input_odom_revision, __flowrt_sync_fusion_main, tick_time_ms as usize, &mut local_context, &introspection_state, &scheduler_events, &mut local_health_map)
                            };
                            if let Some(health) = local_health_map.get_mut(task_name) {
                                health.inflight = false;
                                health.scheduled_time_ms = Some(admission.scheduled_time_ms);
                        ...
                        flowrt::TaskId(2) => {
                            ...
                            let task_outcome = {
                                let _flowrt_lane_guard = flowrt::enter_lane(flowrt::LaneId(2));
                                Self::step_process_main_task_imu_src_main(__flowrt_component_imu_src, tick_time_ms as usize, &mut local_context, &introspection_state, &scheduler_events, &mut local_health_map)
                            };
                            if let Some(health) = local_health_map.get_mut(task_name) {
                                health.inflight = false;
                                health.scheduled_time_ms = Some(admission.scheduled_time_ms);
                        ...
                        flowrt::TaskId(3) => {
                            ...
                            let task_outcome = {
                                let _flowrt_lane_guard = flowrt::enter_lane(flowrt::LaneId(3));
                                Self::step_process_main_task_odom_src_main(__flowrt_component_odom_src, tick_time_ms as usize, &mut local_context, &introspection_state, &scheduler_events, &mut local_health_map)
                            };
                            if let Some(health) = local_health_map.get_mut(task_name) {
                                health.inflight = false;
                                health.scheduled_time_ms = Some(admission.scheduled_time_ms);
                        ...
                        flowrt::TaskId(4) => {
                            ...
                            let task_outcome = {
                                let _flowrt_lane_guard = flowrt::enter_lane(flowrt::LaneId(4));
                                Self::step_process_main_task_sink_main(__flowrt_component_sink, __flowrt_input_estimate_value, __flowrt_input_estimate_stale, __flowrt_input_estimate_revision, tick_time_ms as usize, &mut local_context, &introspection_state, &scheduler_events, &mut local_health_map)
                            };
                            if let Some(health) = local_health_map.get_mut(task_name) {
                                health.inflight = false;
                                health.scheduled_time_ms = Some(admission.scheduled_time_ms);
