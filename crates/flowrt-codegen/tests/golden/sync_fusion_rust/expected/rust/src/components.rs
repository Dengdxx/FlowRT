...
pub trait Fusion: Send {
    ...
    ///
    /// - `imu`: latest snapshot 输入视图。
    /// - `odom`: latest snapshot 输入视图。
    /// - `estimate`: 输出端口写入句柄。
    /// 返回本次回调的 FlowRT 执行状态。
    fn on_tick(
        &mut self,
        ...
        estimate: &mut flowrt::Output<Estimate>,
    ) -> flowrt::Status;
}

/// `imu_src` 组件的 Rust 用户实现 trait。
///
/// 用户代码实现该 trait 并交给 FlowRT 管理的 runtime shell。接口只暴露组件算法所需的生命周期、输入视图和输出句柄，不暴露具体 backend API。
pub trait ImuSrc: Send {
    /// 组件初始化钩子。
    ///
    /// `context` 是 runtime 上下文；v0.1 暂不暴露资源句柄，后续可承载 clock、logger 和参数快照。
    /// 返回本次生命周期步骤的 FlowRT 执行状态。
    ...
    fn on_tick(
        &mut self,
        imu: &mut flowrt::Output<Imu>,
    ) -> flowrt::Status;
}

/// `odom_src` 组件的 Rust 用户实现 trait。
///
/// 用户代码实现该 trait 并交给 FlowRT 管理的 runtime shell。接口只暴露组件算法所需的生命周期、输入视图和输出句柄，不暴露具体 backend API。
pub trait OdomSrc: Send {
    /// 组件初始化钩子。
    ///
    /// `context` 是 runtime 上下文；v0.1 暂不暴露资源句柄，后续可承载 clock、logger 和参数快照。
    /// 返回本次生命周期步骤的 FlowRT 执行状态。
