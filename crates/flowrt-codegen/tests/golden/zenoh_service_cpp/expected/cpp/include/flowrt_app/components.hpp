...
/**
 * @brief `plan_client.plan` service client（zenoh backend）。
 *
 * `slot_` 在所属进程启动时由 runtime shell 经 `bind()` 填充 `ZenohServiceClient`；
...
public:
    ...
    void bind(flowrt::zenoh::ZenohServiceClient<PlanRequest, PlanResponse> client) {
        if (slot_) {
...
private:
    struct Slot {
        std::optional<flowrt::zenoh::ZenohServiceClient<PlanRequest, PlanResponse>> client;
    };
