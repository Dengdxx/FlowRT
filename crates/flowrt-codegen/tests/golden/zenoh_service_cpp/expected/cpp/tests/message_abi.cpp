...
void test_plan_request_wire_codec_omits_native_padding() {
    const auto value = sample_plan_request();
    std::array<std::uint8_t, flowrt_app::PlanRequest::wire_size()> wire{};
    value.encode_wire(wire);
    const std::array<std::uint8_t, flowrt_app::PlanRequest::wire_size()> expected_wire{2, 0, 0, 0};
    assert(wire == expected_wire);
    const auto decoded = flowrt_app::PlanRequest::decode_wire(wire);
...
void test_plan_response_wire_codec_omits_native_padding() {
    const auto value = sample_plan_response();
    std::array<std::uint8_t, flowrt_app::PlanResponse::wire_size()> wire{};
    value.encode_wire(wire);
    const std::array<std::uint8_t, flowrt_app::PlanResponse::wire_size()> expected_wire{1};
    assert(wire == expected_wire);
    const auto decoded = flowrt_app::PlanResponse::decode_wire(wire);
    assert(bytes_of(decoded) == bytes_of(value));
}
...
int main() {
    ...
    test_plan_request_wire_codec_omits_native_padding();
    test_plan_response_message_abi();
    test_plan_response_wire_codec_omits_native_padding();
    return 0;
}
