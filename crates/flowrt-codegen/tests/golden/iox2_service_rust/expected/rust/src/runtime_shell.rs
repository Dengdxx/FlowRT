...
const PACKAGE_NAME: &str = "iox2_service_rust";
...
pub struct App {
    ...
    plan_client: std::sync::Arc<std::sync::Mutex<Box<dyn Planner + Send>>>,
    plan_svc: std::sync::Arc<std::sync::Mutex<Box<dyn PlanService + Send>>>,
    service_client_plan_client_plan: ServiceClient_planner_plan,
    service_server_plan_svc_plan: std::sync::Arc<std::sync::OnceLock<std::sync::Mutex<flowrt::iox2::Iox2ServiceServer<PlanRequest, PlanResponse>>>>,
}

impl App {
    pub fn new(
        plan_client: Box<dyn Planner + Send>,
        plan_svc: Box<dyn PlanService + Send>,
    ) -> Self {
        let startup_status = flowrt::Status::Ok;
        let plan_client = std::sync::Arc::new(std::sync::Mutex::new(plan_client));
        let plan_svc = std::sync::Arc::new(std::sync::Mutex::new(plan_svc));
        Self {
            plan_client: plan_client.clone(),
            plan_svc: plan_svc.clone(),
            service_client_plan_client_plan: ServiceClient_planner_plan { inner: std::sync::Arc::new(std::sync::OnceLock::new()) },
            service_server_plan_svc_plan: std::sync::Arc::new(std::sync::OnceLock::new()),
            startup_status,
        }
    }
    #[allow(dead_code)]
    fn step(
        ...
        {
            {
                let __h = health_map.entry("plan_client.main".to_string()).or_default();
                __h.name = "plan_client.main".to_string();
                __h.lane = "plan_client_serial".to_string();
            }
            match self.plan_client.lock().unwrap_or_else(|poisoned| poisoned.into_inner()).on_tick(&self.service_client_plan_client_plan) {
                flowrt::Status::Ok => {}
        ...
        {
            {
                let __h = health_map.entry("plan_svc.main".to_string()).or_default();
                __h.name = "plan_svc.main".to_string();
                __h.lane = "plan_svc_serial".to_string();
            }
            match self.plan_svc.lock().unwrap_or_else(|poisoned| poisoned.into_inner()).on_tick() {
                flowrt::Status::Ok => {}
                flowrt::Status::Retry => return flowrt::Status::Retry,
                flowrt::Status::Error => return flowrt::Status::Error,
            }
    ...
    #[allow(dead_code)]
    fn step_task_plan_client_main(
        __flowrt_component_plan_client: std::sync::Arc<std::sync::Mutex<Box<dyn Planner + Send>>>,
        __flowrt_service_client_plan_client_plan: ServiceClient_planner_plan,
        tick: usize,
        _tick_context: &mut flowrt::Context,
        introspection_state: &flowrt::IntrospectionState,
        scheduler_events: &flowrt::ScheduleWaiter,
        ...
        let mut __flowrt_output_commits: Vec<FlowrtOutputCommit> = Vec::new();
        {
            ...
            match __flowrt_component_plan_client.lock().unwrap_or_else(|poisoned| poisoned.into_inner()).on_tick(&__flowrt_service_client_plan_client_plan) {
                flowrt::Status::Ok => {}
    ...
    #[allow(dead_code)]
    fn step_task_plan_svc_main(
        __flowrt_component_plan_svc: std::sync::Arc<std::sync::Mutex<Box<dyn PlanService + Send>>>,
        tick: usize,
        _tick_context: &mut flowrt::Context,
        introspection_state: &flowrt::IntrospectionState,
        scheduler_events: &flowrt::ScheduleWaiter,
        ...
        let mut __flowrt_output_commits: Vec<FlowrtOutputCommit> = Vec::new();
        {
            ...
            match __flowrt_component_plan_svc.lock().unwrap_or_else(|poisoned| poisoned.into_inner()).on_tick() {
                flowrt::Status::Ok => {}
                flowrt::Status::Retry => return flowrt::TaskRunOutcome::retry(Vec::new()),
                flowrt::Status::Error => return flowrt::TaskRunOutcome::error(Vec::new()),
            }
    ...
    #[allow(dead_code)]
    fn step_process_client_proc_task_plan_client_main(
        __flowrt_component_plan_client: std::sync::Arc<std::sync::Mutex<Box<dyn Planner + Send>>>,
        __flowrt_service_client_plan_client_plan: ServiceClient_planner_plan,
        tick: usize,
        _tick_context: &mut flowrt::Context,
    ...
    #[allow(dead_code)]
    fn step_process_server_proc_task_plan_svc_main(
        __flowrt_component_plan_svc: std::sync::Arc<std::sync::Mutex<Box<dyn PlanService + Send>>>,
        tick: usize,
        _tick_context: &mut flowrt::Context,
        introspection_state: &flowrt::IntrospectionState,
...
let handled = match server.lock().unwrap_or_else(|poisoned| poisoned.into_inner()).poll_requests(|request| self.plan_svc.lock().unwrap_or_else(|poisoned| poisoned.into_inner()).on_plan_request(&request)) {
Ok(handled) => handled,
...
flowrt::Status::Ok
    ...
    pub fn run(self, backend: &dyn flowrt::Backend, run_ticks: Option<usize>) -> flowrt::Status {
        ...
        if status == flowrt::Status::Ok {
        let _ = app.service_client_plan_client_plan.inner.set(match flowrt::iox2::Iox2ServiceClient::open("FlowRT/service/plan_client_plan") {
            Ok(client) => client,
            Err(error) => {
                eprintln!("FlowRT: failed to open iox2 service client {}: {error}", "FlowRT/service/plan_client_plan");
                status = flowrt::Status::Error;
                flowrt::iox2::Iox2ServiceClient::unavailable("FlowRT/service/plan_client_plan", error.to_string())
            }
        });
        match flowrt::iox2::Iox2ServiceServer::open("FlowRT/service/plan_client_plan", 64usize) {
            Ok(mut server) => {
            ...
            Err(error) => {
                eprintln!("FlowRT: failed to open iox2 service server {}: {error}", "FlowRT/service/plan_client_plan");
                status = flowrt::Status::Error;
            }
        }
        }
        ...
        // Service task 3: plan_client.plan
scheduler.add_task(flowrt::TaskSpec { id: flowrt::TaskId(3), lane: flowrt::LaneId(2), priority: 0 });
    ...
    woke_on_message = true;
                ...
                for admission in ready_batch.admissions().iter().copied() {
                    ...
                    let submitted = match admission.task {
                        flowrt::TaskId(1) => {
                            let __flowrt_component_plan_client = app.plan_client.clone();
                            ...
                            let task_outcome = {
                                ...
                                Self::step_task_plan_client_main(__flowrt_component_plan_client, __flowrt_service_client_plan_client_plan, tick_time_ms as usize, &mut local_context, &introspection_state, &scheduler_events, &mut local_health_map)
                            };
                        ...
                        flowrt::TaskId(2) => {
                            ...
                            let task_outcome = {
                                ...
                                Self::step_task_plan_svc_main(__flowrt_component_plan_svc, tick_time_ms as usize, &mut local_context, &introspection_state, &scheduler_events, &mut local_health_map)
                            };
                            if let Some(health) = local_health_map.get_mut(task_name) {
                                health.inflight = false;
                                health.scheduled_time_ms = Some(admission.scheduled_time_ms);
...
late_drop_count: 0,
                    ...
                    match submitted {
                        Ok(()) => {
                            ...
                            match admission.task {
                                flowrt::TaskId(1) => {
                                    let health = health_map.entry("plan_client.main".to_string()).or_default();
                                    health.name = "plan_client.main".to_string();
                                    health.lane = "plan_client_serial".to_string();
                                ...
                                flowrt::TaskId(2) => {
                                    let health = health_map.entry("plan_svc.main".to_string()).or_default();
                                    health.name = "plan_svc.main".to_string();
                                    health.lane = "plan_svc_serial".to_string();
                                    health.inflight = true;
                                    health.scheduled_time_ms = Some(admission.scheduled_time_ms);
                                    health.observed_time_ms = Some(admission.observed_time_ms);
                                    health.lateness_ms = Some(admission.lateness_ms);
        ...
        if plan_svc_started {
            let stop_status = app.plan_svc.lock().unwrap_or_else(|poisoned| poisoned.into_inner()).on_stop(&mut lifecycle_context);
    ...
    fn run_process_client_proc(self, backend: &dyn flowrt::Backend, run_ticks: Option<usize>) -> flowrt::Status {
        ...
        scheduler.add_periodic(flowrt::PeriodicSpec { task: flowrt::TaskId(1), period_ms: 100 });
        scheduler.wake(flowrt::TaskId(1));
        // Service task 2: plan_client.plan
scheduler.add_task(flowrt::TaskSpec { id: flowrt::TaskId(2), lane: flowrt::LaneId(2), priority: 0 });
        let scheduler_base_period_ms: u64 = 100;
        let mut tick_base: usize = 0;
        let mut scheduler_now_ms: u64 = 0;
                ...
                for admission in ready_batch.admissions().iter().copied() {
                    ...
                    let submitted = match admission.task {
                        flowrt::TaskId(1) => {
                            ...
                            let task_outcome = {
                                let _flowrt_lane_guard = flowrt::enter_lane(flowrt::LaneId(1));
                                Self::step_process_client_proc_task_plan_client_main(__flowrt_component_plan_client, __flowrt_service_client_plan_client_plan, tick_time_ms as usize, &mut local_context, &introspection_state, &scheduler_events, &mut local_health_map)
                            };
                            if let Some(health) = local_health_map.get_mut(task_name) {
                                health.inflight = false;
                                health.scheduled_time_ms = Some(admission.scheduled_time_ms);
...
late_drop_count: 0,
    ...
    fn run_process_server_proc(self, backend: &dyn flowrt::Backend, run_ticks: Option<usize>) -> flowrt::Status {
        ...
        scheduler.wake(flowrt::TaskId(1));
        // Service task 2: plan_client.plan
scheduler.add_task(flowrt::TaskSpec { id: flowrt::TaskId(2), lane: flowrt::LaneId(1), priority: 0 });
                ...
                for admission in ready_batch.admissions().iter().copied() {
                    ...
                    let submitted = match admission.task {
                        flowrt::TaskId(1) => {
                            ...
                            let task_outcome = {
                                let _flowrt_lane_guard = flowrt::enter_lane(flowrt::LaneId(1));
                                Self::step_process_server_proc_task_plan_svc_main(__flowrt_component_plan_svc, tick_time_ms as usize, &mut local_context, &introspection_state, &scheduler_events, &mut local_health_map)
                            };
                            if let Some(health) = local_health_map.get_mut(task_name) {
                                health.inflight = false;
                                health.scheduled_time_ms = Some(admission.scheduled_time_ms);
