...
const PACKAGE_NAME: &str = "service_iox2_dynamic_fallback";
...
// ── Service step functions ─────────────────────────────────────────
    ...
    pub fn run(self, backend: &dyn flowrt::Backend, run_ticks: Option<usize>) -> flowrt::Status {
        ...
        let zenoh_service_session = match flowrt::zenoh::open_session_from_environment() {
            ...
            Err(error) => {
                eprintln!("FlowRT: failed to open zenoh service session: {error}");
                status = flowrt::Status::Error;
                None
            }
        };
        if let Some(session) = zenoh_service_session.as_ref() {
            let _ = app.service_client_plan_client_plan.inner.set(flowrt::zenoh::ZenohServiceClient::open("plan_client.plan", session.clone()));
        }
        let _zenoh_service_server_plan_svc_plan = if let Some(session) = zenoh_service_session.as_ref() {
            let handler_component = app.plan_svc.clone();
            match flowrt::zenoh::ZenohServiceServer::open(
                "plan_client.plan",
                session.clone(),
                move |request| handler_component.as_ref().as_ref().on_plan_request(&request),
            ) {
                Ok(server) => {
                ...
                Err(error) => {
                    eprintln!("FlowRT: failed to open zenoh service server: {error}");
                    status = flowrt::Status::Error;
                    None
                }
            }
