...
const PACKAGE_NAME: &str = "iox2_operation_rust";
...
operation_start_command_rx_0: std::sync::Mutex::new(operation_start_command_rx_0),
    ...
    pub fn run(self, backend: &dyn flowrt::Backend, run_ticks: Option<usize>) -> flowrt::Status {
        ...
        if status == flowrt::Status::Ok {
        let _ = app.operation_client_controller_plan.start_client.set(match flowrt::iox2::Iox2ServiceClient::open("FlowRT/service/__flowrt_operation_controller_plan_start") {
            Ok(client) => client,
            Err(error) => {
                ...
                flowrt::iox2::Iox2ServiceClient::unavailable("FlowRT/service/__flowrt_operation_controller_plan_start", error.to_string())
            }
        });
        ...
        match flowrt::iox2::Iox2ServiceServer::<flowrt::OperationStartRequest<PlanGoal>, flowrt::OperationStartAck>::open("FlowRT/service/__flowrt_operation_controller_plan_start", 1usize) {
            Ok(mut server) => {
