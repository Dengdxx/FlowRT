...
const PACKAGE_NAME: &str = "instance_degrade_demo";
...
pub struct App {
    ...
    monitor: std::sync::Arc<std::sync::Mutex<Box<dyn Producer + Send>>>,
...
impl App {
    pub fn new(
        ...
        Self {
            ...
            bind_0: std::sync::Arc::new(std::sync::Mutex::new(flowrt::LatestChannel::with_stale_config(flowrt::StaleConfig::new(Some(50), flowrt::StalePolicy::HoldLast)))),
    ...
    fn step(
        ...
        let _ = tick_time_ms;
        {
            ...
            match self.monitor.lock().unwrap_or_else(|poisoned| poisoned.into_inner()).on_tick(&mut sample) {
                flowrt::Status::Ok => {}
            ...
            if let Some(value) = sample.as_ref().cloned() {
                ...
                introspection_state.record_route_publish("monitor.sample_to_consumer.sample", Some(tick_time_ms));
                scheduler_events.notify_data();
                if let Some(introspection_probe_bind_0_probe) = self.introspection_probe_bind_0.get() {
                    record_introspection_publish_copy(&introspection_state, "monitor.sample_to_consumer.sample", "Sample", introspection_probe_bind_0_probe, &value, tick_time_ms);
                }
            }
        }
        {
            ...
            record_introspection_input_read(&introspection_state, "consumer.main.sample", "consumer.main", "sample", "monitor.sample_to_consumer.sample", "Sample", &sample, __flowrt_sample_revision, tick_time_ms);
            if sample.stale() {
    ...
    fn step_task_monitor_main(
        __flowrt_component_monitor: std::sync::Arc<std::sync::Mutex<Box<dyn Producer + Send>>>,
        tick: usize,
        _tick_context: &mut flowrt::Context,
        introspection_state: &flowrt::IntrospectionState,
        scheduler_events: &flowrt::ScheduleWaiter,
        ...
        let mut __flowrt_output_commits: Vec<FlowrtOutputCommit> = Vec::new();
        {
            ...
            let mut sample = flowrt::Output::<Sample>::new();
            match __flowrt_component_monitor.lock().unwrap_or_else(|poisoned| poisoned.into_inner()).on_tick(&mut sample) {
                flowrt::Status::Ok => {}
                flowrt::Status::Retry => return flowrt::TaskRunOutcome::retry(Vec::new()),
                flowrt::Status::Error => return flowrt::TaskRunOutcome::error(Vec::new()),
            }
    ...
    pub fn run(self, backend: &dyn flowrt::Backend, run_ticks: Option<usize>) -> flowrt::Status {
        ...
        let introspection_probe_bind_0 = register_introspection_channel(&introspection_state, "monitor.sample_to_consumer.sample", "Sample", Some(4));
        ...
        introspection_state.register_route(flowrt::IntrospectionRouteStatus {
            name: "monitor.sample_to_consumer.sample".to_string(),
            from: "monitor.sample".to_string(),
        ...
        introspection_state.record_input_status(flowrt::IntrospectionInputStatus {
            ...
            channel: "monitor.sample_to_consumer.sample".to_string(),
            message_type: "Sample".to_string(),
            ..Default::default()
        });
                    ...
                    let submitted = match admission.task {
                        ...
                        flowrt::TaskId(2) => {
                            ...
                            let task_outcome = {
                                ...
                                Self::step_task_monitor_main(__flowrt_component_monitor, tick_time_ms as usize, &mut local_context, &introspection_state, &scheduler_events, &mut local_health_map)
                            };
                            if let Some(health) = local_health_map.get_mut(task_name) {
                                health.inflight = false;
                                health.scheduled_time_ms = Some(admission.scheduled_time_ms);
                ...
                for admission in ready_batch.admissions().iter().copied() {
                    ...
                    let submitted = match admission.task {
                        ...
                        flowrt::TaskId(2) => {
                            ...
                            let task_outcome = {
                                let _flowrt_lane_guard = flowrt::enter_lane(flowrt::LaneId(2));
                                Self::step_process_main_task_monitor_main(__flowrt_component_monitor, tick_time_ms as usize, &mut local_context, &introspection_state, &scheduler_events, &mut local_health_map)
                            };
                            if let Some(health) = local_health_map.get_mut(task_name) {
                                health.inflight = false;
                                health.scheduled_time_ms = Some(admission.scheduled_time_ms);
