...
const PACKAGE_NAME: &str = "fault_injection_degrade_demo";
...
pub struct App {
    ...
    boundary_input_feed: flowrt::BoundaryInput<Sample>,
    boundary_output_emit: flowrt::BoundaryOutput<Sample>,
}
...
impl App {
    pub fn new(
        ...
        Self {
            ...
            boundary_input_feed: flowrt::BoundaryInput::new(),
            boundary_output_emit: flowrt::BoundaryOutput::new(),
            startup_status,
        }
    }
    #[allow(dead_code)]
    fn step(
        ...
        let _ = tick_time_ms;
        {
            let sample_read = self.boundary_input_feed.read_at(tick_time_ms);
            ...
            if sample.present() {
                ...
                match self.monitor.lock().unwrap_or_else(|poisoned| poisoned.into_inner()).on_tick(sample, &mut echo) {
                    flowrt::Status::Ok => {}
                ...
                if let Some(value) = echo.as_ref().cloned() {
                    self.boundary_output_emit.publish_at(&value, tick_time_ms);
                }
            }
        }
        flowrt::Status::Ok
    ...
    fn step_task_monitor_main(
        ...
        let mut __flowrt_output_commits: Vec<FlowrtOutputCommit> = Vec::new();
        {
            ...
            if sample.present() {
                let mut echo = flowrt::Output::<Sample>::new();
                match __flowrt_component_monitor.lock().unwrap_or_else(|poisoned| poisoned.into_inner()).on_tick(sample, &mut echo) {
                    flowrt::Status::Ok => {}
                ...
                if let Some(value) = echo.as_ref().cloned() {
                    ...
                    __flowrt_output_commits.push(Box::new(move |app, _introspection_state, _scheduler_events, _health_map| {
                        app.boundary_output_emit.publish_at(&value, tick_time_ms);
                        flowrt::Status::Ok
                    }));
                }
            }
    ...
    pub fn run(self, backend: &dyn flowrt::Backend, run_ticks: Option<usize>) -> flowrt::Status {
        ...
        app.boundary_input_feed.set_schedule_waiter(scheduler_events.clone());
        introspection_state.set_self_description_json(selfdesc::self_description_json());
        introspection_state.register_boundary_input::<Sample>("feed", "Sample", app.boundary_input_feed.clone());
        introspection_state.register_channel("emit", "Sample");
        let _boundary_output_emit_probe = app.boundary_output_emit.register_sink({
            ...
            move |value, published_at_ms| {
                ...
                if <Sample as flowrt::FrameCodec>::encode_frame(value, &mut payload).is_ok() {
                    introspection_state.record_channel_publish_bytes("emit", "Sample", payload, published_at_ms);
                }
            }
        });
        ...
        let replay_boundary_inputs: std::collections::BTreeSet<String> = ["feed"].into_iter().map(::std::string::String::from).collect();
        ...
        let clock_source = "simulated_replay";
        let task_clock_source = flowrt::ClockSource::Replay;
        ...
        let mut monitor_degraded = false;
        let mut __inject_count_1: u64 = 0;
                ...
                || !pending_task_order.is_empty())
        {
            ...
            loop {
                ...
                if app.boundary_input_feed.revision() != boundary_input_feed_seen_revision_for_monitor_main {
                    boundary_input_feed_seen_revision_for_monitor_main = app.boundary_input_feed.revision();
                    ...
                    let submitted = match admission.task {
                        flowrt::TaskId(1) => {
                            ...
                            let (__flowrt_input_sample_value, __flowrt_input_sample_stale, __flowrt_input_sample_revision) = {
                                let __flowrt_sample_boundary_read = app.boundary_input_feed.read_at(tick_time_ms);
                            ...
                            __inject_count_1 += 1;
                            let __inject_fault_1 = [1u64, 2u64].contains(&__inject_count_1);
                            ...
                            let task_outcome = if __inject_fault_1 {
                                flowrt::TaskRunOutcome::error(Vec::<FlowrtOutputCommit>::new())
                            } else {
                                let _flowrt_lane_guard = flowrt::enter_lane(flowrt::LaneId(1));
                                Self::step_task_monitor_main(__flowrt_component_monitor, __flowrt_input_sample_value, __flowrt_input_sample_stale, __flowrt_input_sample_revision, tick_time_ms as usize, &mut local_context, &introspection_state, &scheduler_events, &mut local_health_map)
                            };
            ...
            if status == flowrt::Status::Ok {
                tick_base += 1;
                if let Some(replay_driver) = replay_time_driver.as_mut() {
                    ...
                    let submitted = match admission.task {
                        flowrt::TaskId(1) => {
                            ...
                            let task_outcome = if __inject_fault_1 {
                                ...
                                let _flowrt_lane_guard = flowrt::enter_lane(flowrt::LaneId(1));
                                Self::step_process_main_task_monitor_main(__flowrt_component_monitor, __flowrt_input_sample_value, __flowrt_input_sample_stale, __flowrt_input_sample_revision, tick_time_ms as usize, &mut local_context, &introspection_state, &scheduler_events, &mut local_health_map)
                            };
                            if let Some(health) = local_health_map.get_mut(task_name) {
                                health.inflight = false;
                                health.scheduled_time_ms = Some(admission.scheduled_time_ms);
