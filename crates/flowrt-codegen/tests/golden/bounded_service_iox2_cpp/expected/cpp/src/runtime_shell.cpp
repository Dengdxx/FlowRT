...
flowrt::Status App::step_service_plan_svc_plan(std::size_t tick, flowrt::Context& tick_context, flowrt::IntrospectionState& introspection_state, flowrt::ScheduleWaiter& scheduler_events, std::map<std::string, flowrt::IntrospectionTaskHealth>& health_map) {
    (void)tick;
    (void)tick_context;
    (void)scheduler_events;
    (void)health_map;
    if (service_server_plan_svc_plan_.has_value()) {
        const auto handled = (*service_server_plan_svc_plan_).poll_requests([this](const PlanRequest& request) -> flowrt::ServiceResult<PlanResponse> {
            if (!this->plan_svc_) {
...
flowrt::Status App::run(const flowrt::Backend& backend, std::optional<std::size_t> run_ticks) {
    ...
    auto introspection_server = flowrt::spawn_status_server(
        flowrt::IntrospectionIdentity{
            ...
            .package = "bounded_service_iox2_cpp",
            .process = "main",
            .runtime = "cpp",
        },
        introspection_state);
    ...
    if (status == flowrt::Status::Ok) {
        this->service_client_plan_client_plan_.bind(flowrt::iox2::Iox2FrameServiceClient<PlanRequest, PlanResponse, 44, 21>::open_shared("FlowRT/service/plan_client_plan"));
        this->service_server_plan_svc_plan_ = flowrt::iox2::Iox2FrameServiceServer<PlanRequest, PlanResponse, 44, 21>::open("FlowRT/service/plan_client_plan", 64U);
    ...
    scheduler.add_task(flowrt::TaskSpec{.id = flowrt::TaskId{3}, .lane = flowrt::LaneId{flowrt::fnv1a64("plan_svc_serial")}, .priority = 0});
...
auto flowrt_lane_guard = flowrt::enter_lane(flowrt::LaneId{flowrt::fnv1a64("plan_svc_serial")});
(void)flowrt_lane_guard;
auto task_status = step_service_plan_svc_plan(static_cast<std::size_t>(tick_time_ms), local_context, introspection_state, scheduler_events, local_health_map);
merge_local_health(std::move(local_health_map));
