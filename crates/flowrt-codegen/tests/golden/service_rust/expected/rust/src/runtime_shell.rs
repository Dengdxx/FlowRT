...
const PACKAGE_NAME: &str = "service_demo";
...
pub struct App {
    ...
    service_server_plan_svc_plan: flowrt::InprocServiceServer<PlanRequest, PlanResponse>,
}
...
impl App {
    pub fn new(
        ...
        let service_registry = flowrt::ServiceRegistry::new();
        let plan_svc_handler = plan_svc.clone();
let service_handler_0 = move |req: PlanRequest| -> flowrt::ServiceResult<PlanResponse> {
plan_svc_handler.lock().unwrap_or_else(|poisoned| poisoned.into_inner()).on_plan_request(&req)
};
let service_reg_0 = service_registry.register_result_with_config::<PlanRequest, PlanResponse, _>(
"plan_client.plan",
flowrt::LaneId(3),
flowrt::InprocServiceConfig {
queue_depth: 16,
max_in_flight: 64,
overflow: flowrt::ServiceOverflowPolicy::Busy,
..Default::default()
},
service_handler_0,
);
        Self {
            ...
            service_client_plan_client_plan: ServiceClient_planner_plan { inner: service_reg_0.0 },
            service_server_plan_svc_plan: service_reg_0.1,
            startup_status,
        }
    }
    #[allow(dead_code)]
    ...
    #[allow(dead_code)]
    fn step_process_main_task_plan_client_main(
        __flowrt_component_plan_client: std::sync::Arc<std::sync::Mutex<Box<dyn Planner + Send>>>,
        __flowrt_service_client_plan_client_plan: ServiceClient_planner_plan,
        tick: usize,
        _tick_context: &mut flowrt::Context,
    ...
    #[allow(dead_code)]
    fn step_process_main_task_plan_svc_main(
        __flowrt_component_plan_svc: std::sync::Arc<std::sync::Mutex<Box<dyn PlanService + Send>>>,
        tick: usize,
        _tick_context: &mut flowrt::Context,
        introspection_state: &flowrt::IntrospectionState,
                ...
                || !pending_task_order.is_empty())
        {
            ...
            loop {
                ...
                let mut woke_on_message = false;
                    if app.service_server_plan_svc_plan.pending_count() > 0 {
                ...
                for admission in ready_batch.admissions().iter().copied() {
                    ...
                    let submitted = match admission.task {
                        ...
                        flowrt::TaskId(3) => {
                            ...
                            let task_outcome = {
                                let _flowrt_lane_guard = flowrt::enter_lane(flowrt::LaneId(2));
                                __flowrt_service_server_plan_svc_plan.process_pending_requests();
                                {
                                    let service_stats = __flowrt_service_server_plan_svc_plan.stats();
                                    introspection_state.record_service_health(flowrt::IntrospectionServiceStatus {
                                        name: "plan_client.plan".to_string(),
                                        ready: true,
                                        in_flight: __flowrt_service_server_plan_svc_plan.in_flight_count() as u64,
                                        queued: __flowrt_service_server_plan_svc_plan.pending_count() as u64,
                ...
                for admission in ready_batch.admissions().iter().copied() {
                    ...
                    let submitted = match admission.task {
                        flowrt::TaskId(1) => {
                            ...
                            let task_outcome = {
                                let _flowrt_lane_guard = flowrt::enter_lane(flowrt::LaneId(1));
                                Self::step_process_main_task_plan_client_main(__flowrt_component_plan_client, __flowrt_service_client_plan_client_plan, tick_time_ms as usize, &mut local_context, &introspection_state, &scheduler_events, &mut local_health_map)
                            };
                            if let Some(health) = local_health_map.get_mut(task_name) {
                                health.inflight = false;
                                health.scheduled_time_ms = Some(admission.scheduled_time_ms);
                        ...
                        flowrt::TaskId(2) => {
                            ...
                            let task_outcome = {
                                let _flowrt_lane_guard = flowrt::enter_lane(flowrt::LaneId(2));
                                Self::step_process_main_task_plan_svc_main(__flowrt_component_plan_svc, tick_time_ms as usize, &mut local_context, &introspection_state, &scheduler_events, &mut local_health_map)
                            };
                            if let Some(health) = local_health_map.get_mut(task_name) {
                                health.inflight = false;
                                health.scheduled_time_ms = Some(admission.scheduled_time_ms);
