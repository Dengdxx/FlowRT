...
flowrt::Status App::run(const flowrt::Backend& backend, std::optional<std::size_t> run_ticks) {
    ...
    if (status == flowrt::Status::Ok) {
        ...
        if (!this->operation_start_server_navigator_plan_ || !this->operation_start_server_navigator_plan_->ready()) {
        ...
        if (!this->operation_cancel_server_navigator_plan_ || !this->operation_cancel_server_navigator_plan_->ready()) {
            status = flowrt::Status::Error;
        }
