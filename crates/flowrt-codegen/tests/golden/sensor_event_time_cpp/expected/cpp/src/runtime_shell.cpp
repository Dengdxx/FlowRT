...
App::App(
    std::unique_ptr<ConsumerInterface> consumer
)
...
flowrt::Status App::step(std::size_t tick, flowrt::Context& tick_context, flowrt::IntrospectionState& introspection_state, flowrt::ScheduleWaiter& scheduler_events, std::map<std::string, flowrt::IntrospectionTaskHealth>& health_map) {
    ...
    (void)health_map;
    {
        ...
        const auto consumer_sample = consumer_sample_read.view();
        health_map["consumer.main"].name = "consumer.main";
        health_map["consumer.main"].lane = "consumer_serial";
        if (consumer_sample.present()) {
            flowrt::Output<ImuSample> consumer_echo;
            if (consumer_) {
                switch (consumer_->on_tick(consumer_sample, consumer_echo)) {
            ...
            if (const auto* value = consumer_echo.as_ref()) {
                boundary_output_echo_out_.publish_at(*value, tick_time_ms);
            }
        }
    }
    return flowrt::Status::Ok;
        ...
        if (consumer_sample.present()) {
            ...
            if (const auto* value = consumer_echo.as_ref()) {
                ...
                flowrt_output_commits.emplace_back([flowrt_payload_0 = std::move(flowrt_payload_0), tick_time_ms](App& app, flowrt::IntrospectionState& /*introspection_state*/, flowrt::ScheduleWaiter& /*scheduler_events*/, std::map<std::string, flowrt::IntrospectionTaskHealth>& /*health_map*/) mutable {
                    ...
                    app.boundary_output_echo_out_.publish_at(*value, tick_time_ms);
                    return flowrt::Status::Ok;
                });
            }
        }
...
flowrt::Status App::run(const flowrt::Backend& backend, std::optional<std::size_t> run_ticks) {
    ...
    boundary_input_sample_in_.set_schedule_waiter(scheduler_events);
    ...
    introspection_state.register_channel("echo_out", "ImuSample");
    auto boundary_output_echo_out_probe = boundary_output_echo_out_.register_sink(
        [&introspection_state](const ImuSample& value, std::optional<std::uint64_t> published_at_ms) {
            try {
                std::vector<std::uint8_t> payload(flowrt::detail::encoded_frame_size(value));
                flowrt::detail::encode_frame(value, std::span<std::uint8_t>{payload});
                introspection_state.record_channel_publish_bytes("echo_out", "ImuSample", std::move(payload), published_at_ms);
            } catch (...) {
            }
        });
    (void)boundary_output_echo_out_probe;
    ...
    std::uint64_t boundary_input_sample_in_seen_revision_for_consumer_main = 0;
    ...
    while (status == flowrt::Status::Ok && !shutdown.is_requested() && ((!run_ticks.has_value() || tick_base < *run_ticks) || !pending_task_order.empty())) {
        ...
        while (true) {
            ...
            if (boundary_input_sample_in_.revision() != boundary_input_sample_in_seen_revision_for_consumer_main) {
                boundary_input_sample_in_seen_revision_for_consumer_main = boundary_input_sample_in_.revision();
