...
const PACKAGE_NAME: &str = "island_sensor_rust_demo";
...
pub struct App {
    ...
    boundary_input_sample_in: flowrt::BoundaryInput<ImuSample>,
    boundary_output_echo_out: flowrt::BoundaryOutput<ImuSample>,
}
...
impl App {
    ...
    fn step(
        ...
        let _ = tick_time_ms;
        {
            ...
            if sample.present() {
                let mut echo = flowrt::Output::<ImuSample>::new();
    ...
    fn step_task_consumer_main(
        ...
        __flowrt_input_sample_value: Option<ImuSample>,
    ...
    pub fn run(self, backend: &dyn flowrt::Backend, run_ticks: Option<usize>) -> flowrt::Status {
        ...
        let _boundary_output_echo_out_probe = app.boundary_output_echo_out.register_sink({
            ...
            move |value, published_at_ms| {
                let mut payload = vec![0u8; <ImuSample as flowrt::FrameCodec>::encoded_frame_size(value)];
                if <ImuSample as flowrt::FrameCodec>::encode_frame(value, &mut payload).is_ok() {
                    introspection_state.record_channel_publish_bytes("echo_out", "ImuSample", payload, published_at_ms);
                }
            }
        });
