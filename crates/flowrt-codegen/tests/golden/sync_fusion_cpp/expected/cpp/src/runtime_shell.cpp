...
App::App(
    std::unique_ptr<ImuSrcInterface> imu_src,
    std::unique_ptr<OdomSrcInterface> odom_src,
    std::unique_ptr<FusionInterface> fusion,
    std::unique_ptr<SinkInterface> sink
)
    : imu_src_(std::move(imu_src)),
...
flowrt::Status App::step(std::size_t tick, flowrt::Context& tick_context, flowrt::IntrospectionState& introspection_state, flowrt::ScheduleWaiter& scheduler_events, std::map<std::string, flowrt::IntrospectionTaskHealth>& health_map) {
    ...
    (void)health_map;
    {
        health_map["imu_src.main"].name = "imu_src.main";
        health_map["imu_src.main"].lane = "imu_src_serial";
        flowrt::Output<Imu> imu_src_imu;
        if (imu_src_) {
            switch (imu_src_->on_tick(imu_src_imu)) {
        ...
        if (const auto* value = imu_src_imu.as_ref()) {
            bind_1_.publish_at(*value, tick_time_ms);
            scheduler_events.notify_data();
            record_introspection_publish_copy(introspection_state, "imu_src.imu_to_fusion.imu", "Imu", this->introspection_probe_bind_1, *value, tick_time_ms);
        }
    }
    {
        health_map["odom_src.main"].name = "odom_src.main";
        health_map["odom_src.main"].lane = "odom_src_serial";
        flowrt::Output<Odom> odom_src_odom;
        if (odom_src_) {
            switch (odom_src_->on_tick(odom_src_odom)) {
                case flowrt::Status::Ok:
                    break;
                case flowrt::Status::Retry:
                    return flowrt::Status::Retry;
        ...
        if (const auto* value = odom_src_odom.as_ref()) {
            bind_2_.publish_at(*value, tick_time_ms);
            scheduler_events.notify_data();
            record_introspection_publish_copy(introspection_state, "odom_src.odom_to_fusion.odom", "Odom", this->introspection_probe_bind_2, *value, tick_time_ms);
        }
    }
    {
        const auto fusion_imu = bind_1_.view_at(tick_time_ms);
        if (fusion_imu.stale()) {
            health_map["fusion.main"].stale_input += 1;
        }
        const auto fusion_odom = bind_2_.view_at(tick_time_ms);
        if (fusion_odom.stale()) {
            health_map["fusion.main"].stale_input += 1;
        }
        if (const auto* __flowrt_sync_sample = fusion_imu.get()) {
            __flowrt_sync_fusion_main_.push(0, static_cast<std::uint64_t>(__flowrt_sync_sample->stamp_ns) * 1ULL, 0);
        }
        if (const auto* __flowrt_sync_sample = fusion_odom.get()) {
            __flowrt_sync_fusion_main_.push(1, static_cast<std::uint64_t>(__flowrt_sync_sample->stamp_ns) * 1ULL, 0);
        }
        const bool __flowrt_sync_fusion_main_ready = __flowrt_sync_fusion_main_.poll().has_value();
        health_map["fusion.main"].name = "fusion.main";
        health_map["fusion.main"].lane = "fusion_serial";
        if (__flowrt_sync_fusion_main_ready) {
            flowrt::Output<Estimate> fusion_estimate;
            if (fusion_) {
                switch (fusion_->on_tick(fusion_imu, fusion_odom, fusion_estimate)) {
                    case flowrt::Status::Ok:
                        break;
                    case flowrt::Status::Retry:
                        return flowrt::Status::Retry;
            ...
            if (const auto* value = fusion_estimate.as_ref()) {
                bind_0_.publish_at(*value, tick_time_ms);
                scheduler_events.notify_data();
                record_introspection_publish_copy(introspection_state, "fusion.estimate_to_sink.estimate", "Estimate", this->introspection_probe_bind_0, *value, tick_time_ms);
            }
        }
    }
    {
        const auto sink_estimate = bind_0_.view_at(tick_time_ms);
        if (sink_estimate.stale()) {
            health_map["sink.main"].stale_input += 1;
        }
        health_map["sink.main"].name = "sink.main";
        health_map["sink.main"].lane = "sink_serial";
        if (sink_estimate.present()) {
            if (sink_) {
                switch (sink_->on_tick(sink_estimate)) {
                    case flowrt::Status::Ok:
                        break;
                    case flowrt::Status::Retry:
                        return flowrt::Status::Retry;
...
FlowrtTaskOutcome App::step_task_fusion_main(std::size_t tick, flowrt::Context& tick_context, flowrt::IntrospectionState& introspection_state, flowrt::ScheduleWaiter& scheduler_events, std::map<std::string, flowrt::IntrospectionTaskHealth>& health_map) {
    const auto tick_time_ms = static_cast<std::uint64_t>(tick);
    (void)tick_time_ms;
    (void)tick_context;
    (void)introspection_state;
    ...
    std::vector<FlowrtOutputCommit> flowrt_output_commits;
    {
        ...
        if (__flowrt_sync_fusion_main_ready) {
            ...
            if (const auto* value = fusion_estimate.as_ref()) {
                ...
                scheduler_events.notify_data();
                record_introspection_publish_copy(introspection_state, "fusion.estimate_to_sink.estimate", "Estimate", app.introspection_probe_bind_0, *value, tick_time_ms);
                    return flowrt::Status::Ok;
                });
            }
        }
...
FlowrtTaskOutcome App::step_task_imu_src_main(std::size_t tick, flowrt::Context& tick_context, flowrt::IntrospectionState& introspection_state, flowrt::ScheduleWaiter& scheduler_events, std::map<std::string, flowrt::IntrospectionTaskHealth>& health_map) {
    const auto tick_time_ms = static_cast<std::uint64_t>(tick);
    (void)tick_time_ms;
    (void)tick_context;
    (void)introspection_state;
    ...
    std::vector<FlowrtOutputCommit> flowrt_output_commits;
    {
        ...
        if (const auto* value = imu_src_imu.as_ref()) {
            ...
            flowrt_output_commits.emplace_back([flowrt_payload_0 = std::move(flowrt_payload_0), tick_time_ms](App& app, flowrt::IntrospectionState& introspection_state, flowrt::ScheduleWaiter& scheduler_events, std::map<std::string, flowrt::IntrospectionTaskHealth>& /*health_map*/) mutable {
                const auto* value = &flowrt_payload_0;
            app.bind_1_.publish_at(*value, tick_time_ms);
            scheduler_events.notify_data();
            record_introspection_publish_copy(introspection_state, "imu_src.imu_to_fusion.imu", "Imu", app.introspection_probe_bind_1, *value, tick_time_ms);
                return flowrt::Status::Ok;
            });
        }
    }
...
FlowrtTaskOutcome App::step_task_odom_src_main(std::size_t tick, flowrt::Context& tick_context, flowrt::IntrospectionState& introspection_state, flowrt::ScheduleWaiter& scheduler_events, std::map<std::string, flowrt::IntrospectionTaskHealth>& health_map) {
    const auto tick_time_ms = static_cast<std::uint64_t>(tick);
    (void)tick_time_ms;
    (void)tick_context;
    (void)introspection_state;
    ...
    std::vector<FlowrtOutputCommit> flowrt_output_commits;
    {
        ...
        if (const auto* value = odom_src_odom.as_ref()) {
            ...
            flowrt_output_commits.emplace_back([flowrt_payload_0 = std::move(flowrt_payload_0), tick_time_ms](App& app, flowrt::IntrospectionState& introspection_state, flowrt::ScheduleWaiter& scheduler_events, std::map<std::string, flowrt::IntrospectionTaskHealth>& /*health_map*/) mutable {
                const auto* value = &flowrt_payload_0;
            app.bind_2_.publish_at(*value, tick_time_ms);
            scheduler_events.notify_data();
            record_introspection_publish_copy(introspection_state, "odom_src.odom_to_fusion.odom", "Odom", app.introspection_probe_bind_2, *value, tick_time_ms);
                return flowrt::Status::Ok;
            });
        }
    }
...
FlowrtTaskOutcome App::step_task_sink_main(std::size_t tick, flowrt::Context& tick_context, flowrt::IntrospectionState& introspection_state, flowrt::ScheduleWaiter& scheduler_events, std::map<std::string, flowrt::IntrospectionTaskHealth>& health_map) {
    const auto tick_time_ms = static_cast<std::uint64_t>(tick);
    (void)tick_time_ms;
    (void)tick_context;
    (void)introspection_state;
...
FlowrtTaskOutcome App::step_process_main_task_fusion_main(std::size_t tick, flowrt::Context& tick_context, flowrt::IntrospectionState& introspection_state, flowrt::ScheduleWaiter& scheduler_events, std::map<std::string, flowrt::IntrospectionTaskHealth>& health_map) {
    const auto tick_time_ms = static_cast<std::uint64_t>(tick);
    (void)tick_time_ms;
    (void)tick_context;
    (void)introspection_state;
...
FlowrtTaskOutcome App::step_process_main_task_imu_src_main(std::size_t tick, flowrt::Context& tick_context, flowrt::IntrospectionState& introspection_state, flowrt::ScheduleWaiter& scheduler_events, std::map<std::string, flowrt::IntrospectionTaskHealth>& health_map) {
    const auto tick_time_ms = static_cast<std::uint64_t>(tick);
    (void)tick_time_ms;
    (void)tick_context;
    (void)introspection_state;
...
FlowrtTaskOutcome App::step_process_main_task_odom_src_main(std::size_t tick, flowrt::Context& tick_context, flowrt::IntrospectionState& introspection_state, flowrt::ScheduleWaiter& scheduler_events, std::map<std::string, flowrt::IntrospectionTaskHealth>& health_map) {
    const auto tick_time_ms = static_cast<std::uint64_t>(tick);
    (void)tick_time_ms;
    (void)tick_context;
    (void)introspection_state;
...
FlowrtTaskOutcome App::step_process_main_task_sink_main(std::size_t tick, flowrt::Context& tick_context, flowrt::IntrospectionState& introspection_state, flowrt::ScheduleWaiter& scheduler_events, std::map<std::string, flowrt::IntrospectionTaskHealth>& health_map) {
    const auto tick_time_ms = static_cast<std::uint64_t>(tick);
    (void)tick_time_ms;
    (void)tick_context;
    (void)introspection_state;
...
flowrt::Status App::run(const flowrt::Backend& backend, std::optional<std::size_t> run_ticks) {
    ...
    this->introspection_probe_bind_0 = register_introspection_channel(introspection_state, "fusion.estimate_to_sink.estimate", "Estimate", std::optional<std::size_t>{8});
    this->introspection_probe_bind_1 = register_introspection_channel(introspection_state, "imu_src.imu_to_fusion.imu", "Imu", std::optional<std::size_t>{16});
    this->introspection_probe_bind_2 = register_introspection_channel(introspection_state, "odom_src.odom_to_fusion.odom", "Odom", std::optional<std::size_t>{16});
    ...
    scheduler.add_lane(flowrt::LaneId{flowrt::fnv1a64("fusion_serial")}, flowrt::LaneKind::Serial);
    (void)"fusion_serial";
    scheduler.add_lane(flowrt::LaneId{flowrt::fnv1a64("imu_src_serial")}, flowrt::LaneKind::Serial);
    (void)"imu_src_serial";
    scheduler.add_lane(flowrt::LaneId{flowrt::fnv1a64("odom_src_serial")}, flowrt::LaneKind::Serial);
    (void)"odom_src_serial";
    scheduler.add_lane(flowrt::LaneId{flowrt::fnv1a64("sink_serial")}, flowrt::LaneKind::Serial);
    (void)"sink_serial";
    scheduler.add_task(flowrt::TaskSpec{.id = flowrt::TaskId{1}, .lane = flowrt::LaneId{flowrt::fnv1a64("fusion_serial")}, .priority = 0});
    scheduler.add_task(flowrt::TaskSpec{.id = flowrt::TaskId{2}, .lane = flowrt::LaneId{flowrt::fnv1a64("imu_src_serial")}, .priority = 0});
    ...
    scheduler.add_task(flowrt::TaskSpec{.id = flowrt::TaskId{3}, .lane = flowrt::LaneId{flowrt::fnv1a64("odom_src_serial")}, .priority = 0});
    scheduler.add_periodic(flowrt::PeriodicSpec{.task = flowrt::TaskId{3}, .period = std::chrono::milliseconds{10}});
    scheduler.wake(flowrt::TaskId{3});
    scheduler.add_task(flowrt::TaskSpec{.id = flowrt::TaskId{4}, .lane = flowrt::LaneId{flowrt::fnv1a64("sink_serial")}, .priority = 0});
    std::uint64_t bind_1_seen_revision_for_fusion_main = 0;
    std::uint64_t bind_2_seen_revision_for_fusion_main = 0;
    std::uint64_t bind_0_seen_revision_for_sink_main = 0;
    ...
    while (status == flowrt::Status::Ok && !shutdown.is_requested() && ((!run_ticks.has_value() || tick_base < *run_ticks) || !pending_task_order.empty())) {
        ...
        {
            auto& health = health_map["fusion.main"];
            health.name = "fusion.main";
            health.lane = "fusion_serial";
        }
        {
            auto& health = health_map["imu_src.main"];
            health.name = "imu_src.main";
            health.lane = "imu_src_serial";
        }
        {
            auto& health = health_map["odom_src.main"];
            health.name = "odom_src.main";
            health.lane = "odom_src_serial";
        }
        {
            auto& health = health_map["sink.main"];
            health.name = "sink.main";
            health.lane = "sink_serial";
        }
        ...
        while (true) {
            ...
            if (bind_1_.revision() != bind_1_seen_revision_for_fusion_main || bind_2_.revision() != bind_2_seen_revision_for_fusion_main) {
                bind_1_seen_revision_for_fusion_main = bind_1_.revision();
                bind_2_seen_revision_for_fusion_main = bind_2_.revision();
                scheduler.wake(flowrt::TaskId{1});
                woke_on_message = true;
            }
            if (bind_0_.revision() != bind_0_seen_revision_for_sink_main) {
                bind_0_seen_revision_for_sink_main = bind_0_.revision();
                ...
                const auto submitted = worker_pool.submit_collect(admission.task, task_completion_queue, [this, &introspection_state, &scheduler_events, &task_health_mutex, &task_health_from_workers, admission, scheduled_delta_ms, observed_delta_ms, task_clock_source, tick_base, tick_time_ms]() {
                    ...
                    const auto [task_name, task_trigger] = [&]() -> std::pair<std::string_view, std::string_view> {
                        switch (admission.task.value) {
                            case 1: return {"fusion.main", "on_synchronized"};
                            case 2: return {"imu_src.main", "periodic"};
                            case 3: return {"odom_src.main", "periodic"};
                            case 4: return {"sink.main", "on_message"};
                    ...
                    switch (admission.task.value) {
                    case 1: {
auto flowrt_lane_guard = flowrt::enter_lane(flowrt::LaneId{flowrt::fnv1a64("fusion_serial")});
(void)flowrt_lane_guard;
auto task_outcome = step_task_fusion_main(static_cast<std::size_t>(tick_time_ms), local_context, introspection_state, scheduler_events, local_health_map);
merge_local_health(std::move(local_health_map));
return task_outcome;
}
                    case 2: {
auto flowrt_lane_guard = flowrt::enter_lane(flowrt::LaneId{flowrt::fnv1a64("imu_src_serial")});
(void)flowrt_lane_guard;
auto task_outcome = step_task_imu_src_main(static_cast<std::size_t>(tick_time_ms), local_context, introspection_state, scheduler_events, local_health_map);
merge_local_health(std::move(local_health_map));
return task_outcome;
}
                    case 3: {
auto flowrt_lane_guard = flowrt::enter_lane(flowrt::LaneId{flowrt::fnv1a64("odom_src_serial")});
(void)flowrt_lane_guard;
auto task_outcome = step_task_odom_src_main(static_cast<std::size_t>(tick_time_ms), local_context, introspection_state, scheduler_events, local_health_map);
merge_local_health(std::move(local_health_map));
return task_outcome;
}
                    case 4: {
auto flowrt_lane_guard = flowrt::enter_lane(flowrt::LaneId{flowrt::fnv1a64("sink_serial")});
(void)flowrt_lane_guard;
auto task_outcome = step_task_sink_main(static_cast<std::size_t>(tick_time_ms), local_context, introspection_state, scheduler_events, local_health_map);
merge_local_health(std::move(local_health_map));
return task_outcome;
}
...
break;
        ...
        // 公平性检测：检查 lane 饥饿。
        if (scheduler.lane_starvation_ticks(flowrt::LaneId{flowrt::fnv1a64("fusion_serial")}) > fairness_starvation_threshold) {
        ...
        if (scheduler.lane_starvation_ticks(flowrt::LaneId{flowrt::fnv1a64("imu_src_serial")}) > fairness_starvation_threshold) {
            for (auto &[name, health] : health_map) {
        ...
        if (scheduler.lane_starvation_ticks(flowrt::LaneId{flowrt::fnv1a64("odom_src_serial")}) > fairness_starvation_threshold) {
            for (auto &[name, health] : health_map) {
        ...
        if (scheduler.lane_starvation_ticks(flowrt::LaneId{flowrt::fnv1a64("sink_serial")}) > fairness_starvation_threshold) {
            for (auto &[name, health] : health_map) {
        ...
        if (status == flowrt::Status::Ok) {
            ...
            const auto next_periodic_deadline_ms = std::optional<std::chrono::milliseconds>{std::min({scheduler.next_deadline(flowrt::TaskId{2}), scheduler.next_deadline(flowrt::TaskId{3})})};
    ...
    if (sink_started && sink_) {
        const auto stop_status = sink_->on_stop(lifecycle_context);
...
auto flowrt_lane_guard = flowrt::enter_lane(flowrt::LaneId{flowrt::fnv1a64("fusion_serial")});
(void)flowrt_lane_guard;
auto task_outcome = step_process_main_task_fusion_main(static_cast<std::size_t>(tick_time_ms), local_context, introspection_state, scheduler_events, local_health_map);
merge_local_health(std::move(local_health_map));
return task_outcome;
}
                    case 2: {
auto flowrt_lane_guard = flowrt::enter_lane(flowrt::LaneId{flowrt::fnv1a64("imu_src_serial")});
(void)flowrt_lane_guard;
auto task_outcome = step_process_main_task_imu_src_main(static_cast<std::size_t>(tick_time_ms), local_context, introspection_state, scheduler_events, local_health_map);
merge_local_health(std::move(local_health_map));
return task_outcome;
}
                    case 3: {
auto flowrt_lane_guard = flowrt::enter_lane(flowrt::LaneId{flowrt::fnv1a64("odom_src_serial")});
(void)flowrt_lane_guard;
auto task_outcome = step_process_main_task_odom_src_main(static_cast<std::size_t>(tick_time_ms), local_context, introspection_state, scheduler_events, local_health_map);
merge_local_health(std::move(local_health_map));
return task_outcome;
}
                    case 4: {
auto flowrt_lane_guard = flowrt::enter_lane(flowrt::LaneId{flowrt::fnv1a64("sink_serial")});
(void)flowrt_lane_guard;
auto task_outcome = step_process_main_task_sink_main(static_cast<std::size_t>(tick_time_ms), local_context, introspection_state, scheduler_events, local_health_map);
merge_local_health(std::move(local_health_map));
return task_outcome;
}
                    default: return FlowrtTaskOutcome::error(std::vector<FlowrtOutputCommit>{});
