...
App::App(
    std::unique_ptr<ProducerInterface> flaky,
    std::unique_ptr<SinkInterface> consumer,
    std::unique_ptr<ProducerInterface> guard
)
...
flowrt::Status App::step(std::size_t tick, flowrt::Context& tick_context, flowrt::IntrospectionState& introspection_state, flowrt::ScheduleWaiter& scheduler_events, std::map<std::string, flowrt::IntrospectionTaskHealth>& health_map) {
    ...
    (void)health_map;
    {
        ...
        flowrt::Output<Sample> flaky_sample;
        if (flaky_) {
            switch (flaky_->on_tick(flaky_sample)) {
        ...
        if (const auto* value = flaky_sample.as_ref()) {
            bind_0_.publish_at(*value, tick_time_ms);
            scheduler_events.notify_data();
            record_introspection_publish_copy(introspection_state, "flaky.sample_to_consumer.sample", "Sample", this->introspection_probe_bind_0, *value, tick_time_ms);
        }
    }
    {
        const auto consumer_sample = bind_0_.view_at(tick_time_ms);
        if (consumer_sample.stale()) {
            health_map["consumer.main"].stale_input += 1;
        }
        health_map["consumer.main"].name = "consumer.main";
        health_map["consumer.main"].lane = "consumer_serial";
        if (consumer_sample.present()) {
            if (consumer_) {
                switch (consumer_->on_tick(consumer_sample)) {
                    case flowrt::Status::Ok:
                        break;
                    case flowrt::Status::Retry:
                        return flowrt::Status::Retry;
    ...
    {
        health_map["guard.main"].name = "guard.main";
        health_map["guard.main"].lane = "guard_serial";
        flowrt::Output<Sample> guard_sample;
        if (guard_) {
            switch (guard_->on_tick(guard_sample)) {
                case flowrt::Status::Ok:
                    break;
                case flowrt::Status::Retry:
                    return flowrt::Status::Retry;
...
FlowrtTaskOutcome App::step_task_consumer_main(std::size_t tick, flowrt::Context& tick_context, flowrt::IntrospectionState& introspection_state, flowrt::ScheduleWaiter& scheduler_events, std::map<std::string, flowrt::IntrospectionTaskHealth>& health_map) {
    const auto tick_time_ms = static_cast<std::uint64_t>(tick);
    (void)tick_time_ms;
    (void)tick_context;
    (void)introspection_state;
...
FlowrtTaskOutcome App::step_task_flaky_main(std::size_t tick, flowrt::Context& tick_context, flowrt::IntrospectionState& introspection_state, flowrt::ScheduleWaiter& scheduler_events, std::map<std::string, flowrt::IntrospectionTaskHealth>& health_map) {
    ...
    std::vector<FlowrtOutputCommit> flowrt_output_commits;
    {
        ...
        if (const auto* value = flaky_sample.as_ref()) {
            ...
            scheduler_events.notify_data();
            record_introspection_publish_copy(introspection_state, "flaky.sample_to_consumer.sample", "Sample", app.introspection_probe_bind_0, *value, tick_time_ms);
                return flowrt::Status::Ok;
            });
        }
    }
...
FlowrtTaskOutcome App::step_task_guard_main(std::size_t tick, flowrt::Context& tick_context, flowrt::IntrospectionState& introspection_state, flowrt::ScheduleWaiter& scheduler_events, std::map<std::string, flowrt::IntrospectionTaskHealth>& health_map) {
    const auto tick_time_ms = static_cast<std::uint64_t>(tick);
    (void)tick_time_ms;
    (void)tick_context;
    (void)introspection_state;
...
FlowrtTaskOutcome App::step_process_main_task_consumer_main(std::size_t tick, flowrt::Context& tick_context, flowrt::IntrospectionState& introspection_state, flowrt::ScheduleWaiter& scheduler_events, std::map<std::string, flowrt::IntrospectionTaskHealth>& health_map) {
    const auto tick_time_ms = static_cast<std::uint64_t>(tick);
    (void)tick_time_ms;
    (void)tick_context;
    (void)introspection_state;
...
FlowrtTaskOutcome App::step_process_main_task_guard_main(std::size_t tick, flowrt::Context& tick_context, flowrt::IntrospectionState& introspection_state, flowrt::ScheduleWaiter& scheduler_events, std::map<std::string, flowrt::IntrospectionTaskHealth>& health_map) {
    const auto tick_time_ms = static_cast<std::uint64_t>(tick);
    (void)tick_time_ms;
    (void)tick_context;
    (void)introspection_state;
...
flowrt::Status App::run(const flowrt::Backend& backend, std::optional<std::size_t> run_ticks) {
    ...
    this->introspection_probe_bind_0 = register_introspection_channel(introspection_state, "flaky.sample_to_consumer.sample", "Sample", std::optional<std::size_t>{4});
    auto introspection_server = flowrt::spawn_status_server(
        flowrt::IntrospectionIdentity{
            ...
            .package = "graph_health_stop_cpp_demo",
            .process = "main",
            .runtime = "cpp",
        },
        introspection_state);
    ...
    if (status == flowrt::Status::Ok && guard_) {
        status = guard_->on_init(lifecycle_context);
        guard_initialized = status == flowrt::Status::Ok;
        introspection_state.record_lifecycle_state("guard", guard_initialized ? flowrt::LifecycleState::Initialized : flowrt::LifecycleState::Faulted);
    }
    ...
    if (status == flowrt::Status::Ok && guard_initialized && guard_) {
        status = guard_->on_start(lifecycle_context);
        guard_started = status == flowrt::Status::Ok;
        introspection_state.record_lifecycle_state("guard", guard_started ? flowrt::LifecycleState::Running : flowrt::LifecycleState::Faulted);
    }
    ...
    scheduler.add_lane(flowrt::LaneId{flowrt::fnv1a64("consumer_serial")}, flowrt::LaneKind::Serial);
    (void)"consumer_serial";
    scheduler.add_lane(flowrt::LaneId{flowrt::fnv1a64("flaky_serial")}, flowrt::LaneKind::Serial);
    (void)"flaky_serial";
    scheduler.add_lane(flowrt::LaneId{flowrt::fnv1a64("guard_serial")}, flowrt::LaneKind::Serial);
    (void)"guard_serial";
    scheduler.add_task(flowrt::TaskSpec{.id = flowrt::TaskId{1}, .lane = flowrt::LaneId{flowrt::fnv1a64("consumer_serial")}, .priority = 0});
    scheduler.add_task(flowrt::TaskSpec{.id = flowrt::TaskId{2}, .lane = flowrt::LaneId{flowrt::fnv1a64("flaky_serial")}, .priority = 0});
    ...
    scheduler.add_task(flowrt::TaskSpec{.id = flowrt::TaskId{3}, .lane = flowrt::LaneId{flowrt::fnv1a64("guard_serial")}, .priority = 0});
    ...
    std::uint64_t bind_0_seen_revision_for_consumer_main = 0;
    ...
    bool _graph_terminal_fault = false;
    ...
    while (status == flowrt::Status::Ok && !shutdown.is_requested() && ((!run_ticks.has_value() || tick_base < *run_ticks) || !pending_task_order.empty() || flaky_next_restart_ms.has_value())) {
        ...
        {
            auto& health = health_map["consumer.main"];
            health.name = "consumer.main";
            health.lane = "consumer_serial";
        }
        {
        ...
        {
            auto& health = health_map["guard.main"];
            health.name = "guard.main";
            health.lane = "guard_serial";
        }
        ...
        while (true) {
            ...
            if (bind_0_.revision() != bind_0_seen_revision_for_consumer_main) {
                bind_0_seen_revision_for_consumer_main = bind_0_.revision();
                ...
                const auto submitted = worker_pool.submit_collect(admission.task, task_completion_queue, [this, &introspection_state, &scheduler_events, &task_health_mutex, &task_health_from_workers, admission, scheduled_delta_ms, observed_delta_ms, task_clock_source, tick_base, tick_time_ms]() {
                    ...
                    const auto [task_name, task_trigger] = [&]() -> std::pair<std::string_view, std::string_view> {
                        switch (admission.task.value) {
                            case 1: return {"consumer.main", "on_message"};
                            case 2: return {"flaky.main", "periodic"};
                            case 3: return {"guard.main", "periodic"};
                    ...
                    switch (admission.task.value) {
                    case 1: {
auto flowrt_lane_guard = flowrt::enter_lane(flowrt::LaneId{flowrt::fnv1a64("consumer_serial")});
(void)flowrt_lane_guard;
auto task_outcome = step_task_consumer_main(static_cast<std::size_t>(tick_time_ms), local_context, introspection_state, scheduler_events, local_health_map);
...
(void)flowrt_lane_guard;
auto task_outcome = step_task_flaky_main(static_cast<std::size_t>(tick_time_ms), local_context, introspection_state, scheduler_events, local_health_map);
merge_local_health(std::move(local_health_map));
return task_outcome;
}
                    case 3: {
auto flowrt_lane_guard = flowrt::enter_lane(flowrt::LaneId{flowrt::fnv1a64("guard_serial")});
(void)flowrt_lane_guard;
auto task_outcome = step_task_guard_main(static_cast<std::size_t>(tick_time_ms), local_context, introspection_state, scheduler_events, local_health_map);
merge_local_health(std::move(local_health_map));
return task_outcome;
}
...
break;
        ...
        // 公平性检测：检查 lane 饥饿。
        if (scheduler.lane_starvation_ticks(flowrt::LaneId{flowrt::fnv1a64("consumer_serial")}) > fairness_starvation_threshold) {
        ...
        if (scheduler.lane_starvation_ticks(flowrt::LaneId{flowrt::fnv1a64("guard_serial")}) > fairness_starvation_threshold) {
            for (auto &[name, health] : health_map) {
    ...
    if (guard_started && guard_) {
        const auto stop_status = guard_->on_stop(lifecycle_context);
        if (status == flowrt::Status::Ok && stop_status != flowrt::Status::Ok) {
            status = flowrt::Status::Error;
        }
        introspection_state.record_lifecycle_state("guard", stop_status == flowrt::Status::Ok ? flowrt::LifecycleState::Stopped : flowrt::LifecycleState::Faulted);
    }
    if (consumer_started && consumer_) {
        const auto stop_status = consumer_->on_stop(lifecycle_context);
        if (status == flowrt::Status::Ok && stop_status != flowrt::Status::Ok) {
            status = flowrt::Status::Error;
        }
    ...
    if (guard_initialized && guard_) {
        const auto shutdown_status = guard_->on_shutdown(lifecycle_context);
        if (status == flowrt::Status::Ok && shutdown_status != flowrt::Status::Ok) {
            status = flowrt::Status::Error;
        }
        introspection_state.record_lifecycle_state("guard", shutdown_status == flowrt::Status::Ok ? flowrt::LifecycleState::ShutDown : flowrt::LifecycleState::Faulted);
    }
...
auto flowrt_lane_guard = flowrt::enter_lane(flowrt::LaneId{flowrt::fnv1a64("consumer_serial")});
(void)flowrt_lane_guard;
auto task_outcome = step_process_main_task_consumer_main(static_cast<std::size_t>(tick_time_ms), local_context, introspection_state, scheduler_events, local_health_map);
merge_local_health(std::move(local_health_map));
return task_outcome;
}
                    case 2: {
auto flowrt_lane_guard = flowrt::enter_lane(flowrt::LaneId{flowrt::fnv1a64("flaky_serial")});
(void)flowrt_lane_guard;
auto task_outcome = step_process_main_task_flaky_main(static_cast<std::size_t>(tick_time_ms), local_context, introspection_state, scheduler_events, local_health_map);
merge_local_health(std::move(local_health_map));
return task_outcome;
}
                    case 3: {
auto flowrt_lane_guard = flowrt::enter_lane(flowrt::LaneId{flowrt::fnv1a64("guard_serial")});
(void)flowrt_lane_guard;
auto task_outcome = step_process_main_task_guard_main(static_cast<std::size_t>(tick_time_ms), local_context, introspection_state, scheduler_events, local_health_map);
merge_local_health(std::move(local_health_map));
return task_outcome;
}
                    default: return FlowrtTaskOutcome::error(std::vector<FlowrtOutputCommit>{});
