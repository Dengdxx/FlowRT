...
App::App(
    std::unique_ptr<MonitorInterface> monitor
)
...
flowrt::Status App::step(std::size_t tick, flowrt::Context& tick_context, flowrt::IntrospectionState& introspection_state, flowrt::ScheduleWaiter& scheduler_events, std::map<std::string, flowrt::IntrospectionTaskHealth>& health_map) {
    ...
    (void)health_map;
    {
        const auto monitor_sample_read = boundary_input_feed_.read_at(tick_time_ms);
        const auto monitor_sample = monitor_sample_read.view();
        health_map["monitor.main"].name = "monitor.main";
        health_map["monitor.main"].lane = "monitor_serial";
        if (monitor_sample.present()) {
            flowrt::Output<Sample> monitor_echo;
            if (monitor_) {
                switch (monitor_->on_tick(monitor_sample, monitor_echo)) {
            ...
            if (const auto* value = monitor_echo.as_ref()) {
...
flowrt::Status App::run(const flowrt::Backend& backend, std::optional<std::size_t> run_ticks) {
    ...
    auto introspection_server = flowrt::spawn_status_server(
        flowrt::IntrospectionIdentity{
            ...
            .package = "fault_injection_degrade_demo",
            .process = "main",
            .runtime = "cpp",
        },
        introspection_state);
    ...
    (void)"monitor_serial";
    scheduler.add_task(flowrt::TaskSpec{.id = flowrt::TaskId{1}, .lane = flowrt::LaneId{flowrt::fnv1a64("monitor_serial")}, .priority = 0});
    std::uint64_t boundary_input_feed_seen_revision_for_monitor_main = 0;
    ...
    while (status == flowrt::Status::Ok && !shutdown.is_requested() && ((!run_ticks.has_value() || tick_base < *run_ticks) || !pending_task_order.empty())) {
        ...
        while (true) {
            ...
            if (boundary_input_feed_.revision() != boundary_input_feed_seen_revision_for_monitor_main) {
                boundary_input_feed_seen_revision_for_monitor_main = boundary_input_feed_.revision();
            ...
            for (const auto admission : ready_batch.admissions()) {
                ...
                switch (admission.task.value) {
                    case 1: { ++__inject_count_1; flowrt_inject_fault = __inject_count_1 == 1ULL || __inject_count_1 == 2ULL; break; }
                    default: break;
                }
                const auto submitted = worker_pool.submit_collect(admission.task, task_completion_queue, [this, &introspection_state, &scheduler_events, &task_health_mutex, &task_health_from_workers, admission, scheduled_delta_ms, observed_delta_ms, task_clock_source, tick_base, tick_time_ms, flowrt_inject_fault]() {
                    ...
                    const auto [task_name, task_trigger] = [&]() -> std::pair<std::string_view, std::string_view> {
                        switch (admission.task.value) {
                            case 1: return {"monitor.main", "on_message"};
...
auto task_outcome = flowrt_inject_fault ? FlowrtTaskOutcome::error(std::vector<FlowrtOutputCommit>{}) : step_task_monitor_main(static_cast<std::size_t>(tick_time_ms), local_context, introspection_state, scheduler_events, local_health_map);
...
auto flowrt_lane_guard = flowrt::enter_lane(flowrt::LaneId{flowrt::fnv1a64("monitor_serial")});
(void)flowrt_lane_guard;
auto task_outcome = flowrt_inject_fault ? FlowrtTaskOutcome::error(std::vector<FlowrtOutputCommit>{}) : step_process_main_task_monitor_main(static_cast<std::size_t>(tick_time_ms), local_context, introspection_state, scheduler_events, local_health_map);
merge_local_health(std::move(local_health_map));
return task_outcome;
}
                    default: return FlowrtTaskOutcome::error(std::vector<FlowrtOutputCommit>{});
