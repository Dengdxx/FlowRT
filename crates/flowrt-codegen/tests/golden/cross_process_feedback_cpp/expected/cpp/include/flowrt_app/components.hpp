...
public:
     ...
     *
     * @param state latest snapshot 输入视图。
     * @param cmd 输出端口写入句柄。
     * @return 本次回调的 FlowRT 执行状态。
     */
    virtual flowrt::Status on_tick(
        const flowrt::Latest<State>& state,
        flowrt::Output<Cmd>& cmd) = 0;
};

/**
 * @brief `plant` 组件的 C++ 用户实现接口。
 *
 * 用户代码实现该接口并交给 FlowRT 管理的 runtime shell。接口只暴露组件算法所需的生命周期、输入视图和输出句柄，不暴露具体 backend API。
 */
...
public:
    ...
    /**
     * @brief 执行一次 `plant` 组件调度回调。
     *
     * runtime shell 按 Contract IR 中的 task 和 dataflow 顺序调用该方法。输入使用 latest snapshot 视图，输出通过 `flowrt::Output<T>` 写入，本方法不得保存输入视图内部指针到回调之外。
     *
     * @param cmd latest snapshot 输入视图。
     * @param state 输出端口写入句柄。
     * @return 本次回调的 FlowRT 执行状态。
     */
    virtual flowrt::Status on_tick(
        const flowrt::Latest<Cmd>& cmd,
        flowrt::Output<State>& state) = 0;
};
