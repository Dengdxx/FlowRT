...
App::App(
    ...
    : controller_(std::move(controller)),
      ...
      bind_0_(flowrt::zenoh::ZenohPubSub<Cmd>::open_with_config("flowrt/xproc_feedback_cpp/default/default/bind_0/controller_cmd_to_plant_cmd", flowrt::zenoh::ZenohChannelConfig::latest().with_stale_config(flowrt::StaleConfig{flowrt::StalePolicy::Warn}))),
      bind_1_(flowrt::zenoh::ZenohPubSub<State>::open_with_config("flowrt/xproc_feedback_cpp/default/default/bind_1/plant_state_to_controller_state", flowrt::zenoh::ZenohChannelConfig::latest().with_stale_config(flowrt::StaleConfig{flowrt::StalePolicy::Warn}))) {
}
...
flowrt::Status App::step(std::size_t tick, flowrt::Context& tick_context, flowrt::IntrospectionState& introspection_state, flowrt::ScheduleWaiter& scheduler_events, std::map<std::string, flowrt::IntrospectionTaskHealth>& health_map) {
    ...
    (void)health_map;
    {
        auto controller_state_result = bind_1_.receive_latest_at(tick_time_ms);
        if (std::holds_alternative<flowrt::ChannelError>(controller_state_result)) {
            return flowrt::Status::Error;
        }
        const auto controller_state = std::get<flowrt::Latest<State>>(controller_state_result);
    ...
    {
        auto plant_cmd_result = bind_0_.receive_latest_at(tick_time_ms);
        if (std::holds_alternative<flowrt::ChannelError>(plant_cmd_result)) {
            return flowrt::Status::Error;
        }
        const auto plant_cmd = std::get<flowrt::Latest<Cmd>>(plant_cmd_result);
        if (plant_cmd.stale()) {
...
flowrt::Status App::step_process_ctrl_proc(std::size_t tick, flowrt::Context& tick_context, flowrt::IntrospectionState& introspection_state, flowrt::ScheduleWaiter& scheduler_events, std::map<std::string, flowrt::IntrospectionTaskHealth>& health_map) {
    const auto tick_time_ms = static_cast<std::uint64_t>(tick);
    (void)tick_time_ms;
    (void)tick_context;
    (void)introspection_state;
...
flowrt::Status App::step_process_ctrl_proc_startup(std::size_t tick, flowrt::Context& tick_context, flowrt::IntrospectionState& introspection_state, flowrt::ScheduleWaiter& scheduler_events, std::map<std::string, flowrt::IntrospectionTaskHealth>& health_map) {
    const auto tick_time_ms = static_cast<std::uint64_t>(tick);
    (void)tick_time_ms;
    (void)tick_context;
    (void)introspection_state;
...
flowrt::Status App::step_process_ctrl_proc_shutdown(std::size_t tick, flowrt::Context& tick_context, flowrt::IntrospectionState& introspection_state, flowrt::ScheduleWaiter& scheduler_events, std::map<std::string, flowrt::IntrospectionTaskHealth>& health_map) {
    const auto tick_time_ms = static_cast<std::uint64_t>(tick);
    (void)tick_time_ms;
    (void)tick_context;
    (void)introspection_state;
...
FlowrtTaskOutcome App::step_process_ctrl_proc_task_controller_main(std::size_t tick, flowrt::Context& tick_context, flowrt::IntrospectionState& introspection_state, flowrt::ScheduleWaiter& scheduler_events, std::map<std::string, flowrt::IntrospectionTaskHealth>& health_map) {
    const auto tick_time_ms = static_cast<std::uint64_t>(tick);
    (void)tick_time_ms;
    (void)tick_context;
    (void)introspection_state;
...
flowrt::Status App::step_process_plant_proc(std::size_t tick, flowrt::Context& tick_context, flowrt::IntrospectionState& introspection_state, flowrt::ScheduleWaiter& scheduler_events, std::map<std::string, flowrt::IntrospectionTaskHealth>& health_map) {
    const auto tick_time_ms = static_cast<std::uint64_t>(tick);
    (void)tick_time_ms;
    (void)tick_context;
    (void)introspection_state;
...
flowrt::Status App::step_process_plant_proc_startup(std::size_t tick, flowrt::Context& tick_context, flowrt::IntrospectionState& introspection_state, flowrt::ScheduleWaiter& scheduler_events, std::map<std::string, flowrt::IntrospectionTaskHealth>& health_map) {
    const auto tick_time_ms = static_cast<std::uint64_t>(tick);
    (void)tick_time_ms;
    (void)tick_context;
    (void)introspection_state;
...
flowrt::Status App::step_process_plant_proc_shutdown(std::size_t tick, flowrt::Context& tick_context, flowrt::IntrospectionState& introspection_state, flowrt::ScheduleWaiter& scheduler_events, std::map<std::string, flowrt::IntrospectionTaskHealth>& health_map) {
    const auto tick_time_ms = static_cast<std::uint64_t>(tick);
    (void)tick_time_ms;
    (void)tick_context;
    (void)introspection_state;
...
FlowrtTaskOutcome App::step_process_plant_proc_task_plant_main(std::size_t tick, flowrt::Context& tick_context, flowrt::IntrospectionState& introspection_state, flowrt::ScheduleWaiter& scheduler_events, std::map<std::string, flowrt::IntrospectionTaskHealth>& health_map) {
    const auto tick_time_ms = static_cast<std::uint64_t>(tick);
    (void)tick_time_ms;
    (void)tick_context;
    (void)introspection_state;
...
flowrt::Status App::run(const flowrt::Backend& backend, std::optional<std::size_t> run_ticks) {
    ...
    bind_1_.set_schedule_waiter(scheduler_events);
    ...
    auto introspection_server = flowrt::spawn_status_server(
        flowrt::IntrospectionIdentity{
            ...
            .package = "xproc_feedback_cpp",
            .process = "main",
            .runtime = "cpp",
        },
        introspection_state);
    (void)introspection_server;
    bind_1_.publish_at([]{ State __seed{}; __seed.x = 0.0; return __seed; }(), 0);
...
flowrt::Status App::run_process(const flowrt::Backend& backend, std::string_view process, std::optional<std::size_t> run_ticks) {
    if (process == "ctrl_proc") {
        return run_process_ctrl_proc(backend, run_ticks);
    }
    if (process == "plant_proc") {
        return run_process_plant_proc(backend, run_ticks);
    }
    return flowrt::Status::Error;
}

flowrt::Status App::run_process_ctrl_proc(const flowrt::Backend& backend, std::optional<std::size_t> run_ticks) {
    flowrt::Context lifecycle_context;
    auto status = flowrt::Status::Ok;
    (void)backend;
    auto shutdown = flowrt::install_signal_shutdown_token();
    ...
    if (status == flowrt::Status::Ok) {
        std::map<std::string, flowrt::IntrospectionTaskHealth> startup_health_map;
        status = step_process_ctrl_proc_startup(0, lifecycle_context, introspection_state, scheduler_events, startup_health_map);
    }
    flowrt::DeterministicExecutor scheduler{1};
    flowrt::WorkerPool worker_pool{1};
    scheduler.add_lane(flowrt::LaneId{flowrt::fnv1a64("controller_serial")}, flowrt::LaneKind::Serial);
...
auto flowrt_lane_guard = flowrt::enter_lane(flowrt::LaneId{flowrt::fnv1a64("controller_serial")});
(void)flowrt_lane_guard;
auto task_outcome = step_process_ctrl_proc_task_controller_main(static_cast<std::size_t>(tick_time_ms), local_context, introspection_state, scheduler_events, local_health_map);
merge_local_health(std::move(local_health_map));
return task_outcome;
}
                    default: return FlowrtTaskOutcome::error(std::vector<FlowrtOutputCommit>{});
...
break;
    ...
    if (status == flowrt::Status::Ok) {
        std::map<std::string, flowrt::IntrospectionTaskHealth> shutdown_health_map;
        status = step_process_ctrl_proc_shutdown(0, lifecycle_context, introspection_state, scheduler_events, shutdown_health_map);
    }
    if (controller_started && controller_) {
        const auto stop_status = controller_->on_stop(lifecycle_context);
        if (status == flowrt::Status::Ok && stop_status != flowrt::Status::Ok) {
...
flowrt::Status App::run_process_plant_proc(const flowrt::Backend& backend, std::optional<std::size_t> run_ticks) {
    flowrt::Context lifecycle_context;
    auto status = flowrt::Status::Ok;
    (void)backend;
    auto shutdown = flowrt::install_signal_shutdown_token();
    ...
    if (status == flowrt::Status::Ok) {
        std::map<std::string, flowrt::IntrospectionTaskHealth> startup_health_map;
        status = step_process_plant_proc_startup(0, lifecycle_context, introspection_state, scheduler_events, startup_health_map);
    }
    flowrt::DeterministicExecutor scheduler{1};
    flowrt::WorkerPool worker_pool{1};
    scheduler.add_lane(flowrt::LaneId{flowrt::fnv1a64("plant_serial")}, flowrt::LaneKind::Serial);
    (void)"plant_serial";
    scheduler.add_task(flowrt::TaskSpec{.id = flowrt::TaskId{1}, .lane = flowrt::LaneId{flowrt::fnv1a64("plant_serial")}, .priority = 0});
    scheduler.add_periodic(flowrt::PeriodicSpec{.task = flowrt::TaskId{1}, .period = std::chrono::milliseconds{10}});
    scheduler.wake(flowrt::TaskId{1});
    const auto scheduler_base_period_ms = std::uint64_t{10};
    std::size_t tick_base = 0;
        ...
        while (true) {
            ...
            for (const auto admission : ready_batch.admissions()) {
                ...
                const auto submitted = worker_pool.submit_collect(admission.task, task_completion_queue, [this, &introspection_state, &scheduler_events, &task_health_mutex, &task_health_from_workers, admission, scheduled_delta_ms, observed_delta_ms, task_clock_source, tick_base, tick_time_ms]() {
                    ...
                    const auto [task_name, task_trigger] = [&]() -> std::pair<std::string_view, std::string_view> {
                        switch (admission.task.value) {
                            case 1: return {"plant.main", "periodic"};
                            default: return {"__flowrt_hidden", "on_message"};
                        }
                    }();
                    auto local_context = flowrt::Context::with_timing(flowrt::TaskTiming{
...
auto flowrt_lane_guard = flowrt::enter_lane(flowrt::LaneId{flowrt::fnv1a64("plant_serial")});
(void)flowrt_lane_guard;
auto task_outcome = step_process_plant_proc_task_plant_main(static_cast<std::size_t>(tick_time_ms), local_context, introspection_state, scheduler_events, local_health_map);
merge_local_health(std::move(local_health_map));
return task_outcome;
}
                    default: return FlowrtTaskOutcome::error(std::vector<FlowrtOutputCommit>{});
...
break;
    ...
    if (status == flowrt::Status::Ok) {
        std::map<std::string, flowrt::IntrospectionTaskHealth> shutdown_health_map;
        status = step_process_plant_proc_shutdown(0, lifecycle_context, introspection_state, scheduler_events, shutdown_health_map);
    }
    if (plant_started && plant_) {
        const auto stop_status = plant_->on_stop(lifecycle_context);
        if (status == flowrt::Status::Ok && stop_status != flowrt::Status::Ok) {
...
flowrt::Status run(std::optional<std::size_t> run_ticks) {
    auto backend = flowrt::zenoh_backend();
