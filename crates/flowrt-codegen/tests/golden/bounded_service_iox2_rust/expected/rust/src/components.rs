...
/// `plan_client.plan` service client typed handle（iox2 backend）。
///
...
pub struct ServiceClient_planner_plan {
    pub(crate) inner: std::sync::Arc<std::sync::OnceLock<flowrt::iox2::Iox2FrameServiceClient<PlanRequest, PlanResponse, 44, 21>>>,
}

impl ServiceClient_planner_plan {
    /// 发起同步阻塞 iox2 service 调用。
///
...
None => flowrt::ServiceResult::err(flowrt::ServiceError::Unavailable),
    ...
    /// 发起 iox2 service 调用并返回就绪 `ServiceCallHandle`。
///
