...
const PACKAGE_NAME: &str = "bounded_service_iox2_rust";
...
pub struct App {
    ...
    plan_svc: std::sync::Arc<Box<dyn PlanService + Send + Sync>>,
    service_client_plan_client_plan: ServiceClient_planner_plan,
    service_server_plan_svc_plan: std::sync::Arc<std::sync::OnceLock<std::sync::Mutex<flowrt::iox2::Iox2FrameServiceServer<PlanRequest, PlanResponse, 44, 21>>>>,
}

impl App {
    pub fn new(
        plan_client: Box<dyn Planner + Send>,
        plan_svc: Box<dyn PlanService + Send + Sync>,
    ) -> Self {
        let startup_status = flowrt::Status::Ok;
        let plan_client = std::sync::Arc::new(std::sync::Mutex::new(plan_client));
        let plan_svc = std::sync::Arc::new(plan_svc);
        Self {
    ...
    fn step(
        ...
        }
        {
            ...
            match self.plan_svc.as_ref().as_ref().on_tick() {
                flowrt::Status::Ok => {}
                flowrt::Status::Retry => return flowrt::Status::Retry,
                flowrt::Status::Error => return flowrt::Status::Error,
            }
    ...
    fn step_task_plan_svc_main(
        __flowrt_component_plan_svc: std::sync::Arc<Box<dyn PlanService + Send + Sync>>,
        tick: usize,
        _tick_context: &mut flowrt::Context,
        introspection_state: &flowrt::IntrospectionState,
        scheduler_events: &flowrt::ScheduleWaiter,
        ...
        let mut __flowrt_output_commits: Vec<FlowrtOutputCommit> = Vec::new();
        {
            ...
            match __flowrt_component_plan_svc.as_ref().as_ref().on_tick() {
                flowrt::Status::Ok => {}
                flowrt::Status::Retry => return flowrt::TaskRunOutcome::retry(Vec::new()),
                flowrt::Status::Error => return flowrt::TaskRunOutcome::error(Vec::new()),
            }
    ...
    pub fn run(self, backend: &dyn flowrt::Backend, run_ticks: Option<usize>) -> flowrt::Status {
        ...
        if status == flowrt::Status::Ok {
            status = app.plan_svc.as_ref().as_ref().on_init(&mut lifecycle_context);
        ...
        if status == flowrt::Status::Ok && plan_svc_initialized {
            status = app.plan_svc.as_ref().as_ref().on_start(&mut lifecycle_context);
                    ...
                    let submitted = match admission.task {
                        ...
                        flowrt::TaskId(3) => {
                            ...
                            let task_outcome = 'flowrt_task: {
                                ...
                                let handled = match __flowrt_service_server.lock().unwrap_or_else(|poisoned| poisoned.into_inner()).poll_requests(|request| __flowrt_service_component_plan_svc.as_ref().as_ref().on_plan_request(&request)) {
                                    Ok(handled) => handled,
...
late_drop_count: 0,
        ...
        if plan_svc_started {
            let stop_status = app.plan_svc.as_ref().as_ref().on_stop(&mut lifecycle_context);
        ...
        if plan_svc_initialized {
            let shutdown_status = app.plan_svc.as_ref().as_ref().on_shutdown(&mut lifecycle_context);
