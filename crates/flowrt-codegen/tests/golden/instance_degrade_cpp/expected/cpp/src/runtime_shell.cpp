...
App::App(
    std::unique_ptr<ProducerInterface> monitor,
    std::unique_ptr<SinkInterface> consumer
)
    : monitor_(std::move(monitor)),
      consumer_(std::move(consumer)),
      bind_0_(flowrt::LatestChannel<Sample>::with_stale_config(flowrt::StaleConfig{std::chrono::milliseconds{50}, flowrt::StalePolicy::HoldLast})) {
}
...
flowrt::Status App::step(std::size_t tick, flowrt::Context& tick_context, flowrt::IntrospectionState& introspection_state, flowrt::ScheduleWaiter& scheduler_events, std::map<std::string, flowrt::IntrospectionTaskHealth>& health_map) {
    ...
    (void)health_map;
    {
        health_map["monitor.main"].name = "monitor.main";
        health_map["monitor.main"].lane = "monitor_serial";
        flowrt::Output<Sample> monitor_sample;
        if (monitor_) {
            switch (monitor_->on_tick(monitor_sample)) {
        ...
        if (const auto* value = monitor_sample.as_ref()) {
            bind_0_.publish_at(*value, tick_time_ms);
            scheduler_events.notify_data();
            record_introspection_publish_copy(introspection_state, "monitor.sample_to_consumer.sample", "Sample", this->introspection_probe_bind_0, *value, tick_time_ms);
        }
    }
    {
...
FlowrtTaskOutcome App::step_task_monitor_main(std::size_t tick, flowrt::Context& tick_context, flowrt::IntrospectionState& introspection_state, flowrt::ScheduleWaiter& scheduler_events, std::map<std::string, flowrt::IntrospectionTaskHealth>& health_map) {
    const auto tick_time_ms = static_cast<std::uint64_t>(tick);
    (void)tick_time_ms;
    (void)tick_context;
    (void)introspection_state;
    ...
    std::vector<FlowrtOutputCommit> flowrt_output_commits;
    {
        ...
        if (const auto* value = monitor_sample.as_ref()) {
            ...
            scheduler_events.notify_data();
            record_introspection_publish_copy(introspection_state, "monitor.sample_to_consumer.sample", "Sample", app.introspection_probe_bind_0, *value, tick_time_ms);
                return flowrt::Status::Ok;
            });
        }
    }
...
FlowrtTaskOutcome App::step_process_main_task_monitor_main(std::size_t tick, flowrt::Context& tick_context, flowrt::IntrospectionState& introspection_state, flowrt::ScheduleWaiter& scheduler_events, std::map<std::string, flowrt::IntrospectionTaskHealth>& health_map) {
    const auto tick_time_ms = static_cast<std::uint64_t>(tick);
    (void)tick_time_ms;
    (void)tick_context;
    (void)introspection_state;
...
flowrt::Status App::run(const flowrt::Backend& backend, std::optional<std::size_t> run_ticks) {
    ...
    this->introspection_probe_bind_0 = register_introspection_channel(introspection_state, "monitor.sample_to_consumer.sample", "Sample", std::optional<std::size_t>{4});
    auto introspection_server = flowrt::spawn_status_server(
        flowrt::IntrospectionIdentity{
            ...
            .package = "instance_degrade_cpp_demo",
            .process = "main",
            .runtime = "cpp",
        },
        introspection_state);
    ...
    if (status == flowrt::Status::Ok && monitor_) {
        status = monitor_->on_init(lifecycle_context);
        monitor_initialized = status == flowrt::Status::Ok;
        introspection_state.record_lifecycle_state("monitor", monitor_initialized ? flowrt::LifecycleState::Initialized : flowrt::LifecycleState::Faulted);
    }
    ...
    if (status == flowrt::Status::Ok && monitor_initialized && monitor_) {
        status = monitor_->on_start(lifecycle_context);
        monitor_started = status == flowrt::Status::Ok;
        introspection_state.record_lifecycle_state("monitor", monitor_started ? flowrt::LifecycleState::Running : flowrt::LifecycleState::Faulted);
    }
    ...
    scheduler.add_lane(flowrt::LaneId{flowrt::fnv1a64("monitor_serial")}, flowrt::LaneKind::Serial);
    (void)"monitor_serial";
    scheduler.add_task(flowrt::TaskSpec{.id = flowrt::TaskId{1}, .lane = flowrt::LaneId{flowrt::fnv1a64("consumer_serial")}, .priority = 0});
    scheduler.add_task(flowrt::TaskSpec{.id = flowrt::TaskId{2}, .lane = flowrt::LaneId{flowrt::fnv1a64("monitor_serial")}, .priority = 0});
    ...
    while (status == flowrt::Status::Ok && !shutdown.is_requested() && ((!run_ticks.has_value() || tick_base < *run_ticks) || !pending_task_order.empty())) {
        ...
        {
            auto& health = health_map["monitor.main"];
            health.name = "monitor.main";
            health.lane = "monitor_serial";
        }
                ...
                const auto submitted = worker_pool.submit_collect(admission.task, task_completion_queue, [this, &introspection_state, &scheduler_events, &task_health_mutex, &task_health_from_workers, admission, scheduled_delta_ms, observed_delta_ms, task_clock_source, tick_base, tick_time_ms]() {
                    ...
                    const auto [task_name, task_trigger] = [&]() -> std::pair<std::string_view, std::string_view> {
                        switch (admission.task.value) {
                            ...
                            case 2: return {"monitor.main", "periodic"};
...
return task_outcome;
                    ...
                    case 2: {
auto flowrt_lane_guard = flowrt::enter_lane(flowrt::LaneId{flowrt::fnv1a64("monitor_serial")});
(void)flowrt_lane_guard;
auto task_outcome = step_task_monitor_main(static_cast<std::size_t>(tick_time_ms), local_context, introspection_state, scheduler_events, local_health_map);
merge_local_health(std::move(local_health_map));
return task_outcome;
}
...
break;
                ...
                if (task_result.status == flowrt::Status::Error) {
                    switch (task_result.task.value) {
                        ...
                        {
                            if (!monitor_degraded) {
                                introspection_state.record_lifecycle_state("monitor", flowrt::LifecycleState::Degraded);
                ...
                if (task_result.status == flowrt::Status::Ok) {
                    switch (task_result.task.value) {
                        ...
                        {
                            if (monitor_degraded) {
                                introspection_state.record_lifecycle_state("monitor", flowrt::LifecycleState::Running);
        ...
        if (scheduler.lane_starvation_ticks(flowrt::LaneId{flowrt::fnv1a64("monitor_serial")}) > fairness_starvation_threshold) {
            for (auto &[name, health] : health_map) {
    ...
    if (monitor_started && monitor_) {
        const auto stop_status = monitor_->on_stop(lifecycle_context);
        if (status == flowrt::Status::Ok && stop_status != flowrt::Status::Ok) {
            status = flowrt::Status::Error;
        }
        introspection_state.record_lifecycle_state("monitor", stop_status == flowrt::Status::Ok ? flowrt::LifecycleState::Stopped : flowrt::LifecycleState::Faulted);
    }
    ...
    if (monitor_initialized && monitor_) {
        const auto shutdown_status = monitor_->on_shutdown(lifecycle_context);
        if (status == flowrt::Status::Ok && shutdown_status != flowrt::Status::Ok) {
            status = flowrt::Status::Error;
        }
        introspection_state.record_lifecycle_state("monitor", shutdown_status == flowrt::Status::Ok ? flowrt::LifecycleState::ShutDown : flowrt::LifecycleState::Faulted);
    }
    return status;
}
...
auto flowrt_lane_guard = flowrt::enter_lane(flowrt::LaneId{flowrt::fnv1a64("monitor_serial")});
(void)flowrt_lane_guard;
auto task_outcome = step_process_main_task_monitor_main(static_cast<std::size_t>(tick_time_ms), local_context, introspection_state, scheduler_events, local_health_map);
merge_local_health(std::move(local_health_map));
return task_outcome;
}
                    default: return FlowrtTaskOutcome::error(std::vector<FlowrtOutputCommit>{});
