...
fn emit_cpp_zenoh_service_endpoints(
    ...
    for plan in &zenoh_plans {
        ...
        if active.contains(plan.client_instance.as_str()) {
            ...
            output.push_str(&format!(
                "        this->{client_field}_.bind(flowrt::zenoh::ZenohServiceClient<{req_ty}, {resp_ty}>::open(\n            {name}\n#ifdef FLOWRT_HAS_ZENOH_CXX\n            , zenoh_service_session\n#endif\n            ));\n",
            ));
        }
...
fn emit_cpp_iox2_service_endpoints(
    ...
    for plan in &iox2_plans {
        ...
        if active.contains(plan.client_instance.as_str()) {
            ...
            output.push_str(&format!(
                "        this->{client_field}_.bind({client_ty}::open_shared({name}));\n",
            ));
        }
        if active.contains(plan.server_instance.as_str()) {
            let server_field = cpp_service_server_field_name(plan);
...
fn emit_cpp_iox2_operation_endpoints(
    ...
    for plan in iox2_plans {
        ...
        if active.contains(plan.client_instance.as_str()) {
            ...
            output.push_str(&format!(
                "        this->{client_field}_.bind(\n            {start_client_ty}::open_shared({start_name}),\n            flowrt::iox2::Iox2ServiceClient<flowrt::OperationId, flowrt::OperationStatusSnapshot>::open_shared({cancel_name}),\n            flowrt::iox2::Iox2ServiceClient<flowrt::OperationId, flowrt::OperationStatusSnapshot>::open_shared({status_name}));\n",
            ));
        }
        if active.contains(plan.server_instance.as_str()) {
            let start_server = cpp_operation_start_server_field_name(plan);
