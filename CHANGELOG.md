...
- 增加 Rust runtime 最小 introspection Unix socket 协议和 `flowrt status` live discovery：CLI 会扫描当前用户 socket 目录，连接后读取 handshake 与 tick/channel 摘要，socket 文件名只作为发现入口。
