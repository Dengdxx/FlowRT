...
- 新增本机 `flowrt op start` 控制面：CLI 可用 self-description Message ABI 把
  Operation goal JSON 编码为 wire payload，经 introspection socket 启动 generated Rust
  Operation，并返回 accepted `operation_id` 与 live status。
- 新增本机 `OperationStatus` introspection 请求：`flowrt op status <operation_id>` 可按
  invocation id 精确查询 generated Rust Operation live status，并与 `start/cancel` 使用一致
  的 `operation_id=...` 输出前缀。
- 新增远程 zenoh Operation 控制面基座：generated Rust shell 会暴露
  ...
  发现或用 `--runtime` 精确选择远端 runtime。
- 远程 `flowrt op start <operation> --remote` 复用 self-description Message ABI 编码
  goal JSON，并经 zenoh Operation control-plane 启动远端 generated Rust Operation。
- 新增 `flowrt op result <operation_id>`：generated Rust Operation 成功返回值会编码为
...
- 新增 `flowrt op follow <operation_id>` 和 `flowrt op start --follow`：runtime 通过
  `OperationObserve` 保留 state/progress/result event，generated Rust Operation 会把
