...
- FlowRT core skills 套组：`.agents/skills/` 当前只落地 `frt-core-*`，先服务 FlowRT 仓库维护者；首批包含 change intake、RSDL/IR、codegen、runtime parity 和 backend 五个 P0 skill。`frt-app-*` 暂作为 1.0.0 之后的保留命名空间，`write-frt-skill` 元技能负责约束命名、触发条件、硬门控、验证证据和 FlowRT 专有术语。
