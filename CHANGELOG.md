...
- v0.25.0 iox2 focused CI 在 amd64 / arm64 runner 上显式安装
  `x86_64-unknown-linux-gnu` Rust target，保证示例声明 `linux-amd64` target 时 arm64
  runner 也能执行真实 SDK build/run 路径。
