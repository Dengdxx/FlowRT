...
- 生成的 Rust runtime shell 改为启动基于 `IntrospectionState` 的 status server，并在 scheduler tick 入口记录 live tick 计数；Rust channel 发布统计与 payload snapshot 已接入，C++ runtime parity 仍保留为后续切片。
