...
- 新增远端托管运行闭环：`flowrt deploy --activate --start` 会把 bundle 上传到
  `<remote-dir>/incoming/<release-id>`，在目标机安装进 managed release store，原子更新
  active release，并可启动 generated supervisor。
- 新增公开 `flowrt remote activate/start/stop/status/logs/rollback` 命令，通过 SSH 调用
  目标机本地 managed runtime，支持查看 active/run/release 状态、读取 supervisor 日志、
  graceful stop 和 `rollback --start`。
- 新增隐藏目标机本地 `flowrt managed ...` 命令，维护
  `releases/<release-id>/`、`state/active.json`、`state/run.json` 和
  `logs/<run-id>/supervisor.log`；release id 由 bundle 内容和 artifact hash 派生。

### 变更

- `examples/workspace_demo` 迁移为 module-local 用户目录示例：Rust `perception` 实现位于
  `app/perception/rust/processor.rs`，C++ `control` 实现位于
  `app/control/cpp/src/processor.cpp`，C++ headers 位于 `app/control/cpp/inc/`；示例 contract
  改为 Rust/C++ process 分离并通过 `iox2` backend 连接，避免继续展示旧的 root-only
  用户代码组织。

### 测试

- 新增 `scripts/test-v0290-remote-managed-runtime-smoke.sh`，复制 `workspace_demo` 到临时目录，
  验证 App API、reference stub、generated CMake 的 module-aware 路径，并执行 `deps` 与
  `build --launcher`；同一 smoke 还用伪 bundle 覆盖 managed install、start、status、logs、
...
- App API manifest、`implementation.md` 和 reference stubs 现在按 workspace module 暴露用户侧
  组织路径：Rust module component 使用 `app/<module>/rust/<component>.rs`，C++ module
  component 使用 `app/<module>/cpp/src/<component>.cpp`，C module component 使用
  `app/<module>/c/<component>.c`；对应 C++ 参考 stub 生成到
  `flowrt/app/stubs/<module>/cpp/src/<component>.cpp`。root component 保持既有
  `app/rust/mod.rs`、`app/cpp/**` 和 `app/c/**` 路径。

### 变更

- generated CMake 的 C/C++ 用户代码自动发现范围扩展到 `app/<module>/cpp/src/**` 和
  `app/<module>/c/**`，并把 `app/<module>/cpp/inc` 加入 C++ include 路径；Rust 仍由
  `app/rust/mod.rs` 作为 graph 级入口聚合 module-local 实现。

### 测试

- 新增 `scripts/test-v0280-module-app-layout-smoke.sh`，覆盖 module-aware App API、reference stub
  路径、用户 `app/` 不被 `prepare` 覆盖，以及 generated CMake 对 module C/C++ 用户目录的
  自动发现。
...
- `flowrt replay` 新增 `--verify-operation-observations` 和 `--format json`：MCAP Operation
  replay 仍只重放 start/cancel command，验证模式会读取录制中的 state/progress/result/error
  observation evidence，并在回放后用 recorded id 到 replay id 的映射比较 Operation 名称、
  event sequence、progress sequence、终态/error 和 typed payload bytes；missing、extra 或
  mismatch 会返回非零并输出结构化 diff。
- C ABI 升级到 `0.3`，C component callback ABI 升级到 `0.4`：callback context 在保留
  `flowrt_c_param_snapshot_v0_t` JSON snapshot 前缀的同时新增
  `flowrt_c_param_snapshot_v1_t`，为 bool、整数、浮点和 string 参数提供 typed readonly
  value view；string 参数以 callback 生命周期内有效的 borrowed UTF-8 slice 暴露给 C callback。

### 变更

- 反馈回边 `init` 从 primitive-only 扩展为 fixed-size plain data 的 typed literal：
  struct 字段按零值 seed 做 sparse overlay，嵌套 struct 递归套用同一规则；fixed array
  一旦出现必须完整给出；variable frame 字段继续 fail-fast，避免把不可定长 payload
  混入反馈初值播种。
- route transport error 新增 Rust/C++ 对齐的 typed kind：`flowrt status --format json`
  会暴露 `last_error_kind`，`QueueFull` 才按 route overflow policy 投影到
  drop/backpressure/overflow counters，`Unknown` 等粗粒度 backend error 只记录错误和
  backend health。
- runtime recorder 的 Operation progress/result/error observation JSON 会保留 typed payload
  byte array 和 `payload_len`，不再只能通过长度判断 observation evidence。
- C adapter 的 readonly params snapshot v0 对 string 参数保留 JSON 字符串表达，array/table
  继续以 JSON fragment 暴露；v1 typed view 不新增 C 侧参数写接口、service、operation、
  variable frame 或动态 runtime。

### 修复

- 清理 generated C++ 中 `clang-tidy` 会稳定发现的冗余默认初始化：空 channel 构造成员不再进入
  `App` 构造初始化列表，`std::string` / `std::vector` variable frame 字段不再生成冗余
  default member initializer，保留 plain-data 字段的零初始化语义。

### 测试

- 新增 `scripts/evidence-matrix.toml` 和 `scripts/check-evidence-matrix.sh`，把 generated
  runtime shell golden、真编译和代表性 C++ static quality case 收敛到同一证据矩阵事实源。
- `scripts/test-codegen-compile.sh` 改为从 evidence matrix 读取 `syntax_compile` case，
  `scripts/check-codegen-compile-coverage.sh` 作为兼容入口委托矩阵检查，避免脚本内维护第二份
  compile case 列表。
- `scripts/test-codegen-compile.sh` 的 Rust generated case 改为先带默认 5 次外层重试执行 `cargo fetch`，
  再用 `cargo check --locked --offline` 做编译证明，降低 crates.io TLS 瞬断导致 focused gate
  误挂的概率；v0.27.1 focused gate 同时自测重试 helper 在失败路径保留真实退出码。
- 新增 `scripts/test-cpp-static-quality.sh`，按 runtime、generated 和 ABI/POD 三类 profile
  运行 C++ 静态质量门禁；历史 `v0.23.2 C++ clang-tidy` smoke 改为委托该长期门禁。
- 扩展 `feedback_v2_{rust,cpp}` golden 与 validator 回归，覆盖嵌套 struct sparse overlay、
  fixed array 完整初始化、未知字段、数组长度错误和 variable frame init 拒绝；生成物编译网
  真编译 Rust/C++ 两侧 feedback v2 shell。
- `v0.21.4` 跨进程 feedback smoke 改为按函数体检查 source process 对 feedback route 的
  sparse overlay seed publish，避免继续绑定旧的 struct literal 文本。
- 增加 Rust/C++ route health 回归，锁定 `QueueFull` 与 `Unknown` transport error 的
  counter 差异，并覆盖 CLI route 行 `last_error_kind` 展示。
- 增加 Operation observation record reader、recorder payload 和 `flowrt replay` verification
  回归，覆盖 replay id 映射、payload mismatch 结构化 diff，以及缺少 observation stream 的
  fail-fast。
- 增加 C ABI v1 params layout、C string param validator 和 C adapter codegen 回归，锁定 v0
  snapshot 前缀兼容与 v1 typed string borrowed view。

## v0.27.0 - 2026-06-24

### 新增

- 新增本机 `flowrt op start` 控制面：CLI 可用 self-description Message ABI 把
  Operation goal JSON 编码为 wire payload，经 introspection socket 启动 generated Rust/C++
  Operation，并返回 accepted `operation_id` 与 live status。
- 新增本机 `OperationStatus` introspection 请求：`flowrt op status <operation_id>` 可按
  invocation id 精确查询 generated Rust/C++ Operation live status，并与 `start/cancel` 使用一致
  的 `operation_id=...` 输出前缀。
- 新增远程 zenoh Operation 控制面基座：generated Rust/C++ shell 会暴露
  `flowrt/op/<package>/<selfdesc_hash>/<pid>` queryable，`flowrt op status <operation_id>
  --remote` 与 `flowrt op cancel <operation_id> --remote` 可复用 self-description hash
  发现或用 `--runtime` 精确选择远端 runtime；C++ inproc/iox2 app 会 opportunistic 查找
  `zenohcxx::zenohc`，缺少 SDK 时本机 Operation 控制面仍可用。
- 远程 `flowrt op start <operation> --remote` 复用 self-description Message ABI 编码
  goal JSON，并经 zenoh Operation control-plane 启动远端 generated Rust/C++ Operation。
- 新增 `flowrt op result <operation_id>`：generated Rust/C++ Operation 成功返回值会编码为
  Message ABI payload 并保留在 introspection result 中，CLI 可按 self-description 解码为
  JSON；本机 socket 与远程 zenoh Operation control-plane 均支持查询。
- 新增 `flowrt op follow <operation_id>` 和 `flowrt op start --follow`：runtime 通过
  `OperationObserve` 保留 state/progress/result event，generated Rust/C++ Operation 会把
  typed progress feedback 编码为 Message ABI payload，CLI 可按 self-description 解码后持续
  输出到 terminal；本机 socket 与远程 zenoh Operation control-plane 均支持。
- C++ Operation runtime 与 generated shell 补齐 typed payload parity：`OperationProgressPublisher`
  会把 feedback 编码为 canonical Message ABI payload，`OperationHandlerResult::Succeeded`
  的 result value 会进入 runtime result event，C++ generated introspection drain 使用
  payload-aware progress/result 记录入口，不再只保留 sequence 或终态字符串。
- C++ 本机 introspection socket 补齐 Operation typed control-plane parity：支持
  `OperationStart`、`OperationStatus`、`OperationResult` 和 `OperationObserve` 请求，
  generated C++ shell 会注册 start/status/cancel hook，result payload 与 observation event
  page 可通过 `flowrt op result/follow` 查询。
- C++ `ZenohOperationServer` 增加 shared-session `open(...)` 入口，便于 runtime smoke 和
  嵌入式 host 在已有 zenoh session 上注册 remote Operation queryable。
- `flowrt op list/status/start/cancel/result/follow` 新增 `--format json` 输出模式；JSON
  schema 以 Operation 语义对象为主，result/follow 会同时保留原始 payload byte array 与按
  self-description Message ABI 解码后的 `value`。
- Operation start/cancel accepted 后会记录可重放 command event；`flowrt replay --file
  <recording.mcap>` 可读取 `operation.command.start/cancel` 时间线，按记录顺序重新驱动
  `OperationStart` / `OperationCancel`，并把录制 invocation id 映射到回放运行的新 id。
...
- 修复 Rust generated self-description 在 JSON 包含非 ASCII 诊断时使用 raw byte string
  ...
  不变。
- 修复 module-qualified 消息类型在 generated self-description 和 ABI 测试中的 identity
  漂移：`message_abi` / `message_frames` 现在统一使用 Contract IR canonical type id
  作为协议和 CLI 主键，C++/Rust 生成名只用于语言 binding；boundary endpoint 与 dataflow
  channel 暴露的类型都会有可匹配的 ABI 或 frame layout registry entry。
- 修复 C++/Rust generated Message ABI 测试对 module-qualified 消息类型生成 roundtrip
  测试却漏生成 `sample_*` fixture 的问题；测试和 fixture 现在使用同一套 fixed ABI inclusion
  规则，避免完整 `flowrt build` 在 `message_abi.cpp` 编译阶段失败。
- 修复 Operation result retention 未完整驱动 generated runtime 与 introspection 清理的问题：
  `result_retention_ms = 0` 不再保留已完成 invocation，retention 过期后 `op result` /
  `op follow` 会按 id 返回 not found，而不是继续暴露旧 result 或 event log。
- 修复远程 zenoh Operation control-plane 不支持命令的错误提示仍只列出 `status/cancel`
  的旧文案；现在提示会列出完整 JSON command family。
- 修复 `flowrt op result/follow --format json` 只按 fixed Message ABI 解码 typed payload 的问题；
  Operation progress/result payload 为 canonical frame (`message_frames`) 时也会解码出 `value`。
- 修复 `flowrt op start/result/follow` 对 bounded canonical frame 类型表达式支持不完整的问题；
  `string<max=N>`、`bytes<max=N>` 和 `sequence<T,max=N>` 会按 generated self-description 正常编码或解码。
- 修复 `flowrt echo` 的 variable frame formatter 仍拒绝 `sequence<T,max=N>`、`string<max=N>`
  和 `bytes<max=N>` 的问题；bounded frame 输出现在与 generated self-description 对齐。
- 修复 `flowrt fault-matrix run` 生成 replay source 时对 boundary input 写空 payload 的问题；
  现在会按 Contract IR 的 canonical frame layout 写入可解码的默认零值 payload，覆盖 fixed、
  padding fixed 和 empty variable frame。
- 修复 `flowrt fault-matrix run` 生成 replay source 时只驱动第一个 boundary input 的问题；
  多 boundary input case 现在会为每个 input boundary 写入默认激励事件。
- 修复 `flowrt fault-matrix run --report` 在 expectation 失败或单个 case 执行失败时提前返回
  导致报告未写出的问题；现在会先保留完整 case JSON report，再按失败 case 返回非零退出。
- 修复 bounded `run_ticks` 场景下 task 在 tick 预算尾部完成并触发 restart 时，generated Rust/C++
  scheduler 可能在观察到 pending restart 前退出，导致 fault matrix final status 漏记
  `restart_count` 的问题。
- 修复 `global_tick` 外部 tick generated Rust shell 在 fault matrix 等 simulated replay 路径中
  仍生成 unused `scheduler_now_ms` / `observed_data_generation` warning 的问题。
- 修复 `flowrt op start --follow` 的 follow 阶段观察请求超时不可独立配置的问题；新增
  `--follow-timeout-ms` 控制 `OperationObserve` 轮询超时，`--timeout-ms` 继续只表示
  Operation start timeout override，且未传 `--follow` 时会拒绝单独使用该参数。
- 修复 `scripts/package-deb.sh` 在 zenoh Debian 包下载失败时继续把空路径传给
  `dpkg-deb` 的问题；下载或 sha256 校验失败现在会直接中止在真实失败点。
- 修复 `flowrt launch` 拒收带 Service / Operation control-plane frame metadata 的
  `launch.json` 的问题；supervisor manifest parser 现在接受 codegen 已输出的
  `backend_source` 与 canonical frame 诊断字段。
- 修复仓库开发模式下 generated Rust app 构建没有复用根 `Cargo.lock` 的问题；repo runtime
  fallback 现在会把已验证的依赖锁写入 generated build workspace，避免真实 `iox2` app
  fresh resolve 到未经验证的 `iceoryx2` 版本。
- 修复 bounded variable frame generated `message_frame` fixture 仍可能生成超出 `max=N` 的样例值
  的问题；`bytes<max=N>`、`string<max=N>` 和 `sequence<T,max=N>` 的 Rust/C++ fixture 现在会按
  字段上界收缩样例，避免真实 C++ bounded iox2 Operation build/run 在测试初始化阶段失败。
- 修复 `flowrt launch --run-steps` 在子进程已达到 tick 上限后立即终止进程的问题；supervisor
  现在会给有界运行一个短暂自然退出窗口，使 generated child 有机会完成 shutdown/status 写出。
- 修复 Rust generated shell 在无 dataflow bind、但组件 output 使用 named message 类型时漏导入
  `crate::messages::*` 的问题；`profile_switch_demo --profile iox2` 这类 output-only 示例现在可
  正常完成 release build。

### 测试

- 清理 generated Rust shell 在 `-D warnings` 下暴露的 unused/dead-code/import/parens 告警：
  feedback seed 显式忽略初始化 publish 返回值，Service / Operation hidden task 不再保留
  未调用 helper，纯 zenoh Service shell 不再导入未用消息模块，参考 Rust stub 对未用
  Service / Operation / params 参数加 `_` 前缀。
...
- C++ zenoh runtime smoke 覆盖 remote Operation queryable 的真实 SDK path：同一 zenoh
  session 内发送 `status`、`operation_start`、`operation_status`、`operation_cancel`、
  `operation_result` 和 `operation_observe` query 并验证 JSON response。
- `flowrt-cli` 增加远程 Operation JSON 输出回归测试：用显式 loopback zenoh
  client/server session 覆盖 `start/status/cancel/result/follow`，验证 runtime target 与
  typed progress/result payload 解码。
- Rust zenoh pub/sub peer restart smoke 改用显式 loopback `listen/connect` 配置，避免依赖
  本机 multicast discovery 或外部 `zenohd`。
- C++ iox2 runtime smoke 在真实 `iceoryx2-cxx` SDK path 下补充
  `Iox2FramePubSub` 和 `Iox2FrameServiceClient` / `Iox2FrameServiceServer` roundtrip，验证
  bounded variable frame dataflow 与 service slot 不只停留在 slot codec 和 generated golden
  证据。
- `v0.25.0 Iox2 Service Operation Smoke` 的真实 SDK 分支改为 `flowrt build --launcher`
  后启动 generated supervisor，并通过 `flowrt status --live-only --format json` 轮询
  Operation live counter；该分支同时覆盖 `iox2_service_demo` 的 Rust iox2 Operation 真实运行，
  以及 generated C++ bounded Operation start frame 经 iox2 定容 slot 的 build/run 路径。
- Release Candidate 中的 `v0.25.0 Iox2 Service Operation Smoke` 会先按 `scripts/deps.lock`
  准备 locked `iceoryx2-cxx` C++ SDK prefix，再运行真实 SDK 分支，避免 generated C++
  bounded Operation 构建在 package job 之前找不到 `iceoryx2-cxx`。
- Release Candidate 和 `scripts/package-deb.sh` 为 Cargo 网络访问设置重试、超时和禁用 HTTP/2
  multiplexing 的默认值；Debian 打包入口会先 `cargo fetch --locked` 并对 fetch/vendor 做外层
  重试，降低 arm64 原生 runner 首次 cache miss 时 crates.io HTTP/2 抖动导致 deb job 失败的概率。
- 新增 `v0.27.0 Operation Control Plane Smoke` focused gate，覆盖本机/远程 Operation CLI、
  bounded variable frame payload、fault matrix 多 boundary input、self-description canonical
...
- release readiness 的 v0.25.2 文档检查不再引用本地 backend 设计草案，改为校验入库的
  `CONTEXT.md` 与 `docs/cli.md` 事实源，避免 CI 环境缺少本地草案时误失败。
...
- 有界变长类型 `bytes<max=N>`、`string<max=N>` 和 `sequence<T,max=N>` 现在可以在
  dataflow、Service request/response 和 Operation start goal 中继续选择 `iox2`：
  generated Rust/C++ shell 使用 `Iox2FramePubSub` / `Iox2FrameServiceClient` /
  `Iox2FrameServiceServer` 的定容 canonical frame slot 承载，用户 API 仍保持
  `Vec` / `String` / `std::vector` / `std::string` 形态。

### 变更

- dataflow、Service 和 Operation 统一按 edge 解析 backend：省略 `backend` 继承
  profile/default backend；显式 `auto` 继续按 topology 解析。
- `iox2` 长期边界收窄为“不承载无界变长或指针所有权 payload”：fixed-size plain data
  和可推导 frame 上界的 bounded variable frame 都可以走 `iox2`，超出字段上界时返回
  包含字段名与上界的错误，不截断。
- profile/default backend 为 `iox2` 且 edge payload 含无界变长字段时，未显式 backend 会按
  该 edge 自动 fallback 到 `zenoh`；显式 `backend = "iox2"` 只在无界变长 payload 上被
  validator fail-fast 拒绝。有界 `max=N` payload 不再触发 fallback。
- Contract IR capability 推导使用 `abi:variable_payload_frame`，并通过
  `allocation:bounded` 与 `allocation:unbounded_dynamic` 区分有界和无界；`iox2` 提供
  variable frame 编码能力但不提供无界动态分配能力。
- self-description、launch manifest 和 CLI 摘要会暴露 variable frame 的 `bounded`、
  `max_size_bytes`、`iox2_slot_cap_bytes`、`backend_source` 和 fallback 诊断；无界 payload
  从 profile `iox2` 落到 `zenoh` 时会提示“加 max=N 可留在 iox2”。
- Service / Operation 省略 backend 不再表示无条件 topology auto，而是与 dataflow route 一样
  继承 profile/default backend。
- 日常 `ci.yml` 拆为快速 push/PR 验证，只保留生成物保护、Rust fmt/test/clippy、C++
  runtime 和 codegen 编译网；完整历史矩阵、deb package 和 `Release Evidence Gate`
  迁移到仅由 `dev/v*` push 触发的 `release-candidate.yml`。

### 修复

- 修复 Rust generated shell 在有界 variable frame over iox2 的 Service / Operation
  start endpoint 上生成 `Iox2FrameService*<...>::open` 的语法问题；现在 open /
  unavailable 调用使用 Rust turbofish，真实 `iox2_service_demo` build/run 可通过。
- v0.25.0 iox2 focused CI 的真实 SDK build/run 子步按 runner 架构执行：amd64
  runner 保留入库示例的 `linux-amd64` target，arm64 runner 将临时示例 workspace 改为
  `linux-arm64` target，避免交叉构建出的 amd64 二进制在 arm64 host 上被直接运行。
...
- focused smoke 与 codegen golden 增加无界 `bytes` 到 `bytes<max=N>` 的 route 切换覆盖，
  验证用户代码不变、无界走 `zenoh`、有界走 `iox2` slot。

## v0.24.0 - 2026-06-19

### 新增

- 新增 test-only `flowrt fault-matrix check/run`：矩阵文件可按 case 描述故障注入点、
  运行模式和 graph / instance / route / failover 期望；`check` 只解析、投影并运行
  Contract IR validator，`run` 逐 case 生成 test-only artifact、运行并输出 JSON report。
- generated Rust/C++ app 和 generated supervisor 支持 `FLOWRT_STATUS_OUT` final status
  snapshot；matrix runner 在进程退出后读取该证据，不依赖 live socket。
- 新增 `examples/fault_matrix_demo`，覆盖 restart、backend drop route health 和 standby
  failover 三个矩阵 case。

### 变更

- fault matrix launch mode 会为 case 复制项目 `app/` 用户实现并注入 replay source，保证
  simulated replay 时钟和 generated shell 的用户代码路径都能在隔离 case 目录中运行。

### 修复

- 修复仓库开发模式下 `fault-matrix run` 的 generated Cargo build 未进入离线模式的问题，
  避免 v0.24.0 focused smoke 在已预热依赖后仍访问 crates.io。
- 放宽 `fault_matrix_demo` 的矩阵运行 tick 预算，避免 restart final status 尚未写出时
  就断言 `restart_count`，导致 v0.24.0 focused smoke 在 CI 上偶发失败。

### 测试

- 新增 `v0.24.0 Fault Matrix Smoke` focused gate，覆盖 CLI parser/check/evaluator、
  final status snapshot codegen、supervisor snapshot aggregation 和 fault matrix demo run。
...
- Rust runtime 新增 additive tracing exporter API：`TracingExporterConfig`、`FlowrtSpan` 和
  `FlowrtSpanSink` 可从 introspection status 派生 `process/task/service/operation/route/failover/global_tick`
  spans；默认关闭，sink 失败只生成 tracing diagnostic，不改变 runtime 状态。
- `observability.trace` resource 已满足时，codegen 会在 launch manifest 和 self-description
  的 graph section 输出 `tracing` 配置；未声明或未满足时不生成 tracing section。
- C v0 component 支持 params readonly snapshot：callback context 新增
  `flowrt_c_param_snapshot_v0_t`，primitive/array/table 参数以 borrowed JSON view 暴露给
  C callback；string param、service、operation、variable frame、`io_boundary` 和 `external`
  继续由 validator fail-fast。

### 变更

- 开启 `v0.23.3 既有缺口收束`：本版本集中处理当前已确认缺口和 defer，并明确不纳入
...
- 修复 C++ runtime Operation health counter 的冗余默认成员初始化，保持 v0.23.2
  `clang-tidy` focused gate 在 v0.23.3 发布分支继续通过。
...
- 新增 tracing exporter dry-run、失败诊断和 launch/self-description 投影测试，锁定默认关闭、
  additive export 和 `observability.trace` 守门行为。
- 新增 C params snapshot 的 ABI layout、validator 和 codegen 测试，并同步 C++ runtime
  static_assert / ctest 覆盖 callback context 布局。
- 新增 v0.23.3 focused smoke、release gate registry、readiness adapter 和 CI job，覆盖
  global tick、standby failover、Operation zenoh、FrameDescriptor payload、tracing exporter
  与 C v0 params snapshot 的发布前回归。
