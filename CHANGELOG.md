...
- 为后续 `0.14.1` 架构修缮新增 `architecture guard`：检查 tracked Rust、C/C++ 和
  shell 源文件单文件行数阈值，并用带中文理由、上限和目标版本的 legacy allowlist 管住
  当前待拆大文件。
...
- 明确 `0.14.1` 当前定位为架构修缮版本，重点收敛 `SchedulerRuntimePlan`、大文件拆分和
  architecture guard，不提前修改 workspace 版本号或创建正式 `v0.14.1` 版本段。
