...
- 修复 generated supervisor 无法解析 launch manifest 中 `artifact.clock` 元数据的问题；
  `flowrt launch`、demo smoke 和 ROS2 bridge smoke 会继续保留严格未知字段校验。
...
- **v0.13.1：Scheduler dispatch hardening**。修复已确认的 scheduler 同步等待
  `ReadyBatch::run_collect` 导致长任务阻塞后续 tick 的问题。长期方向是把调度/admission
  与 task completion 解耦，引入 completion queue 或等价异步完成通知；不能回退到临时
  polling 或牺牲 deterministic commit order。
- **v0.14.0：Python 与更多语言入口**。在 v0.13.0 稳定后的 C ABI 和 App API
  manifest 基础上提供 Python binding / generator。Python 不直接接触 iox2、
  zenoh 或底层 runtime 对象，只通过 FlowRT 语义边界实现 component、参数、
  message view、boundary 和 operation 接入。
