...
- 增加 `zenoh` backend capability 骨架：Contract IR/backend catalog、validator、Rust runtime 和 C++ runtime 现在都认识 `zenoh`，并把它建模为支持 `topology:multi_process`、`topology:multi_host` 与 `transfer:copy` 的跨主机 backend；实际 zenoh transport endpoint、wire codec 和跨机示例仍在后续切片实现。
- 增加 `zenoh` 生成物契约基础：Rust/C++ runtime shell 现在会按 selected backend 选择 `zenoh_backend()`，`flowrt/launch/launch.json` 与静态 self-description 会为 zenoh channel 暴露 deterministic `key_expr`；channel transport 仍在后续切片接入。
- 增加 Rust canonical wire codec 基础：Rust runtime 暴露 `WireCodec` / `WireCodecError`，profile 选择 `zenoh` 时 Rust message codegen 会生成字段顺序、little-endian、无 padding 的 fixed-size wire codec，并用生成测试验证 wire payload 不包含 native struct padding；C++ wire codec 和真实 transport endpoint 仍在后续切片实现。
- 增加 C++ canonical wire codec 基础：C++ runtime 暴露 `WireCodecError`、`ensure_wire_size`、`write_wire_le` 和 `read_wire_le`，profile 选择 `zenoh` 时 C++ message codegen 会生成同一字段顺序、little-endian、无 padding 的 fixed-size wire codec，并由生成的 C++ Message ABI 测试验证可编译和 roundtrip。
