...
- C++ zenoh service request/response 运行时：`ZenohServiceClient<Req, Resp>` 和 `ZenohServiceServer<Req, Resp>` 与 Rust 同语义，通过 `shared_ptr<::zenoh::Session>` 共享 session。适配 zenoh-cpp 1.6+ API（callback-based queryable、`Reply::get_ok()`、`Session::get` with `GetOptions`）。
