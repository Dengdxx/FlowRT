...
- 增加 mixed runtime readiness 分类：CLI 会区分同进程 C++/Rust 混合、`inproc` 跨进程混合，以及 `iox2` 下仍缺 C++ transport/supervisor wiring 的真实缺口。
...
- 增加 C++/iox2 backend readiness 保护：含 C++ component 的 `iox2` contract 在 C++ iox2 runtime shell 完成前会拒绝 `build` / `run` / `launch`，避免误用 inproc C++ shell。
...
- 增加 C++ runtime 的 `flowrt::iox2::Iox2ChannelConfig` 和 `Iox2PubSub<T>` API 边界；C++ iox2 shell 生成现在会使用 canonical service name、bind-level channel/stale config、`receive_latest_at` 和 `publish_at`。当前 C++ transport 方法仍安全返回 `ChannelError::Transport`，CLI readiness guard 继续保留。
