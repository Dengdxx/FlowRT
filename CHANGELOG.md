...
- `v0.3.1` 计划承接编排与生态增强：新增系统编排结构，引入 workspace + module 机制。每个功能模块独立声明自己的 type 和 component，组合层统一描述 instance、bind 和跨模块装配关系，最终仍由一次 `flowrt build` 生成完整应用，避免大型机器人项目把感知、规划、控制、底盘等功能域全部挤在单个扁平 package 中。该版本还规划 supervisor 依赖顺序和故障传播、可配置 restart policy、Service 请求/响应语义切片、ROS2 bridge 类型或方向扩展、`flowrt pub`、录制回放、`flowrt watch`，以及 C/Python ABI 的边界准备。
