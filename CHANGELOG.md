...
- `v0.4.0` 目标定为 Service runtime 完整化：在 v0.3.1 已落地的 Service RSDL/IR/validator/manifest 语义切片之上，生成 Rust/C++ service client/server 用户 API，支持 `inproc` 与 `zenoh` request/response transport，request arrival 直接驱动 server，补齐 request id、correlation、timeout、server unavailable 和 structured error 语义，并把 service endpoints 与 health 纳入 self-description、`flowrt list` 和 `flowrt status`。`iox2` 在 v0.4.0 不作为 Service transport；它继续专注 fixed-size shared-memory dataflow，Service 跨进程默认走 `zenoh`。
- `v0.5.0` 规划 Operation（Action 上位替代）+ lifecycle/supervisor 深化。Operation 是 typed long-running command、generated state machine、explicit policy 和 observable handle，不复制 ROS2 Action；底层编译期 lower 成 Service + Channel，用户只看 Operation，调试时才展开底层拓扑。Operation policy 必须显式声明 concurrency、preempt、cancel、timeout 和 result retention，用户不得手写 start/cancel/result/progress 四套底层协议。
- `v0.6.0` 规划 record/replay + simulated clock + deterministic debug；`v0.7.0` 规划 external process / driver package 接入边界，主项目不做硬件 backend；`v0.8.0` 规划跨机器部署、交叉编译和多架构安装包；`v0.9.0` 规划 C/Python API 与 ROS2 互操作扩展；`v1.0.0` 规划 ABI/schema 稳定、兼容策略、故障注入和性能矩阵。
...
- 新增 `flowrt-selfdesc` workspace crate：抽取 CLI、codegen 和 runtime 共用的 self-description schema 类型、JSON/binary section 加载/校验、SHA-256 哈希和 Message ABI / variable frame 字段格式化，消除三处复制结构体的 drift 风格。
- `flowrt-codegen` 的 self-description 构建和序列化改为使用 `flowrt-selfdesc` 共享类型，移除私有 schema 定义。
- `flowrt-cli` 的 self-description 读取、echo payload 格式化改为使用 `flowrt-selfdesc`，移除私有 loader 和 ABI 格式化代码。
- 共享 schema 类型使用 `serde(default)` 兼容旧版 JSON 和未来扩展字段，未知字段在反序列化时被静默忽略。
- 预留 service/operation 观测扩展字段：`SelfDescriptionChannel` 增加 `service` 和 `key_expr` 可选字段，均用 `serde(default)` 保证前向兼容。

### 新增

- `flowrt-selfdesc` crate 包含 21 个单元测试：覆盖从 `selfdesc.json` 文件和 `.flowrt.selfdesc` 二进制 section 读取、fixed-size ABI 字段格式化（u32/f32/f64/bool/i8/固定数组）、variable frame 字段格式化（string/bytes/sequence）、未知字段兼容、协议版本不支持时加载、无效 JSON 报清晰错误、缺失文件报清晰错误和 SHA-256 哈希确定性。
