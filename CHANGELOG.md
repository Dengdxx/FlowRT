...
- 修复 Contract IR 派生元数据校验边界：显式 external dataflow `backend = "zenoh"` 保留 explicit backend source；validator 即使在 forged IR 缺少 target 时也会检查 selected backend capability；参数 enum choice 也必须满足声明的 min/max 约束。
