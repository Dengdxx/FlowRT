...
- `v0.3.1` 计划作为复杂系统结构能力的突破版本：新增 workspace / module / composition 机制和 module name resolver。每个功能模块只声明自己的 type 和 component，composition 层统一描述 instance、bind、bridge、profile、target 和跨模块装配关系；模块默认不声明 instance 或 bind，跨模块引用采用 `module::Name`，无冲突时可使用短名，最终仍由一次 `flowrt build` 生成完整应用，避免大型机器人项目把感知、规划、控制、底盘等功能域全部挤在单个扁平 package 中。该版本还规划 supervisor 编排增强，包括 process 依赖顺序、故障传播和可配置 restart policy；新增 Service 请求/响应语义切片；完成 C/Python ABI 边界准备。录制回放系统暂不实施，但相关 runtime、self-description 或 CLI 边界在实现上述功能时应预留接口，避免后续引入时破坏已发布契约。
