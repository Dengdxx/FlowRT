...
- Rust codegen：为 service client 端口生成 typed handle struct（`ServiceClient_{instance}_{port}`），暴露同步 `call()` 和非阻塞 `start_call()` 路径。
...
- Rust codegen：service server 组件使用 `Rc<RefCell<...>>` 存储，handler 闭包通过 `borrow_mut()` 调用组件方法。
...
- `examples/service_demo/`：service RSDL demo。
- `InprocServiceServer::pending_count()` 方法：返回排队中的 pending request 数量。
