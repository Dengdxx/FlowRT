...
- `flowrt add component --lang c` 生成的 C 骨架现在明确说明 input/output buffer 只在本次
  callback 调用期间有效，并在有 input port 时校验 input array view 形状。
- 发布文档中的版本示例改为占位变量，不再把历史版本号写成默认发布路径。

### 测试

- 补充 C adapter codegen 断言，覆盖 callback table 失败原因和 output slot 失败诊断。
- 补充 CLI 骨架测试，覆盖 C callback v0 的 borrowed view 注释、input view 校验和
  size/version/feature bit 字段。
