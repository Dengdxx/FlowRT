...
- **交叉编译与多架构发布**：当前 `v0.8.2` 已接通单架构安装包内的
  native target SDK 与 `linux-amd64 -> linux-arm64` 交叉编译基础；后续仍需补齐
  双架构完整 SDK 聚合、显式 sysroot / SDK overlay、板级私有依赖说明、更多 demo
  的真实 C++ cross build smoke，以及远端安装后的自动验证闭环。
...
- **语言边界与稳定性**：`v0.9.0` 之后应推进 C/Python API、C ABI 边界、生态互操作
  和长期兼容策略；`v1.0.0` 前必须冻结 ABI/schema 基线，并补齐故障注入、性能矩阵
  和版本兼容测试。
