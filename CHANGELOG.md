...
- 修缮 bounded variable frame backend 选择：`iox2` 不再声明或生成变长承载能力，profile 默认 backend 为 `iox2` 且 route 使用 bounded variable frame 时，Contract IR 会把该 route 自动选择到 `zenoh`；fixed-size route 仍继续走 `iox2`。
...
- 增加 bounded variable frame 主线：FlowRT 现在可以把 `bytes<max=N>`、`string<max=N>` 和 `sequence<T,max=N>` 生成成固定 header + 尾部变长区的 canonical frame codec，Rust/C++ runtime 与 codegen 都支持这套布局，并把它暴露给 `inproc` 和 `zenoh` 路径。
