...
- CI demo smoke 改为：C++ only demo 执行 build/run，mixed `imu_demo` 只执行 build，Rust-only `import_demo` 执行 run/launch，`mixed_iox2_demo` 执行 check，避免在基础 CI 中强制安装可选 `iceoryx2-cxx`。
