...
- `v0.14.0` 计划新增 task timing context，用户算法通过现有 `flowrt::Context`
...
- `v0.14.0` 调度主线将 scheduler admission 与长任务 completion 解耦，scheduler 线程
  继续负责 ready 判定、backend commit、introspection 和 deterministic output commit，
  worker 只运行用户 task。
...
- `v0.14.0` 明确不承诺硬实时，不做 sensor timestamp、sensor event-time、clock
