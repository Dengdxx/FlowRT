...
- 增加 C++ runtime introspection socket API 基础：支持与 Rust wire JSON 兼容的 `status`、`channel_snapshot`、未知 channel 结构化错误响应，以及 `$XDG_RUNTIME_DIR/flowrt/<pid>.sock` / `/tmp/flowrt.<uid>/<pid>.sock` 路径规则；当前尚未接入 generated C++ shell。
...
- 生成的 Rust runtime shell 改为启动基于 `IntrospectionState` 的 status server，并在 scheduler tick 入口记录 live tick 计数；Rust channel 发布统计与 payload snapshot 已接入，generated C++ shell 的 live state 接入仍保留为后续切片。
