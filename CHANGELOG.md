...
- simulated_replay 生成 C++ runtime shell 改为 runtime 原生回放：循环前从 `FLOWRT_REPLAY_SOURCE`
  装配 `ReplayDriver`（只含本图 boundary 激励），调度逐周期步进并在命中事件时经
  `publish_boundary_input` 进程内注入；不再经 introspection socket 由外部 wall-clock 节奏注入，
  也不读注入的 data_time。realtime 调度路径不变。
