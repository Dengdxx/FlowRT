...
- `v0.6.0` 的交付边界是 Operation + record-only 录制系统 + 时间事件模型基础。Operation 是 typed long-running command、generated state machine、explicit policy 和 observable handle；底层编译期 lower 成 Service + Channel，用户只看 Operation，调试时才展开底层拓扑。录制使用 MCAP 容器和 FlowRT record envelope，覆盖 channel、参数、service、operation、scheduler/time 和 runtime/process 事件；本版本只做 `flowrt record`，不做 `flowrt replay` 或 simulated clock 驱动执行。
