...
- 扩展 Debian 安装后用户项目 smoke：在本机存在 ROS2 Jazzy 和 `rmw_zenoh_cpp` 时，会用已打包的 `flowrt` 构建 `ros2_bridge_demo`，确认 adapter 链接 ROS2 `zenoh_cpp_vendor`，并通过 `ros2 topic echo /flowrt/text --once` 验证端到端桥接。
