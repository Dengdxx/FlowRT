...
- **C/Python API 与 SDK 化**：`v1.0.0` 继续围绕 FlowRT 自身语义推进 C ABI、Python
  binding、SDK packaging 和生态互操作扩展；Python 不进入实时热路径，优先服务工具、
  测试、迁移和非实时控制面。
...
- **语言边界与稳定性**：ABI/schema 冻结、长期兼容策略、故障注入、性能矩阵和版本
  兼容测试顺延到 `v1.1.0`。
