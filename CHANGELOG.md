...
- 将 GitHub Actions CI 从单个串行 Linux job 拆成 `guard-generated`、`rust-fmt`、`rust-test`、`rust-clippy`、`cpp-runtime`、`cpp-zenoh-runtime`、`package`、`demo-smoke` 和 `release`，让格式化、Rust、C++、打包和示例 smoke 可以分层并行执行，release 仍等待完整 gate。
...
1. **multi tasks in one instance**：一个 instance 可以拥有多个 task，用于表达同一组件内的多个周期、事件和 lifecycle 执行单元。
2. **zenoh 与 ROS2 bridge**：通过 zenoh 桥接 ROS2 Jazzy 及之后版本，测试基线使用 Jazzy。
3. **变长消息机制修缮**：非 `iox2` backend 天然支持完整变长消息；`iox2` 保留 fixed-size channel 的共享内存路径，变长 channel 通过 route-level backend resolver 自动选择支持变长的 backend。
4. **大文件拆分**：按语义边界拆分过大的 parser、validator、codegen、CLI 或 runtime 文件，提升可读性、测试定位和长期维护性。
5. **CI 模块化编排**：把格式化、Rust 检查、C++ 构建、demo smoke、打包和 release 拆成合理 job，利用并行执行缩短反馈时间，并让失败原因更精确。
6. **full offline 依赖内嵌**：发布包内嵌 FlowRT 锁定版本的 Rust crate vendor、`iceoryx2-cxx`、`zenoh-c`、`zenoh-cpp` 和对应 license material；安装后用户不需要联网拉取 backend 依赖或手动安装第三方 SDK。
...
- 修复 C++ echo 数据面 probe 在带固定容量上限时重建内部 buffer 的问题，避免 bounded variable frame 或固定容量 snapshot 在观察者存在时被误判为 drop。
...
- 增加 bounded variable frame 主线：FlowRT 现在可以把 `bytes<max=N>`、`string<max=N>` 和 `sequence<T,max=N>` 生成成固定 header + 尾部变长区的 canonical frame codec，Rust/C++ runtime 与 codegen 都支持这套布局，并把它暴露给 `inproc`、`iox2` 和 `zenoh` 路径。
- 增加 `iox2` bounded variable frame transport slot：codegen 会为变长消息额外生成固定容量 iox2 payload slot，typed IPC service 传输 slot，用户组件接口继续使用结构化消息。
...
- 增加 `examples/mixed_zenoh_demo`：示例同时验证 Rust source、C++ sink、bounded variable frame、`zenoh` mixed launch 路径，以及跨主机 session 配置注入。
- 增加 `examples/variable_iox2_demo`：示例验证 Rust source 与 C++ sink 通过 iox2 fixed slot 跨语言传递 bounded variable frame，C++ sink 会校验 `string`、`bytes` 和 `sequence` 内容并可写 marker 文件供 CI smoke 断言。
...
- 增强 `flowrt echo` 输出：CLI 会按 self-description 的 Message ABI layout 格式化 fixed-size 字段和 bounded variable frame 字段，同时保留 raw/canonical bytes；`--follow [--interval-ms <ms>]` 会持续轮询同一 channel snapshot，并只在发布计数、时间戳或 payload 变化时输出。
...
- 增加 `iox2` endpoint 自动恢复：Rust/C++ typed endpoint 和 bounded variable frame slot endpoint 在本地 transport 资源丢失或操作失败后会按 `ReconnectPolicy` 重建本地 node、publisher 和 subscriber；frame codec 错误不触发重连，backend health 仍保持 ready。
...
- 增加 Message ABI bounded variable frame 主线：validator、codegen、Rust runtime 和 C++ runtime 现在都支持 `bytes<max=N>`、`string<max=N>` 和 `sequence<T,max=N>` 这类 bounded variable 字段，并会生成 canonical frame codec；`iox2` 通过 fixed-size transport slot 满足 typed payload 约束。
