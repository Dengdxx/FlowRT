...
- `v0.4.0` 的配套切片包括：为 ROS2 Service bridge 先固定语义和 manifest 边界，为 external process adapter 预留 process/service 接入边界，为 `flowrt pub` 和后续 record/replay 预留接口。参数热更新继续作为 runtime control-plane service-like RPC，不并入 graph 业务 Service 语义。
