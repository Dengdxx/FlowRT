...
- 增加 mixed runtime readiness 分类：CLI 会区分同进程 C++/Rust 混合、`inproc` 跨进程混合，以及 `iox2` 下仍缺 supervisor cross-language channel wiring 的真实缺口。
...
- 收窄 C++/iox2 backend readiness 保护：C++ only `iox2` contract 不再被 CLI 主动拒绝；mixed `run` / `launch` 仍在 supervisor 跨语言 channel wiring 完成前拒绝。
