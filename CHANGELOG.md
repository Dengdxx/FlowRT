...
- Rust inproc service 运行时：`ServiceRegistry` 注册 typed handler，`InprocServiceClient` 支持阻塞 `call()` 和非阻塞 `start_call()`，`InprocServiceServer` 由 request arrival 驱动 `process_pending_requests()`；支持有界请求队列、overflow 返回 `Busy`、zero timeout 返回 `Timeout`、server 未注册通过 registry 查询；same-lane 阻塞调用通过 thread-local `ACTIVE_LANES` 检测并返回 `WouldDeadlock`；`LaneGuard` RAII guard 管理 lane 活跃标记；`ServiceCallHandle` 支持 `poll()` 和 `complete()` 非阻塞等待；`ServiceStatsSnapshot` 暴露请求/成功/超时/繁忙/死锁计数；request arrival 通过 `ScheduleWaiter` 唤醒 scheduler waiter。
