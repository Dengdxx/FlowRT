...
- 新增 `flowrt init [path] --lang <rust|cpp>`，可生成现代 FlowRT app 项目骨架：
  `flowrt.toml`、`rsdl/robot.rsdl` 和 `app/rust/mod.rs` 或 `app/cpp/components.cpp`；
  当前明确不开放 C app 入口。
- 新增 FlowRT app 项目入口 `flowrt.toml`，`check`、`prepare`、`build`、`run`、`deps`
  和 `doctor` 省略 RSDL 路径时会从当前目录向上发现 `project.main`；显式 RSDL 路径仍保持
  优先。
- RSDL / Contract IR 增加 `language = "c"` component 基础语义，target runtime canonical
  ordering 同步纳入 `c`；当前只作为 C ABI v0 后续 codegen 的前置语义入口。
...
- C ABI v0 新增 C component callback table 基础边界：定义 context、fixed input view、
  output slot、生命周期 callback 和 task callback 的 POD 形状，并补齐 Rust/C++ layout
  测试；当前不开放 C codegen 或 C app demo。
- 新增 `flowrt add message/module/component`，可基于 `flowrt.toml` 默认主 RSDL 追加 message、
  workspace module 文件、Rust/C++ native component RSDL 片段和现代 `app/` 用户代码骨架；
  写入前会重新解析、归一化并校验更新后的 Contract IR，当前仍拒绝 C component 入口。

### 变更

- validator 对 C v0 component 采用 fail-fast 边界：暂不允许 `pkg_config`、参数、
  service、operation、variable frame、`io_boundary` 或 `external` kind；codegen 在 C app
  生成路径落地前会明确拒绝 C component，而不是写出半成品生成物。
...
- Rust generated shell 在含 `iox2` route 时改用 `ReadyBatch::run_local_collect`，保留
  two-phase output commit，但不再把 thread-affine iox2 endpoint 捕获进 worker closure。
...
- Rust/C++ `ReadyBatch` 增加 collect 路径，用于 worker 执行用户回调后按原 ready 顺序返回
  status 与可提交 output；非 `Ok`、panic 或 exception 路径都会丢弃本次 output。
...
- 补充 Rust/C++ runtime 覆盖：task-local output 成功后形成 commit record、非 `Ok`
  ...
  收集 output 时保持结果顺序。
- 补充 codegen 覆盖：Rust/C++ generated shell 使用 `ReadyBatch::run_collect`，输出提交
  块位于 scheduler loop，非 `Ok` 状态不会提交本次输出。
...
- **App SDK 化与 C ABI v0**：`v0.11.0` 围绕 FlowRT 自身语义推进 `flowrt init` /
  `flowrt add`、用户 API 可发现性、现代 `app/` 项目结构校验，以及 C ABI v0 与
  C component 最小可运行路径。旧 `src/` 用户结构不作为已存在历史处理，不提供兼容、
  fallback 或迁移提示；Python binding 不进入该版本。
...
- **语言边界与稳定性**：`v1.0.0` 聚焦 ABI/schema 冻结、长期兼容策略、故障注入、
  性能矩阵和版本兼容测试；0.x 版本继续承载功能突破和 SDK 体验完善。
