...
- C++ inproc service request/response 运行时：`InprocServiceServer<Req, Resp>` 注册 typed handler，`InprocServiceClient<Req, Resp>` 发起 deadline-bound 调用，`InprocServiceHandle<Resp>` 支持阻塞 `wait()` 和非阻塞 `poll()`。`InprocServiceRegistry` 单例管理 service 注册/注销。支持 queue depth、max_in_flight、overflow 返回 `Busy`、server 销毁后返回 `Unavailable`、same-lane 死锁检测返回 `WouldDeadlock`、handler 异常返回 `HandlerError`、业务错误透传。请求到达可选回调通知 runtime（不依赖 tick polling）。默认 timeout 5000ms，不允许无界阻塞。
