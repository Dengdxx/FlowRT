...
- iox2/zenoh endpoint 需要保持 peer endpoint 重建后的继续收发回归测试。Runtime 提供 C ABI 友好形状的 `BackendHealthState`、`BackendHealthSnapshot`、`ReconnectPolicy` 和 `BackendHealthTracker`：状态只表达 `ready`、`degraded`、`reconnecting`、`failed`，退避策略使用毫秒和 attempt 数，后续 C、Python 或更多语言 runtime 应复用该稳定形状。`zenoh` endpoint 已接入自动恢复，本地 session 关闭或 transport 操作失败会重建 session、publisher 和 subscriber；codec/schema 错误不触发重连。`iox2` endpoint 自动恢复尚未接入前，generated shell 对 iox2 transport error 仍按 fatal `Status::Error` 处理；后续接入必须复用这套抽象，不要在 shell 中临时吞掉错误。
