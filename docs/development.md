...
focused smoke、amd64/arm64 C++ zenoh runtime、amd64/arm64 deb package、v0.8.3 安装版
amd64 到 arm64 cross smoke、amd64/arm64 demo smoke、amd64/arm64
...
`examples/island_demo`，通过 `flowrt pub` 向 boundary input 注入 fixed Message ABI JSON，
再用 `flowrt echo` 校验 boundary output。该 gate 保证 island 脚手架、边界输入输出和
安装后 CLI 观测路径一起进入 release 主路径。
