...
- 扩展 backend capability 时，先在 `flowrt-ir` 的 typed capability catalog 中增加内部枚举项，再由 `backend_capabilities`、`channel_capabilities`、`trigger_capability` 或 message ABI 推导函数输出既有 `CapabilityAtom` 字符串。不要在 validator、normalizer 或 codegen 中散落新 capability 字符串；新增 backend 必须补唯一且顺序稳定的 capability 列表测试。
