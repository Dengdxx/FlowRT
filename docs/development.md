...
- C++ runtime introspection API 要保持与 Rust JSON-line wire 格式兼容：`status` 返回 handshake、tick 和 channel 摘要，`channel_snapshot` 只返回 raw ABI bytes、发布计数和发布时间，未知 channel 返回结构化 error。当前 C++ 侧只是 runtime API 基础，generated C++ shell 尚未自动启动 socket 或记录 live state，文档和测试不要提前声明 C++ 示例已可被 `flowrt status` / `flowrt echo` 发现。
