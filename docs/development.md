...
- iox2/zenoh endpoint 需要保持 peer endpoint 重建后的继续收发回归测试；但 generated shell 当前对真实 transport error 仍按 fatal `Status::Error` 处理。自动重连、backend health 和 retry/backoff policy 需要单独设计，不要在 shell 中临时吞掉错误。
