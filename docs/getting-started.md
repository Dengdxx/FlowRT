...
`context.timing()` 读取 timing，并可用 `context.now_ms()` / `now_secs()` / `dt_ms()` /
`dt_secs()` 直接取当前调度时间与相对上一次运行的步长；C callback context 指针可先检查
`context->has_timing`，再读取 `context->timing`。handler 签名不因此改变。这是用户算法
获取时间的规范入口：`realtime` 运行时是 runtime monotonic observed scheduling time，
`simulated_replay` 是注入事件 fixture 驱动的逻辑时间。用户算法不要直读 `steady_clock` /
`Instant::now` / `SystemTime`，否则回放将失去确定性。生命周期 context 默认不携带 timing，
用户代码必须按可选值（`None` / `std::nullopt`）处理。
...
`flowrt/app/stubs/<module>/cpp/src/<component>.cpp`。它们和其他 `flowrt/` 内容一样由 FlowRT
管理，可以删除后重新生成。用户算法代码应放在项目自己的 `app/` 目录，不放进生成目录；
`prepare` 不直接修改用户 `app/`。module component 的建议用户路径是
`app/<module>/rust/<component>.rs`、`app/<module>/cpp/src/<component>.cpp` 或
`app/<module>/c/<component>.c`；C++ headers 建议放在 `app/<module>/cpp/inc/`，Rust 仍通过
`app/rust/mod.rs` 聚合并暴露 `build_app()`。
...
`params set` 的值必须是合法 JSON。`on_tick` 参数会在下一个 tick 边界通过用户组件的
`on_params_update` 钩子提交；钩子返回非 `Ok` 时 pending 更新被拒绝，旧参数继续生效。
`startup` 参数运行时不可修改。
