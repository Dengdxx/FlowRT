...
或 `context.now_ms()` / `dt_ms()` 等规范入口，C callback context 指针使用
`context->has_timing` / `context->timing`；生命周期 context 默认无 timing，`realtime` 与
`simulated_replay` 两种 clock source 分别按 runtime monotonic observed scheduling time 和
注入事件驱动的 deterministic 逻辑时间区分。新项目应按 `flowrt init`、可选 `flowrt add ...`、
...
- `flowrt/app/app_api.json` 和 `flowrt/app/implementation.md` 会把 module component 的建议
  ...
  会生成到 `flowrt/app/stubs/<module>/cpp/src/<component>.cpp`。C/C++ module sources 会被
  generated CMake 自动发现，C++ headers 建议放在 `app/<module>/cpp/inc/`；Rust module
  文件仍由 `app/rust/mod.rs` 聚合。
...
- `iox2` 承载 fixed-size plain data 和带 `max=N` 上界的 bounded variable frame；如果 route 使用无界 variable frame，Contract IR 会把该 route 自动选择到支持无界变长消息的 backend（当前为 `zenoh`），不生成无界变长 over iox2 的兼容承载层。
...
- self-description 与 `launch/launch.json` 对 iox2 endpoint 暴露 `service` 字段，而不是
  zenoh 的 `key_expr` 字段。
- `iox2` 承载 fixed-size plain data 和 bounded variable frame；Service request/response
  或 Operation start goal 含 `bytes<max=N>`、`string<max=N>` 或 `sequence<T,max=N>` 时，
  generated shell 使用定容 frame slot。含无界 `bytes`、`string` 或 `sequence<T>` 时，
  省略 backend 会按 edge fallback 到 `zenoh`，显式 `backend = "iox2"` 会被 validator 拒绝。
...
多传感器同步示例。`imu_src`、`odom_src` 各周期产样（消息声明 `[type.X.timestamp]`
sample-time 源），`[[sync]]` 组 `fused_in` 把两路输入按 sample-time 对齐：
...
`fusion` 用 `trigger = "on_synchronized"`、`sync = "fused_in"`，只在 synchronizer
发射出对齐集（两路 sample-time 跨度 ≤ `tolerance_ms`）时被调用，回调收到一对时间
对齐的 typed 样本；融合结果经 `latest` channel 投递给 `sink`。

```bash
flowrt check examples/sync_fusion_demo/rsdl/robot.rsdl
```
