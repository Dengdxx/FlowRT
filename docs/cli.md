...
timeout/deadline 由 runtime hidden task 驱动，不依赖用户 handler 自觉检查。`backend =
...
`op result` 依赖 contract 的 `result_retention_ms`；retention 过期或 runtime 未保留该
invocation 时会返回 not found。
...
依赖 runtime event retention；retention 过期或 runtime 未保留该 invocation 时会返回 not
found。
