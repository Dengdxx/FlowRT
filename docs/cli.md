...
- 每个 graph 内实际使用的 component language、kind 和建议用户实现路径：Rust 为
  ...
  会显示为 `app/<module>/rust/<component>.rs`、`app/<module>/cpp/<component>.cpp` 或
  `app/<module>/c/<component>.c`。Rust 生成入口仍是 `app/rust/mod.rs`，用户可在其中
  聚合 module-local Rust 文件。
...
仍在 `flowrt/app/stubs/<lang>/`；module component 的 stub 位于
...
可按 `app/<module>/<lang>/<component>.*` 组织。generated CMake 会自动发现
`app/<module>/cpp/**` 和 `app/<module>/c/**`；Rust 仍由 `app/rust/mod.rs` 聚合。
