...
- task context timing 能力：Rust/C++ 通过 `context.timing()` 读取可选 timing，C callback
  context 通过 `context->has_timing` / `context->timing` 读取；handler 签名不因此改变，
...
`explain` 和 `prepare` 中的 timing 说明只描述 FlowRT runtime 看到的调度时序。realtime
运行时使用 runtime observed scheduling time；replay / temporary island 使用 fixture
驱动的 deterministic timing。它不承诺硬实时，也不引入 sensor timestamp、event-time、
clock domain、PTP、NTP 或 approximate sync 语义。
