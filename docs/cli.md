...
`flowrt_app_<instance>_callbacks()` 接入线索。当前 C 用户入口只支持 native component 和
fixed-size plain data message，并可把 bool、整数、浮点、string、array/table params 作为
callback context 中的 readonly snapshot 暴露；其中 string 是 borrowed UTF-8 view，array/table
继续是 JSON view。service、operation、variable frame、`io_boundary` 和 `external`
不在 C 用户入口范围内。
...
`flowrt add component --lang c` 只追加当前 C 用户入口能表达的 native component 契约。它不会
...
Message ABI v0.1 仍以 fixed-size plain data 作为 native ABI 基线，但 RSDL type expression 现在也可以解析 `bytes`、`string`、`sequence<u32>` 这类无界 variable 字段，以及 `bytes<max=N>`、`string<max=N>`、`sequence<u32,max=N>` 这类有界 variable 字段。选中 backend 具备 `abi:variable_payload_frame` 时，`prepare` 和 `build` 生成的产物会输出 canonical frame codec。`iox2` 不承载无界变长或指针所有权 payload；当 profile 默认 backend 为 `iox2` 且某条 route 使用无界 variable frame 时，该 route 会自动选择支持无界变长消息的 backend（当前为 `zenoh`），其他 fixed-size 和 bounded variable route 仍继续走 `iox2`。
...
- 每个 graph 内实际使用的 component language、kind 和建议用户实现路径：Rust 为
  `app/rust/mod.rs`，C++ 为 `app/cpp/**`，C 为 `app/c/<component>.c`；module component
  会显示为 `app/<module>/rust/<component>.rs`、`app/<module>/cpp/src/<component>.cpp`
  或 `app/<module>/c/<component>.c`。C++ module headers 建议放在
  `app/<module>/cpp/inc/`；Rust 生成入口仍是 `app/rust/mod.rs`，用户可在其中聚合
  module-local Rust 文件。
- task name、trigger、period、readiness、lane、concurrency 和所属 instance。
- task context timing 能力：Rust/C++ 通过 `context.timing()` 读取可选 timing，并可用
  `context.now_ms()` / `now_secs()` / `dt_ms()` / `dt_secs()` 直接取调度时间与积分步长；
  C callback context 通过 `context->has_timing` / `context->timing` 读取；handler 签名不
  因此改变，生命周期 context 默认不携带 timing。用户算法应经这些入口取时间，不直读
  `steady_clock` / `Instant::now` / `SystemTime`，否则 `simulated_replay` 回放将失去确定性。
...
graph default
  ...
  task timing context: access=rust:context.timing() cpp:context.timing() c:context->has_timing/context->timing available_in_task_context=true available_in_lifecycle_context=false handler_signature_changed=false
  timing fields=scheduled_time_ms|observed_time_ms|scheduled_delta_ms|observed_delta_ms|lateness_ms|missed_periods|deadline_missed|overrun clock_sources=runtime|replay
...
由 generated `flowrt_app/c_components.h` 声明。该入口仍是静态 C callback table 接入路径，
不表示完整 C runtime、动态加载或 Python binding 能力。

`explain` 和 `prepare` 中的 timing 说明只描述 FlowRT runtime 看到的调度时序。clock source
是 Contract IR 一等概念：常规合同为 `realtime`，temporary island overlay 为
`simulated_replay`（validator 重新推导并拒绝不一致取值，`external_stepped` 暂不支持）。
`realtime` 运行时使用 runtime monotonic observed scheduling time；`simulated_replay` 由
注入事件 fixture 的逻辑时间驱动调度器，不读 wall-clock 节拍，因此回放结果只取决于事件
...
manifest 渲染的用户实现清单；`flowrt/app/stubs/` 是参考模板。root component 的 stub
仍在 `flowrt/app/stubs/<lang>/`；module C++ component 的 stub 位于
`flowrt/app/stubs/<module>/cpp/src/<component>.cpp`，其他 module language 仍按
`flowrt/app/stubs/<module>/<lang>/<component>.*`。三者都是 FlowRT 管理产物，可删除后由
`flowrt prepare` 重建。用户可以参考 `stubs/` 后在项目 `app/` 保留实现；`prepare` 不直接
创建、追加或覆盖用户 `app/`。
...
- C++ contract 含 Operation 但 Operation backend 不是 `zenoh` 时，CMake 会 opportunistic 查找 `zenohcxx::zenohc` 以启用远程 `flowrt op ... --remote` control-plane；找不到 SDK 不会阻止本机 socket control-plane 或 inproc/iox2 data-plane 构建。
...
| 字段 | 类型 | 默认值 | 说明 |
|------|------|--------|------|
| `backend` | string | profile/default backend | 传输后端：native generated Service 与 Operation 支持 `inproc`、`iox2` 与 `zenoh`；external endpoint 可在 manifest 中选择 `zenoh` |
...
- `backend = "iox2"` 支持 fixed-size plain-data request/response，以及带 `max=N`
  上界的 bounded variable request/response；包含无界 `bytes`、`string` 或
  `sequence<T>` 的 request/response 若显式选择 `iox2` 会被拒绝。省略 `backend` 且
  profile/default backend 为 `iox2` 时，无界 payload service 会按 edge 自动 fallback 到
  `zenoh`。

iox2 service 的 launch manifest 和 self-description 会暴露 canonical service name，
格式为 `FlowRT/service/<escaped service-name>`。zenoh service 会暴露 request key expression，
格式为 `flowrt/service/<escaped service-name>/request`，其中 service name 使用
`<client_instance>.<client_port>`。CLI 输出中 `service` 和 `key_expr` 是不同字段：
`service` 表示 inproc/iox2 canonical endpoint，`key_expr` 只表示 zenoh transport key。
...
`sequence<primitive>`、`sequence<fixed struct>`、对应 `max=N` 有界形式和嵌套 fixed
struct header 字段。显式 `empty = true` 的空消息可以用 `{}` 或 `null` 注入，payload
长度为 0。
...
`boundary input` 注入事件；它不读取 ROS2 bag，不转换 ROS2 MCAP，也不向普通生产 channel
写入。strict self-description 会被拒绝，错误会提示使用 island profile 或 temporary island
overlay。fixture 中的
...
传入 `.mcap` 时，`replay` 读取 FlowRT record envelope 中的 Operation command timeline，只重放
`flowrt.operation.command.start.v1` 和 `flowrt.operation.command.cancel.v1`。progress、
result、error、status 等 Operation observation event 只作为证据，不会重放，避免双执行。
...
flowrt op status 111:7:3 --socket /run/user/1000/flowrt/12345.sock
...
或 `message_frames` canonical frame layout 解码后的 JSON 对象；bounded `string<max=N>`、
`bytes<max=N>` 和 `sequence<T,max=N>` 使用 generated self-description 中的同一 layout。
`op start --follow --format json` 输出单个 `operation_start_follow` JSON 文档，内部包含
`started` 和 `follow` 两个对象。远程 text 输出继续在 stderr 打印 `target:`；远程 JSON
输出把命中 runtime 写入 `runtime` 字段。
...
invocation id、owner 和 deadline；同一 scope 的第二个 owner start 会返回结构化冲突错误。
...
timeout/deadline 由 runtime hidden task 驱动，不依赖用户 handler 自觉检查。runtime hidden
task 同时驱动 result retention 清理：`result_retention_ms = 0` 不保留已完成 invocation，
retention 过期后 result payload 和 observation event page 都不可再按 id 查询。`backend =
"iox2"` 时，start/cancel/status control path 走内部 iox2 service transport；`backend =
"zenoh"` 时，start/cancel/status control path 走内部 zenoh service transport。`iox2`
Operation 支持 fixed-size plain-data goal/feedback/result；goal 含 bounded variable frame 时，
start service 使用定容 frame slot，cancel/status 仍保持 fixed-size。无界动态 payload 会按 edge
fallback 到 `zenoh`（省略 backend）或 fail-fast（显式 `iox2`）。用户 API 始终保持 Operation
语义。
...
`op status` 读取 live runtime status。省略参数时输出所有 live Operation；传入
`<client_instance>.<client_port>` 时只输出该 Operation；传入 `operation_id`（例如
`111:7:3`）时，CLI 通过 introspection `OperationStatus` 请求精确查询该 invocation，并输出
`operation_id=...` 前缀。按名称输出格式与 `status` 的 operation 行一致：

远程 `op status <operation_id> --remote` 通过 zenoh Operation control-plane 查询远端
runtime。generated Rust runtime 会启用该 queryable；generated C++ runtime 仅在构建时找到
`zenohcxx::zenohc` 并定义 `FLOWRT_HAS_ZENOH_CXX` 后启用，缺少 SDK 时仍可使用本机
socket control-plane。CLI 读取 `--image` 的 self-description hash，扫描
`flowrt/op/{package}/{selfdesc_hash}/{pid}` key expression，并只选择 hash 匹配的 runtime；
多个匹配时要求用 `--runtime <key_expr>` 精确选择。`--socket` 是本机 Unix socket，不能与
`--remote` 同用。`--timeout-ms` 控制发现和请求超时，默认 5000ms。CLI 会在 stderr 输出
`target:` 行，明确命中的 runtime。
...
`--file` 中的 goal JSON 编码成 wire payload；canonical frame 输入同时支持无界和
`max=N` 有界 variable 字段。`--timeout-ms` 覆盖 start 请求超时；省略时使用 contract 中的
默认超时。成功输出 accepted invocation id 和当前 live status：
...
远程 `op cancel <operation_id> --remote` 使用同一个 zenoh Operation control-plane 和
`flowrt/op/{package}/{selfdesc_hash}/{pid}` targeting 模型，返回取消请求后的 Operation status
快照。

`op result <operation_id>` 读取 runtime retained result。必须传入 `--image`，因为 CLI 需要
self-description 中的 Operation result type 和 Message ABI / `message_frames` layout 才能把
retained wire payload 解码为 JSON；canonical frame 解码同时支持无界和 `max=N` 有界
variable 字段。本机路径会校验 socket handshake 的 self-description hash；远程路径复用
`flowrt/op/{package}/{selfdesc_hash}/{pid}` targeting，可用 `--runtime` 精确选择。成功
result 输出示例：

```text
operation_id=111:7:3 operation=controller.plan state=succeeded result={"accepted":true}
```

失败、取消和超时没有 typed result payload，输出 state/error：

```text
operation_id=111:7:3 operation=controller.plan state=failed error=handler error
```

`op result` 依赖 contract 的 `result_retention_ms`；`result_retention_ms = 0` 的 invocation
完成后不会进入 retained result，retention 过期或 runtime 未保留该 invocation 时会返回 not
found。

`op follow <operation_id>` 读取 runtime retained observation event。必须传入 `--image`，
因为 CLI 需要 self-description 中的 Operation feedback/result type 和 Message ABI /
`message_frames` layout；canonical frame 解码同时支持无界和 `max=N` 有界 variable 字段。
runtime 返回按游标排序的 `state`、`progress`、`result` 和 `error` event；CLI 会持续轮询到
terminal。progress/result payload 按 self-description 解码为 JSON：

```text
operation_id=111:7:3 state=running
operation_id=111:7:3 progress_sequence=1 progress={"percent":50}
operation_id=111:7:3 state=succeeded result={"accepted":true}
```

远程 `op follow <operation_id> --remote` 复用
`flowrt/op/{package}/{selfdesc_hash}/{pid}` targeting，可用 `--runtime` 精确选择。`op follow`
依赖 runtime event retention；`result_retention_ms = 0` 的 invocation 完成后不会保留 event
page，retention 过期或 runtime 未保留该 invocation 时会返回 not found。
...
`flowrt.operation.command.cancel.v1` command event。它们是 replay 输入；Operation
state/progress/result/error/status event 是 observation evidence，不作为 replay command。
...
```text
recorded output=run.mcap socket=/run/user/1000/flowrt/12345.sock event_count=42 dropped_count=0 bytes_written=4096 active_filters=[all] descriptor_payload=descriptor_only payload_artifact_count=0
```
...
当 RSDL descriptor schema 显式声明 `record_payload = true` 和
`payload_capture = "boundary"` 时，I/O boundary 可以调用
`record_frame_descriptor_payload_event` 上报 payload artifact；MCAP descriptor event 会记录
`artifact_ref`、`content_hash` 和 `size_bytes`，CLI 摘要会输出
`descriptor_payload=artifact_ref payload_artifact_count=N`。FlowRT 仍不会把真实 payload 复制成
普通 channel sample。
...
profile 投影还会重算来自 profile default 的 bind-level policy：未在 `bind.dataflow` 上显式声明的 `overflow`、`stale_policy` 和 `max_age_ms` 会采用选中 profile 的默认值；bind 上显式声明的 policy 保持不变。未显式声明 `backend` 或声明 `backend = "auto"` 的 bind 会跟随选中 profile backend；如果该 route 使用无界 variable frame 且 profile backend 为 `iox2`，会自动选择 `zenoh`；带 `max=N` 的 bounded variable frame 会留在 `iox2`。`backend` 是单条 route 的属性；同一 `from`/`to` route 只能声明一次，跨 import 的 RSDL 片段会先合并成一个 Contract IR，再由 validator 拒绝多重 incoming bind 或冲突连线。投影后的 `contract.ir.json` 会同时刷新 route 和 deployment 的 capability 元数据。
...
`flowrt_user::build_app()` 接入；Rust 用户代码通过生成 trait 和用户模块接入；C 用户代码
通过 `flowrt_app/c_components.h` 声明的 callback table factory 接入。
root component 继续使用 `app/rust/mod.rs`、`app/cpp/**` 和 `app/c/**`；module component
可按 `app/<module>/rust/<component>.rs`、`app/<module>/cpp/src/<component>.cpp` 和
`app/<module>/c/<component>.c` 组织。generated CMake 会自动发现
`app/<module>/cpp/src/**` 和 `app/<module>/c/**`，并把 `app/<module>/cpp/inc` 加入 C++
include 路径；Rust 仍由 `app/rust/mod.rs` 聚合。
