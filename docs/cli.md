...
self-description 中的 Operation result type 和 Message ABI layout 才能把 retained wire payload
解码为 JSON。本机路径会校验 socket handshake 的 self-description hash；远程路径复用
`flowrt/op/{package}/{selfdesc_hash}/{pid}` targeting，可用 `--runtime` 精确选择。成功 result
输出示例：
...
因为 CLI 需要 self-description 中的 Operation feedback/result type 和 Message ABI layout。
