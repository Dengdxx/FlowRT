...
或 `message_frames` canonical frame layout 解码后的 JSON 对象。`op start --follow
--format json` 输出单个 `operation_start_follow` JSON 文档，内部包含 `started` 和
`follow` 两个对象。远程 text 输出继续在 stderr 打印 `target:`；远程 JSON 输出把命中
runtime 写入 `runtime` 字段。
...
contract 中的默认超时。成功输出 accepted invocation id 和当前 live status：
...
retained wire payload 解码为 JSON。本机路径会校验 socket handshake 的 self-description
hash；远程路径复用 `flowrt/op/{package}/{selfdesc_hash}/{pid}` targeting，可用 `--runtime`
精确选择。成功 result 输出示例：
