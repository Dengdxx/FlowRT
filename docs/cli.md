...
`--lang c` 使用 C ABI v0 callback table：用户 C 文件实现
`flowrt_app_<instance>_callbacks()`，并设置
`FLOWRT_ABI_FEATURE_C_COMPONENT_CALLBACKS_V0`。当前只支持 native component 和
fixed-size plain data message；params、service、operation、variable frame、
`io_boundary` 和 `external` 不在 C v0 用户入口范围内。
...
task，同时把 Rust 骨架合并到 `app/rust/mod.rs`，把 C++ 骨架合并到
`app/cpp/components.cpp`，或为 C v0 生成 `app/c/<component>.c`：
...
`flowrt add component --lang c` 只生成当前 C v0 能表达的 native callback table skeleton。
它不会扩展 service、operation、variable frame、`io_boundary`、`external` 或 Python
binding，也不会把用户代码写入 `flowrt/`。
...
预检仍可使用。`launch`、`bundle` 和 `deploy` 不做默认 RSDL 发现。
...
`bundle.toml` 写入 `artifact_mode = "strict" | "island"`，并默认拒绝 `island` 产物；只有
明确用于开发、测试或迁移对比时才传入 `--allow-island`。
...
如果 bundle manifest 标记 `artifact_mode = "island"`，`deploy` 默认拒绝部署。需要把 island
脚手架临时部署到测试机器时，必须显式传入 `--allow-island`；迁移完成后的普通生产包应删除
boundary endpoint、切回 `strict` profile 后重新构建和打包。
...
会被拒绝，错误会提示使用 island profile 或 temporary island overlay。
