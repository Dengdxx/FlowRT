...
- `backend = "iox2"` 只支持 fixed-size plain-data request/response；包含 `bytes`、
  `string` 或 `sequence<T>` 的 request/response 若显式选择 `iox2` 会被拒绝。省略
  `backend` 且 profile/default backend 为 `iox2` 时，动态 payload service 会按 edge
  自动 fallback 到 `zenoh`。
...
Operation 只支持 fixed-size plain-data goal/feedback/result；动态 payload 会按 edge fallback
到 `zenoh`（省略 backend）或 fail-fast（显式 `iox2`）。用户 API 始终保持 Operation 语义。
