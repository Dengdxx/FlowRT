...
FlowRT skill 只服务 FlowRT 范围内的可复用工作流。当前阶段优先写 `frt-core-*`
和跨 core/app 编排通用的 `frt-subtask`；`frt-app-*` 仅在用户明确放开并且工作流
依赖已发布的 FlowRT app 主路径时允许。它可以借鉴通用 skill 的短入口、渐进披露
和硬门控，但不能写成适配任何项目的泛用流程。

<HARD-GATE>
写任何 `SKILL.md` 前必须先确认：

1. 目标 workflow 属于 `frt-core-*`、`frt-app-*` 或既有 `frt-subtask`。
2. 用户明确说明这个 skill 要减少哪类 FlowRT 返工或错误。
3. 已读 `.agents/skills/README.md` 和同前缀现有 skill，确认没有重复。
4. 已列出该 skill 必须守住的 FlowRT 语义边界和验证证据。
5. 若创建 `frt-app-*` skill，必须确认用户已明确放开 app skill 禁令，且该 skill
   只依赖已发布或已明确进入主线的用户能力。
...
| `frt-app-*` | FlowRT app 开发 | 用户明确放开后可写；必须围绕稳定或已发布的 app 主路径。 |
| `frt-subtask` | FlowRT 子任务编排 | core 和 app 通用的私有 skill；不要复制到其他目录。 |

不要新增其他裸名 skill。`build-run`、`debug-flow`、`write-code` 这类名称必须改成
带 FlowRT 前缀的稳定名称。app 侧 skill 不得固化尚未落地的推测性用法。

## Process

1. **归类**
   - 判断目标是否维护 FlowRT 仓库、开发 FlowRT app，或是否属于跨 core/app 的子任务编排。
   - 如果目标是开发 FlowRT app，确认用户已明确放开 app skill 禁令，并确认该 workflow
     依赖的 FlowRT 用户能力已经发布或已明确进入主线。
...
- [ ] name 和目录名一致，且为 `frt-core-*`、`frt-app-*`、`write-frt-skill` 或既有
  `frt-subtask`。
- [ ] 若新增 `frt-app-*` skill，已确认用户明确放开 app skill 禁令，且未固化未发布能力。
