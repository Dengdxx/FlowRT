...
和跨 core/app 编排通用的 `frt-subtask`，不写 `frt-app-*`。它可以借鉴通用
skill 的短入口、渐进披露和硬门控，但不能写成适配任何项目的泛用流程。

<HARD-GATE>
写任何 `SKILL.md` 前必须先确认：

1. 目标 workflow 属于 `frt-core-*` 或既有 `frt-subtask`。
...
1. **归类**
   - 判断目标是否维护 FlowRT 仓库，或是否属于跨 core/app 的子任务编排。
