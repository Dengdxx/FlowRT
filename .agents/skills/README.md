...
当前阶段优先落地 `frt-core-*` 和跨 core/app 编排通用的 `frt-subtask`。`frt-app-*`
只有在用户明确放开，并且 workflow 依赖已发布或已明确进入主线的 FlowRT app 能力时
才允许落地；app skill 必须避免固化未发布的推测性用法。
...
| `frt-app-*` | FlowRT app 开发者 | 围绕已发布或已进入主线的 app 工作流，帮助开发者正确使用 FlowRT。 |
...
## `frt-app-*`：使用 FlowRT 开发 app

app skill 面向 FlowRT app 开发者，不用于维护 FlowRT core。新增 app skill 必须满足：

- 用户明确放开或指定该类 skill。
- 依赖能力已发布或已明确进入主线。
- 不把未稳定的推测性用法写成教程。
- 不把 ROS2、backend SDK、硬件 driver 或外部框架概念冒充 FlowRT 核心语义。

| 优先级 | Skill | 触发条件 | 职责 | 硬门控 |
|---|---|---|---|---|
| P0 | `frt-app-ros2-migration` | 迁移 ROS2 package/node 到 FlowRT，或用 island mode 做单功能单位 IO 测试 | 盘点 ROS2 IO，建立 FlowRT migration island，行为对比后拆除脚手架 | strict 模式不得残缺生成，boundary 不得替代长期拓扑 |
...
- 使用 `write-frt-skill`。
- 在 `README.md` 表格中登记。
- 必须属于 `frt-core-*`、`frt-app-*` 或既有 `frt-subtask`。
- `frt-app-*` 必须依赖已发布或已明确进入主线的 app 能力。
